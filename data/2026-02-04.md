...
TL;DR: 提出了一种统一的牛顿-舒尔茨正交化(UNSO)框架，通过可学习系数优化多项式，避免传统迭代范式中的重复矩阵乘积计算负担


<details>
  <summary>Details</summary>
Motivation: 传统牛顿-舒尔茨迭代在效率和稳定性方面存在问题，现有改进方法仍受限于传统迭代范式，导致沿长维度重复矩阵乘积带来巨大计算负担

Method: 将迭代结构整合为统一框架(UNSO)，避免多项式展开，评估各矩阵幂次的作用，移除不显著项，提供具有可学习系数的推荐多项式，并优化这些系数

Result: 方法实现了出色的性能表现和稳定收敛，代码已开源

Conclusion: UNSO框架通过可学习系数优化多项式，有效解决了传统牛顿-舒尔茨迭代的计算效率和稳定性问题
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习方法面临三个主要挑战：1）跨任务参数重用效率低下；2）任务不相似时存在灾难性遗忘风险；3）为每个任务引入新参数阻碍了相似任务间的知识共享。需要一种更高效的参数重用机制来平衡知识保留和参数效率。

Method: SAFM采用两阶段框架：决策阶段和调优阶段。决策阶段通过架构搜索决定是引入新适配器、重用现有适配器还是添加空适配器，优先重用或添加空适配器以最小化参数消耗。调优阶段采用分层损失函数促进适配器间的差异化，有效捕获同一任务内的知识。

Result: 实验结果表明SAFM持续优于最先进方法，在保持可比性能的同时，参数使用量减少60%以上，实现了参数效率和性能的良好平衡。

Conclusion: SAFM通过动态融合新旧适配器的两阶段框架，有效解决了持续学习中的参数重用效率和灾难性遗忘问题，为自然语言处理中的持续学习提供了更高效的解决方案。
...
TL;DR: ORDER是一个针对复合材料的多模态预训练框架，通过建立序数感知的图像-表格对齐机制，解决了复合材料连续非线性设计空间在数据稀缺条件下的表征学习问题。


<details>
  <summary>Details</summary>
Motivation: 复合材料具有连续非线性设计空间，缺乏明确的图结构，传统基于离散图表示的方法失效。现有多模态框架在数据丰富的晶体或聚合物系统中有效，但无法处理复合材料在极端数据稀缺条件下的连续设计空间问题。

Method: 提出ORDinal-aware imagE-tabulaR alignment (ORDER)框架，将序数性作为复合材料表征的核心原则。该框架确保具有相似目标特性的材料在潜在空间中占据相邻区域，有效保留复合材料特性的连续性，并能在稀疏观测的设计之间进行有意义的插值。

Result: 在公开的纳米纤维增强复合材料数据集和内部构建的碳纤维T700数据集上，ORDER在特性预测、跨模态检索和微结构生成任务中均优于最先进的多模态基线方法。

Conclusion: ORDER框架通过引入序数感知对齐机制，成功解决了复合材料在数据稀缺条件下的连续设计空间表征问题，为复合材料的多模态学习提供了有效解决方案。
...
TL;DR: GraphDancer：基于强化学习的框架，教会LLM通过交替推理和函数执行来导航异构图，使用图感知课程提升训练效果，在跨域泛化上表现出色


<details>
  <summary>Details</summary>
Motivation: 现实世界知识源多为异构图而非纯文本，LLM需要处理结构化关系导航和多跳证据聚合两大挑战，现有方法难以有效应对图结构知识推理

Method: 提出GraphDancer强化学习框架，让LLM学习在图中交替进行推理和函数执行；引入图感知课程，通过结构复杂度调度训练，使用易到难的偏置采样器

Result: 仅使用3B骨干模型，GraphDancer在跨域基准测试中超越了配备14B骨干或GPT-4o-mini的基线方法，展示了强大的图探索和推理技能泛化能力

Conclusion: GraphDancer通过强化学习和结构化课程训练，使中等规模LLM能够有效导航异构图知识源，实现跨域泛化，为图结构知识推理提供了有效解决方案
...
TL;DR: 论文提出SDPA的新数学形式，将其重新解释为输入向量在由输入自身决定的公共曲面上的投影，而非传统的"查询-键-值"概念，这为时间序列数据的非线性依赖建模提供了新视角。


<details>
  <summary>Details</summary>
Motivation: 传统SDPA基于数据库理论的"查询-键-值"概念，难以与数学信号处理的标准方法相协调。作者旨在为SDPA提供更符合信号处理框架的数学解释，并探索其在时间序列数据分析中的理论基础。

Method: 将SDPA重写为数学等价但形式不同的表达式，将其表示为输入向量在由输入自身确定的公共曲面上的投影。这种重写揭示了SDPA发现时间依赖和上下文依赖的非线性依赖的能力。

Result: 新形式不仅提高了前馈和学习算法的速度，更重要的是为SDPA的扩展提供了可能性。在语言处理背景下，SDPA被重新解释为寻找由输入向量所在曲面决定的时变上下文意义。

Conclusion: SDPA的新数学形式为时间序列数据中时变局部非线性依赖的建模提供了强有力的理论依据，与"自注意力"概念有本质区别，为SDPA在更广泛信号处理应用中的使用奠定了理论基础。
...
TL;DR: IMU-1是一个430M参数的语言模型，在72B tokens上训练，性能接近使用56倍更多数据训练的基准模型


<details>
  <summary>Details</summary>
Motivation: 研究如何通过优化的训练配方，用相对较少的数据量达到与大规模数据训练模型相当的性能，提高训练效率

Method: 结合架构改进（QK-norm注意力、per-head门控、value残差、LayerNorm缩放）、优化技术（NorMuon优化器、谨慎权重衰减、muP参数化）以及三阶段训练计划与后验检查点EMA

Result: IMU-1在72B tokens上训练达到接近使用56倍更多数据（约4T tokens）训练的基准模型性能

Conclusion: 通过系统化的训练配方优化，可以在相对较少的数据量下实现接近大规模数据训练模型的性能，为高效语言模型训练提供了可复现的解决方案
...
TL;DR: 研究发现传统困惑度(PPL)在上下文稳定化机制中具有欺骗性，揭示了一种"语义隧道"失效模式，并提出MNCIS框架通过流形展开来防止语义多样性崩溃。


<details>
  <summary>Details</summary>
Motivation: 传统上使用困惑度(PPL)来监控基于递归合成数据训练的生成式AI的稳定性，但作者发现PPL在上下文稳定化机制中具有欺骗性，无法检测到语义多样性的灾难性损失。

Method: 采用严格的滑动窗口协议(N=1500)，识别"语义隧道"失效模式；应用多尺度负耦合信息系统(MNCIS)框架，特别是自适应谱负耦合(ASNC)作为拓扑算子，诱导"流形展开"。

Result: 基线模型在保持高语法流畅性(PPL约83.9)的同时，在七代内语义多样性灾难性损失，收敛到单一低熵叙事吸引子("Robert Boulton"奇点)；MNCIS将有效秩从各向异性基线3.62扩展到超多样状态5.35，构建了抵抗语义吸引子引力的人工流形。

Conclusion: 困惑度(PPL)在上下文稳定化机制中是一个具有欺骗性的指标；语义隧道现象代表了潜在流形的完全崩溃；MNCIS框架通过流形展开有效防止语义多样性崩溃，保持训练数据的长尾分布。
...
TL;DR: 基于深度强化学习的主动流动控制策略，用于稳定马赫5高超声速进气道的不启动现象，具有强零样本泛化能力和实时部署潜力。


<details>
  <summary>Details</summary>
Motivation: 高超声速不启动现象是马赫5及以上吸气式推进系统可靠运行的主要挑战，强激波-边界层相互作用和快速压力波动会破坏进气道稳定性，需要开发有效的主动控制策略。

Method: 采用深度强化学习（DRL）框架，结合高保真CFD求解器（具有自适应网格细化功能）进行模拟，捕捉激波运动、边界层动力学和流动分离等关键流动特征，学习物理一致的实时控制策略。

Result: DRL控制器在代表不同燃烧室条件的宽范围背压下稳健稳定进气道，能泛化到未见过的场景（不同背压水平、雷诺数和传感器配置），在噪声测量下仍保持鲁棒性，最小化传感器配置也能获得可比性能。

Conclusion: 该研究建立了数据驱动方法用于现实操作不确定性下的实时高超声速流动控制，展示了DRL在复杂流动控制问题中的强大泛化能力和实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 现有迁移学习方法在处理源域和目标域之间的域偏移时存在困难。策略蒸馏能提供战术指导但无法传递长期战略知识，而自动机方法能捕捉任务结构但缺乏细粒度动作指导。需要一种能统一战略和战术知识的自适应知识迁移方法。
...
Conclusion: CADENT为强化学习中的自适应知识迁移建立了一种稳健方法，成功统一了战略和战术知识，通过动态信任机制实现了对域偏移的有效适应。
...
TL;DR: UniMod提出了一种从稀疏决策转向密集推理轨迹的多模态安全审核新范式，通过结构化轨迹和多维度边界学习，使用不到40%训练数据即可达到竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 多模态安全审核面临数据和监督信号的双重稀疏性问题，传统的二元标签依赖导致捷径学习，掩盖了有效的多模态判别所需的内在分类边界。

Method: 提出UniMod学习范式，构建包含证据定位、模态评估、风险映射、策略决策和响应生成的结构化轨迹；开发多头部标量奖励模型(UniRM)提供多维度监督；引入专门优化策略解耦任务特定参数并重新平衡训练动态。

Result: UniMod在文本审核上达到竞争性性能，并在多模态审核上建立了新基准，仅使用领先基线不到40%的训练数据；消融实验验证了多属性轨迹推理的有效性。

Conclusion: UniMod通过从稀疏决策转向密集推理轨迹，为多模态审核提供了一个有效且高效的框架，解决了捷径学习问题并改善了多任务学习中的目标干扰。
...
TL;DR: FAQ是一种新的后训练量化方法，通过利用未来层激活来指导量化，减少量化偏差和误差累积，在边缘部署中具有优势。


<details>
  <summary>Details</summary>
Motivation: 传统的后训练量化方法通常基于当前层激活设置量化超参数，这种方法虽然高效，但存在量化偏差和误差累积问题，特别是在校准数据存在偏差时，会导致次优和不稳定的量化结果。

Method: 提出Future-Aware Quantization（FAQ）方法，利用未来层激活来指导量化，更好地识别和保护重要权重，同时降低对校准噪声的敏感性。引入窗口式预览机制，软聚合多个未来层激活，避免过度依赖任何单层。使用预搜索配置来最小化开销，避免昂贵的贪婪搜索。

Result: 实验表明，FAQ在可忽略的额外成本下持续优于先前方法，不需要反向传播、数据重构或调优，非常适合边缘部署。

Conclusion: FAQ通过利用未来层激活信息，有效解决了传统后训练量化中的偏差和误差累积问题，提供了一种高效、稳定且适合边缘部署的量化解决方案。
...
TL;DR: 该研究探讨了视觉中心模型在长上下文建模中的信息容量极限，通过控制实验发现了视觉令牌信息压缩的三个相变阶段，并建立了统一的概率缩放定律。


<details>
  <summary>Details</summary>
Motivation: 尽管以DeepSeek-OCR为代表的视觉中心方法在长上下文建模中取得了显著进展，能够将渲染文本编码为连续视觉令牌并实现高压缩率，但视觉编码器作为具有有限表示能力的损失通道，其信息上限尚未明确。研究旨在探索视觉令牌的信息容量极限。

Method: 通过控制压力测试，逐步增加图像中的信息量（字符数量），观察视觉令牌编码的行为变化。分析相变现象的机械起源和关键因素，并制定统一的概率缩放定律，将平均视觉令牌负载和视觉密度整合为潜在难度度量指标。

Result: 实验发现了三个明显的相变阶段：1）近乎完美的稳定阶段；2）误差方差增大的不稳定阶段；3）完全崩溃阶段。建立的缩放定律在多种视觉语言模型中表现出普适性，为视觉上下文压缩的效率-精度权衡优化提供了关键经验指导。

Conclusion: 该研究揭示了视觉令牌编码的信息容量极限及其相变行为，提出的概率缩放定律为视觉中心长上下文建模系统的设计和优化提供了理论框架和实用指导，有助于在保持识别精度的同时最大化压缩效率。
...
TL;DR: 该论文提出使用Fubini-Study度量来量化高维表示漂移，通过投影几何视角分离内在数据变化与规范变换引起的伪变化。


<details>
  <summary>Details</summary>
Motivation: 传统欧几里得距离和余弦距离在量化表示漂移时，会将内在数据变化与规范变换（如全局缩放、符号翻转）引起的伪变化混在一起，导致对表示变化的系统性高估。

Method: 引入基于Fubini-Study度量的投影几何框架，该度量在规范变换下保持不变。通过构造表示轨迹并跟踪其累积几何漂移，比较欧几里得距离、余弦距离和Fubini-Study距离的差异。

Result: 传统度量在表示存在真实投影模糊性时系统性高估变化，而Fubini-Study度量通过保持规范变换不变性来隔离内在演化。余弦距离与Fubini-Study漂移之差定义了可计算的单调量，直接捕捉规范自由度引起的表示扰动。

Conclusion: 建立了评估高维系统表示稳定性的几何标准，阐明了角度距离的局限性。将表示动力学嵌入投影空间连接了数据分析与几何理论，产生了可直接在经验工作流中测试的可观测量。
...
<details>
  <summary>Details</summary>
Motivation: 现有的Locate-and-Edit (L&E)范式在连续编辑中会出现模型崩溃问题，研究发现这与编辑后MLP权重范数的爆炸性增长密切相关

Method: 提出Norm-Anchor Scaling (NAS)策略，这是一种即插即用的范数约束方法，通过控制MLP权重范数增长来防止模型崩溃

Result: NAS将代表性L&E算法的崩溃点延迟了4倍以上，编辑性能平均相对提升72.2%，仅需一行额外代码且计算开销可忽略

Conclusion: 通过约束MLP权重范数可以有效缓解L&E编辑范式中的模型崩溃问题，NAS为模型编辑提供了简单有效的解决方案
...
TL;DR: 该论文将成员推理攻击评估重新定义为因果推断问题，揭示了现有评估方法的统计偏差，并提出了具有理论保证的因果评估框架。


<details>
  <summary>Details</summary>
Motivation: 现有MIA评估方法存在统计有效性问题：多轮重训练计算成本高，单轮方法存在数据点间干扰，零轮方法（如LLM评估）存在非随机成员分配混杂。需要建立统计上可靠的评估框架。

Method: 将MIA评估构建为因果推断问题，定义记忆化作为数据点包含在训练集中的因果效应。推导标准MIA指标的因果对应版本，为多轮、单轮和零轮评估方案提出具有非渐近一致性保证的实用估计器。

Result: 在真实数据上的实验表明，该方法即使在重训练不可行和分布偏移的情况下，也能实现可靠的记忆化测量，为现代AI系统的隐私评估提供了理论基础。

Conclusion: 因果推断框架为MIA评估提供了统计上可靠的基础，解决了现有方法的偏差问题，使隐私风险评估在实际应用中更加可行和准确。
...
### [16] [Beyond Alignment: Expanding Reasoning Capacity via Manifold-Reshaping Policy Optimization](https://arxiv.org/abs/2602.02545)
*Dayu Wang,Jiaye Yang,Weikang Li,Jiahui Liang,Yang Li*

Main category: cs.LG

TL;DR: 提出MRPO方法，通过几何干预扩展LLM的推理空间，挑战现有RL方法仅对齐预训练模型潜在能力的观点


<details>
  <summary>Details</summary>
Motivation: 现有研究质疑强化学习是否真正扩展了LLM的推理能力，还是仅仅对齐了预训练模型中已有的潜在能力。作者挑战"可访问性边界假设"，认为通过有针对性的几何干预可以根本上扩展潜在推理空间。

Method: 提出Manifold-Reshaping Policy Optimization (MRPO)几何框架，包含两个阶段：1) Spectral Orthogonal Exploration (SOE)将策略初始化弹射到偏置流形的零空间；2) 在策略优化目标中集成有效秩正则化项，激励发现和维护高维推理轨迹。

Result: 4B参数的方法在数学任务上达到最先进性能，显著优于更大的模型（如Qwen3-32B），并将能力边界扩展到标准GRPO之外。

Conclusion: 通过几何干预可以根本上扩展LLM的潜在推理空间，挑战了现有关于强化学习仅对齐预训练模型潜在能力的观点，为提升LLM推理能力提供了新方向。
...
### [17] [A Random Matrix Theory Perspective on the Consistency of Diffusion Models](https://arxiv.org/abs/2602.02908)
*Binxu Wang,Jacob Zavatone-Veth,Cengiz Pehlevan*

Main category: cs.LG

TL;DR: 扩散模型在不同数据子集上训练时，使用相同噪声种子会产生相似输出，这源于共享高斯统计特性。研究者建立了随机矩阵理论框架，量化有限数据集如何影响去噪器和采样图的期望与方差，解释了跨训练数据分割时样本差异的来源。


<details>
  <summary>Details</summary>
Motivation: 研究动机是解释扩散模型在不同、非重叠数据子集上训练时，使用相同噪声种子却能产生惊人相似输出的现象。这种现象挑战了直觉，因为不同训练集应该产生不同模型。研究者希望理解这种跨训练分割一致性的根本原因，并为扩散训练的可重复性建立理论基础。

Method: 方法包括：1) 建立随机矩阵理论框架，在线性设置下量化有限数据集如何塑造学习到的去噪器和采样图的期望与方差；2) 分析采样变异性如何通过自洽关系重新归一化噪声水平；3) 扩展确定性等价工具到分数矩阵幂，以分析整个采样轨迹；4) 在UNet和DiT架构的非记忆化区域验证理论预测。

Result: 结果显示：1) 期望方面，采样变异性通过自洽关系 σ² ↦ κ(σ²) 重新归一化噪声水平，解释有限数据为何过度收缩低方差方向并将样本拉向数据集均值；2) 方差方面，公式揭示跨分割分歧的三个关键因素：特征模态的各向异性、输入间的非均匀性、以及随数据集大小的整体缩放；3) 理论准确预测线性扩散模型行为，并在UNet和DiT架构的非记忆化区域得到验证。

Conclusion: 结论是扩散模型跨训练分割的一致性源于共享高斯统计特性，随机矩阵理论框架成功量化了有限数据集对生成输出的影响。该研究为扩散训练的可重复性提供了原则性基线，将数据的光谱特性与生成输出的稳定性联系起来，解释了样本在训练数据分割间偏离的位置和方式。
...
### [18] [D$^2$Quant: Accurate Low-bit Post-Training Weight Quantization for LLMs](https://arxiv.org/abs/2602.02546)
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型的计算和内存成本高，在资源受限场景中部署困难。仅权重量化后训练量化虽然能减少内存使用并实现实际加速，但在亚4比特精度下准确率显著下降。分析发现两个主要原因：1）下投影矩阵是量化瓶颈，保持其保真度需要额外比特宽度；2）权重量化引起激活偏差，但有效的校正策略尚未充分探索。

Method: 提出D²Quant框架，从权重和激活两个角度改进量化。权重侧：设计针对下投影矩阵的双尺度量化器（DSQ），包含可吸收的缩放因子，在不增加比特预算的情况下显著提升准确率。激活侧：提出偏差感知校正（DAC），在LayerNorm中融入均值偏移校正，以减轻量化引起的激活分布偏移。

Result: 在多个LLM家族和评估指标上的广泛实验表明，D²Quant在亚4比特精度的仅权重量化后训练量化中提供了卓越的性能。

Conclusion: D²Quant通过创新的双尺度量化器和偏差感知校正机制，有效解决了仅权重量化在亚4比特精度下的准确率下降问题，为资源受限场景中的大语言模型部署提供了实用解决方案。
...
### [19] [Notes on the Reward Representation of Posterior Updates](https://arxiv.org/abs/2602.02912)
*Pedro A. Ortega*

Main category: cs.LG

TL;DR: 论文探讨了在何种条件下，决策作为推断这一概念可以从隐喻变为字面意义，研究KL正则化软更新何时能成为单一固定概率模型中的精确贝叶斯后验。


<details>
  <summary>Details</summary>
Motivation: 现代控制和强化学习中常将决策视为推断，但这一概念通常是隐喻性的。本文旨在探究何时这种"决策即推断"的类比可以变为字面意义上的精确对应，即更新过程能在一个固定概率模型中表示为真正的贝叶斯后验。

Method: 研究KL正则化软更新在单一固定概率模型中精确对应贝叶斯后验的特殊情况。分析更新变量作为信息传输通道的条件，要求行为变化仅由该通道携带的证据驱动，更新必须可解释为基线分布的证据重加权。

Result: 得到尖锐的识别结果：后验更新决定了相对、上下文相关的激励信号，这些信号驱动行为变化，但无法唯一确定绝对奖励值，后者在上下文特定基线范围内保持模糊。要求不同更新方向共享可重用延续值会进一步增加一致性约束，连接不同条件顺序相关的奖励描述。

Conclusion: 决策作为推断的类比在特定条件下可以精确实现为贝叶斯后验更新，这揭示了激励信号的相对性和上下文依赖性，同时表明绝对奖励值在缺乏额外约束时存在固有模糊性。
...
TL;DR: 该论文从理论角度分析机器遗忘，通过渐近线性稳定性框架研究优化动态与数据几何的相互作用，提出数据相干性作为关键分析量，建立了遗忘稳定性阈值，并在两层ReLU CNN信号加噪声模型中验证了遗忘难度与记忆强度的关系。


<details>
  <summary>Details</summary>
Motivation: 机器遗忘（无需从头重新训练即可消除特定训练样本影响的能力）对隐私、法规遵从性和效率至关重要。然而，现有遗忘研究大多停留在经验层面，缺乏对遗忘何时及为何有效的理论理解。本文旨在填补这一理论空白。

Method: 1. 通过渐近线性稳定性框架分析遗忘过程，捕捉优化动态与数据几何的相互作用；2. 提出数据相干性作为关键分析量，沿三个轴分解：保留集内、遗忘集内、保留集与遗忘集之间；3. 建立严格的稳定性阈值，区分收敛与发散；4. 在信号加噪声模型下研究两层ReLU CNN，分析信噪比与遗忘难度的关系；5. 使用随机矩阵理论工具进行理论分析，并通过Hessian测试和CNN热图进行实证验证。

Result: 1. 证明了数据相干性决定遗忘稳定性：相干性越弱，遗忘越容易；相干性越强，遗忘越困难；2. 建立了遗忘稳定性阈值，能够准确预测梯度基遗忘的收敛边界；3. 在信号加噪声模型中显示：信噪比越低，跨样本对齐越弱，相干性降低，遗忘越容易；反之，高信噪比、高度对齐的模型抵抗遗忘；4. 实证验证显示Hessian测试和CNN热图与预测边界高度一致，成功映射了基于批处理、混合和数据/模型对齐的稳定性前沿。

Conclusion: 本文首次从原理上解释了记忆、相干性与遗忘之间的权衡关系。通过渐近线性稳定性框架和数据相干性分析，为机器遗忘提供了理论基础，揭示了遗忘难度与模型记忆强度之间的反直觉关系：更强的记忆反而使遗忘更容易。该理论框架能够预测遗忘稳定性边界，为实际遗忘算法设计提供了指导原则。
...
### [21] [HyPAC: Cost-Efficient LLMs-Human Hybrid Annotation with PAC Error Guarantees](https://arxiv.org/abs/2602.02550)
*Hao Zeng,Huipeng Huang,Xinhao Qu,Jianguo Huang,Bingyi Jing,Hongxin Wei*

Main category: cs.LG

TL;DR: HyPAC是一种自适应路由方法，将输入分配到最经济的标注源（如LLM、推理模型、人类专家），同时提供分布无关的标注误差保证，能减少78.51%的标注成本。


<details>
  <summary>Details</summary>
Motivation: 数据标注通常涉及多个具有不同成本-质量权衡的标注源（快速LLM、慢速推理模型、人类专家），需要一种方法能够在控制测试实例标注误差的同时，将输入路由到最具成本效益的标注源。

Method: HyPAC使用重要性采样和置信上界校准两个决策阈值，基于不确定性将输入划分为三个区域，并将每个区域路由到适当的标注源。该方法提供分布无关的标注误差保证，并实现最小期望成本。
...
### [22] [Self-Hinting Language Models Enhance Reinforcement Learning](https://arxiv.org/abs/2602.03143)
...
<details>
  <summary>Details</summary>
Motivation: GRPO在稀疏终端奖励下经常停滞，因为组内rollout经常获得相同奖励，导致相对优势崩溃和更新消失。需要解决稀疏奖励下GRPO训练效率低下的问题。

Method: 提出SAGE框架：在训练期间注入特权提示来重塑rollout分布。对于每个提示x，模型采样紧凑提示h（如计划或分解），然后基于(x,h)生成解决方案τ。任务奖励R(x,τ)保持不变，提示仅增加有限采样下的组内结果多样性。测试时设置h=∅，部署无提示策略。
...
Conclusion: SAGE通过自我提示对齐和特权监督，有效解决了GRPO在稀疏奖励下的训练停滞问题，提高了组内结果多样性，防止优势函数崩溃，最终显著提升了模型性能。
...
### [23] [EEO-TFV: Escape-Explore Optimizer for Web-Scale Time-Series Forecasting and Vision Analysis](https://arxiv.org/abs/2602.02551)
*Hua Wang,Jinghao Lu,Fan Zhang*

Main category: cs.LG

TL;DR: 提出轻量级Transformer架构与新颖的Escape-Explore优化器，解决多变量长序列预测中的误差累积和图像任务中的分布外样本脆弱性问题，提升Web规模数据分析的泛化能力和稳定性。
...
Method: 提出轻量级Transformer架构结合新颖的Escape-Explore优化器(EEO)。该优化器增强探索和泛化能力，同时有效避免尖锐最小值和鞍点陷阱。
...
### [24] [Soft-Radial Projection for Constrained End-to-End Learning](https://arxiv.org/abs/2602.03461)
*Philipp J. Schneider,Daniel Kuhn*

Main category: cs.LG

TL;DR: 提出Soft-Radial Projection层，通过径向映射将欧几里得空间映射到可行集内部，解决传统正交投影层梯度饱和问题，保证严格可行性同时保持几乎处处满秩雅可比矩阵。


<details>
  <summary>Details</summary>
Motivation: 将硬约束集成到深度学习中对安全关键系统至关重要。现有将预测投影到约束边界上的构造层面临根本瓶颈：梯度饱和。标准正交投影将外部点折叠到低维曲面，导致秩不足的雅可比矩阵，使垂直于活动约束的梯度归零，阻碍优化。

Method: 引入Soft-Radial Projection，一种可微重参数化层，通过径向映射将欧几里得空间映射到可行集内部。这种构造保证严格可行性，同时几乎处处保持满秩雅可比矩阵，防止基于边界方法的典型优化停滞。

Result: 理论上证明该架构保留了通用逼近性质，实证显示相比最先进的优化和基于投影的基线方法，具有改进的收敛行为和解决方案质量。

Conclusion: Soft-Radial Projection解决了深度学习硬约束集成中的梯度饱和问题，通过径向映射保持梯度信息，在保证严格可行性的同时改善优化性能。
...
### [25] [BatCoder: Self-Supervised Bidirectional Code-Documentation Learning via Back-Translation](https://arxiv.org/abs/2602.02554)
...
<details>
  <summary>Details</summary>
Motivation: 训练代码相关任务的LLMs通常依赖高质量的代码-文档对，但这些数据成本高昂且对于小众编程语言稀缺。需要一种仅使用代码数据就能训练模型的方法。
...
Conclusion: BatCoder通过自监督强化学习有效解决了代码-文档对数据稀缺的问题，仅使用代码数据就能训练出性能优越的代码生成模型，具有很好的实用性和扩展性。
...
### [26] [A Function-Space Stability Boundary for Generalization in Interpolating Learning Systems](https://arxiv.org/abs/2602.03514)
*Ronald Katende*

Main category: cs.LG

TL;DR: 该论文研究了算法稳定性在解释学习系统泛化能力中的作用，提出了基于函数空间轨迹的稳定性证书框架，并证明了稳定性并非解释所有插值泛化现象的唯一机制。


<details>
  <summary>Details</summary>
Motivation: 现代学习系统经常插值训练数据但仍能良好泛化，但算法稳定性何时能解释这种行为尚不清楚。需要建立理论框架来区分稳定性解释泛化的机制和需要其他解释的机制。

Method: 将训练过程建模为函数空间轨迹，测量沿该轨迹对单样本扰动的敏感性。提出收缩传播条件和通过展开递归获得的稳定性证书。小证书意味着基于稳定性的泛化，同时证明存在插值机制下风险小但收缩敏感性不成立的情况。

Result: 实验证实证书增长能预测不同优化器、步长和数据集扰动下的泛化差异。框架识别了稳定性解释泛化的机制和需要替代机制解释成功的机制。

Conclusion: 稳定性并非解释所有插值泛化现象的通用机制。该框架能够区分稳定性解释泛化的机制和需要其他机制解释成功的机制，为理解学习系统泛化提供了更精细的理论工具。
...
### [27] [Beyond Experience Retrieval: Learning to Generate Utility-Optimized Structured Experience for Frozen LLMs](https://arxiv.org/abs/2602.02556)
*Xuancheng Li,Haitao Li,Yujia Zhou,Yiqun Liu,Qingyao Ai*

Main category: cs.LG

TL;DR: SEAM是一种轻量级插件模块，通过参数化存储经验并在单次前向传播中生成结构化经验条目，指导冻结的LLM执行器，实现经验复用而不依赖外部检索。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型通常是静态的，会重复推理或错误；现有经验复用方法依赖外部检索，存在相似性噪声和延迟问题，需要更高效、低噪声的经验复用机制。
...
Conclusion: SEAM提供了一种高效、低延迟的经验复用方法，通过参数化经验存储和结构化经验生成，显著提升LLM推理性能，同时保持执行器冻结，具有实际部署价值。
...
TL;DR: 论文揭示了LLM训练中幂律收敛的根源：softmax和交叉熵在拟合尖峰概率分布时会产生幂律衰减的损失和梯度，导致优化的基本瓶颈，产生1/3的普适指数。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型训练计算成本高昂，部分原因是损失函数表现出缓慢的幂律收敛，但其起源仍存在争议。本文旨在通过系统分析揭示这种行为的根本原因。

Method: 通过玩具模型的系统分析和LLM的实证评估，研究softmax和交叉熵在拟合尖峰概率分布（如下一个token分布）时的优化行为。

Result: 研究发现，softmax和交叉熵在拟合尖峰概率分布时会产生幂律衰减的损失和梯度，形成基本的优化瓶颈，最终导致损失函数以1/3的普适指数进行幂律时间缩放。

Conclusion: 该研究为观察到的神经缩放现象提供了机制性解释，并为改进LLM训练效率提供了新的方向。揭示了softmax和交叉熵在LLM优化中的基本限制。
...
TL;DR: 该论文研究了从文本到音频的跨模态越狱攻击转移，发现文本转移的音频越狱攻击性能与现有音频越狱攻击相当甚至更好，揭示了模态对齐可能传播文本漏洞到音频模态的"对齐诅咒"。


<details>
  <summary>Details</summary>
Motivation: 当前端到端训练的全能模型在多模态理解方面取得显著进展，安全红队测试已从文本扩展到音频越狱攻击。然而，文本和音频越狱攻击之间的跨模态转移研究不足。本文基于两种模态的语义相似性和文本越狱方法的成熟度，研究从文本到音频的越狱攻击转移。

Method: 首先分析模态对齐与跨模态越狱转移之间的关系，揭示强对齐可能无意中将文本漏洞传播到音频模态的"对齐诅咒"。在此基础上，对近期全能模型进行实证评估，比较文本越狱、文本转移的音频越狱和现有音频越狱攻击的性能。

Result: 文本转移的音频越狱攻击性能与现有音频越狱攻击相当甚至更好，成为未来音频红队测试的简单而强大的基线。研究还展示了强大的跨模型可转移性，并证明文本转移的音频攻击在更严格的纯音频访问威胁模型下仍然有效。

Conclusion: 文本到音频的跨模态越狱攻击转移是有效的，文本转移的音频越狱可作为强大的音频红队测试基线。模态对齐虽然有益于多模态理解，但也可能传播漏洞，揭示了"对齐诅咒"现象，对多模态模型安全设计具有重要启示。
...
TL;DR: 该论文提出了用于大规模语言模型预训练的"随时"学习调度方法，通过权重平均和简单的无时间范围依赖的学习率（如1/√t）替代传统的余弦衰减调度，在持续学习场景中实现与精心调优的余弦调度相当的性能。


<details>
  <summary>Details</summary>
Motivation: 当前大规模语言模型越来越多地在持续或开放式设置中训练，但大多数现有预训练方法依赖于时间范围相关的学习率调度（如余弦衰减），需要在固定计算预算下进行大量调优。这限制了模型在未知训练时间范围情况下的适应性。

Method: 1. 理论分析：证明了过参数化线性回归中存在随时学习调度；2. 提出权重平均（模型合并）是实现随机梯度下降极小极大收敛率的关键；3. 设计多项式衰减的随时调度方案，衰减率由问题的源条件和容量条件决定；4. 实证评估：在150M和300M参数的语言模型上，比较常数学习率+权重平均、1/√t调度+权重平均与精心调优的余弦调度。

Result: 1. 理论证明：权重平均是实现极小极大收敛率的关键机制；2. 实证结果：在整个训练范围内，随时调度（常数学习率+权重平均、1/√t+权重平均）达到与余弦衰减相当的最终损失；3. 在1-32倍Chinchilla规模下验证了方法的有效性。

Conclusion: 权重平均结合简单的无时间范围依赖的学习率调度（如1/√t）为大规模语言模型预训练提供了一种实用且有效的随时替代方案，避免了传统余弦调度对训练时间范围的依赖和大量调优需求。
...
<details>
  <summary>Details</summary>
Motivation: 尽管Sybil深度学习模型在肺癌风险预测方面表现出色并经过临床验证，但现有评估仅依赖观察性指标，这种基于相关性的方法忽略了模型的实际推理机制。在临床部署前需要进行因果验证以确保决策的鲁棒性。

Method: 提出S(H)NAP模型无关审计框架，利用真实的3D扩散桥建模系统性地修改解剖特征，构建生成干预性归因，并由放射科专家验证，从而隔离特定对象对风险评分的因果贡献。

Result: 首次对Sybil进行干预性审计，发现模型虽然常表现出类似专家放射科医生的行为（能区分恶性与良性肺结节），但存在关键失效模式：包括对临床不合理伪影的危险敏感性，以及明显的径向偏差。

Conclusion: 需要从相关性评估转向因果验证来确保AI模型在临床部署前的鲁棒性。S(H)NAP框架为深度学习模型的审计提供了有效的因果分析方法，揭示了Sybil模型的重要局限性，强调了临床AI系统验证中因果推理的重要性。
...
### [32] [A Comparative Simulation Study of the Fairness and Accuracy of Predictive Policing Systems in Baltimore City](https://arxiv.org/abs/2602.02566)
*Samin Semsar,Kiran Laxmikant Prabhu,Gabriella Waters,James Foulds*

Main category: cs.LG

TL;DR: 本文通过巴尔的摩的模拟研究比较了预测性警务与传统热点警务的公平性和准确性，发现预测性警务在短期更公平准确，但长期会更快放大偏见，且在某些情况下偏向过度监管白人社区。


<details>
  <summary>Details</summary>
Motivation: 现有关于预测性警务系统（如洛杉矶和巴尔的摩部署的系统）公平性的讨论主要关注种族偏见问题，但比较研究较少且不够全面。本研究旨在通过全面的模拟研究，比较预测性警务技术在公平性和准确性方面的表现。

Method: 在巴尔的摩进行全面的比较模拟研究，评估预测性警务技术的公平性和准确性。研究采用城市特定的评估方法，比较预测性警务与传统热点警务在反馈循环、历史偏见数据影响等方面的表现。

Result: 1. 预测性警务确实存在先前报道的反馈循环偏见，但传统热点警务也有类似问题；2. 短期来看，预测性警务比热点警务更公平和准确；3. 预测性警务会更快放大偏见，表明长期行为可能更差；4. 在巴尔的摩的某些情况下，这些系统的偏见倾向于过度监管白人社区，与先前研究不同。

Conclusion: 预测性警务中的偏见问题比先前假设的更复杂。研究展示了一种城市特定评估和行为趋势比较的方法论，表明此类模拟可以揭示不平等和长期趋势。预测性警务在短期表现更好，但长期可能放大偏见，需要更细致的政策考虑。
...
### [33] [Mitigating Task-Order Sensitivity and Forgetting via Hierarchical Second-Order Consolidation](https://arxiv.org/abs/2602.02568)
*Protik Nag,Krishnan Raghavan,Vignesh Narayanan*

Main category: cs.LG

TL;DR: HTCL框架通过层次化泰勒级数结合快速局部适应与保守二阶全局整合，解决持续学习中任务顺序随机性带来的高方差问题，显著提升性能并降低结果波动。


<details>
  <summary>Details</summary>
Motivation: 解决持续学习（Continual Learning）中随机任务顺序引入的高方差问题，传统单层次CL系统无法支持多尺度知识整合，需要一种能够同时处理局部适应和全局整合的框架。

Method: 提出层次化泰勒级数持续学习框架（HTCL）：1）识别最佳组内任务序列；2）通过Hessian正则化的泰勒展开整合局部更新；3）扩展到L层次结构实现多尺度知识整合；4）作为模型无关的整合层应用于各种基线方法。

Result: 在广泛的数据集和回放/正则化基线方法上，HTCL一致提升性能：平均准确率提升7%到25%，同时将最终准确率的标准差降低高达68%（跨随机任务排列）。

Conclusion: HTCL通过层次化泰勒级数框架有效解决了持续学习中的任务顺序方差问题，实现了快速局部适应与保守全局整合的平衡，为多尺度知识整合提供了理论保证的解决方案。
...
### [34] [Trajectory Consistency for One-Step Generation on Euler Mean Flows](https://arxiv.org/abs/2602.02571)
...
<details>
  <summary>Details</summary>
Motivation: 现有流基生成模型在长时程轨迹一致性约束上存在监督和优化困难，需要高采样成本，且训练框架存在计算和内存开销大的问题。

Method: 从流基模型的半群表述出发，推导出原理性的线性替代，用直接数据监督替代难优化的轨迹一致性约束，形成统一的JVP-free训练框架，支持u预测和x1预测变体。

Result: 在图像合成、基于粒子的几何生成和函数生成任务中，在固定采样预算下表现出更好的优化稳定性和样本质量，相比现有一步图像生成方法训练时间和内存消耗减少约50%。

Conclusion: EMF通过线性替代长程轨迹一致性约束，实现了高效的一步和少步生成，在保持样本质量的同时显著降低了计算和内存开销。
...
TL;DR: 论文提出了一种优化奖励模型设计的方法，以解决传统对齐方法中KL正则化导致的基策略偏差问题，通过Stackelberg博弈框架和奖励塑形方案提升用户效用。


<details>
  <summary>Details</summary>
Motivation: 现有对齐方法直接使用从用户偏好数据学习的奖励模型来优化LLM策略，并采用相对于基策略的KL正则化。这种做法在最大化用户效用方面是次优的，因为KL正则化可能导致LLM继承基策略中与用户偏好冲突的偏差。虽然放大偏好输出的奖励可以减轻这种偏差，但也会增加奖励黑客的风险。

Method: 将奖励模型优化问题形式化为Stackelberg博弈，并提出一个简单的奖励塑形方案来有效近似最优奖励模型。该方法在推理时对齐设置中进行实证评估，能够无缝集成到现有对齐方法中，开销最小。

Result: 该方法在实证评估中一致提高了平均奖励，在推理时对齐设置中，对所有基线的胜-平率超过66%（跨评估设置平均）。

Conclusion: 通过将奖励模型优化形式化为Stackelberg博弈并提出有效的奖励塑形方案，可以解决传统对齐方法中KL正则化导致的基策略偏差问题，显著提升对齐效果和用户效用。
...
### [36] [QuantLRM: Quantization of Large Reasoning Models via Fine-Tuning Signals](https://arxiv.org/abs/2602.02581)
*Nan Zhang,Eugene Kwek,Yusen Zhang,Muyu Pan,Suhang Wang,Prasenjit Mitra,Rui Zhang*

Main category: cs.LG

TL;DR: QuantLRM：一种基于微调信号的大推理模型权重量化方法，通过保护权重更新幅度最大和最小的两端，使用受限二次函数拟合权重更新来计算通道重要性，在多个推理基准上实现一致改进。


<details>
  <summary>Details</summary>
Motivation: 受经典幅度剪枝启发，研究微调过程中权重更新的幅度是否能为大推理模型的量化提供有价值信号。假设微调过程中最小和最大的权重更新比中间幅度更新更重要，即"保护两端"现象。

Method: 提出QuantLRM方法：1）验证"保护两端"假设；2）使用简单受限二次函数拟合权重更新来保护两端；3）通过二次函数平均值乘以通道零权重更新计数来计算通道重要性；4）支持非微调模型通过伪微调收集有效信号。

Result: 在四个推理基准（AIME-120、FOLIO、时间序列、GPQA-Diamond）上量化各种微调模型（监督、直接偏好优化、强化学习微调），QuantLRM为LRM量化提供一致改进，在强化学习微调模型上平均改进6.55%。
...
### [37] [Copula-Based Aggregation and Context-Aware Conformal Prediction for Reliable Renewable Energy Forecasting](https://arxiv.org/abs/2602.02583)
...
<details>
  <summary>Details</summary>
Motivation: 可再生能源渗透率快速增长需要可靠的聚合级概率预测支持电网运营，但实践中系统运营商缺乏车队级模型，只能依赖第三方提供的异构站点级预测，这些预测在聚合时面临复杂的跨站点依赖性和聚合引起的校准问题

Method: 提出校准概率聚合框架，结合copula依赖建模捕捉跨站点相关性，使用上下文感知的保形预测(CACP)校正聚合级校准偏差，实现依赖感知的聚合同时保证有效覆盖和锐利的预测区间

Result: 在MISO、ERCOT和SPP的大规模太阳能发电数据集上的实验表明，Copula+CACP方法始终实现接近标称的覆盖率，且预测区间比未校准的聚合基线显著更锐利

Conclusion: 该框架为无法训练或维护系统级模型的情况提供了一种有效的解决方案，能够从异构的站点级预测生成可靠的车队级概率预测，解决了可再生能源聚合预测中的关键挑战
...
### [38] [Effective Frontiers: A Unification of Neural Scaling Laws](https://arxiv.org/abs/2602.02593)
*Jiaxuan Zou,Zixuan Gong,Ye Su,Huayi Tang,Yong Liu*

Main category: cs.LG

TL;DR: 提出统一框架，将学习任务抽象为从长尾分布中逐步覆盖模式的过程，引入有效边界概念，推导出N、D、C的精确缩放定律，并通过最大瓶颈原则统一这些机制。


<details>
  <summary>Details</summary>
Motivation: 现有神经缩放定律理论解释通常依赖特定架构或复杂核方法，缺乏直观的普适性。需要建立统一框架来解释模型容量(N)、数据量(D)和计算量(C)如何共同影响学习性能。

Method: 提出统一框架，将一般学习任务抽象为从长尾(Zipfian)分布中逐步覆盖模式的过程。引入有效边界(k⋆)概念，作为模式排序空间中区分已学知识和未学尾部的阈值。证明可约损失渐近地由尾部概率质量决定。基于该框架推导N、D、C的精确缩放定律。

Result: 推导出模型容量、数据覆盖和优化瓶颈分别对应的精确缩放定律。通过最大瓶颈原则统一这些机制，证明Kaplan和Chinchilla缩放定律不是矛盾的，而是在不同活跃瓶颈下同一约束优化问题的平衡解。

Conclusion: 提出的统一框架为神经缩放定律提供了直观的普适性解释，将学习过程抽象为模式覆盖问题，有效边界概念连接了资源限制与性能表现，最大瓶颈原则统一了不同缩放定律的机制。
...
### [39] [ContextEvolve: Multi-Agent Context Compression for Systems Code Optimization](https://arxiv.org/abs/2602.02597)
*Hongyuan Su,Yu Zheng,Yong Li*

Main category: cs.LG

TL;DR: ContextEvolve：一种多智能体框架，通过将优化上下文分解为三个正交维度（总结、导航、采样），在参数不可见的约束下实现强化学习级别的搜索效率，用于LLM驱动的系统算法优化。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在系统研究中自动化发现性能关键算法，但生成的代码需要迭代优化以满足正确性和性能要求。测试时强化学习搜索效率高但需要参数更新，而现有的无训练进化方法存在上下文利用效率低和搜索无方向的问题。

Method: 提出ContextEvolve多智能体框架，将优化上下文分解为三个正交维度：总结智能体通过代码到语言抽象浓缩语义状态；导航智能体通过轨迹分析提炼优化方向；采样智能体通过优先示例检索策划经验分布。这种编排形成了与强化学习（状态表示、策略梯度、经验回放）的功能同构。

Result: 在ADRS基准测试中，ContextEvolve比最先进的基线方法性能提升33.3%，同时减少29.0%的token消耗。

Conclusion: ContextEvolve通过多智能体框架实现了在参数不可见约束下的高效优化，为LLM驱动的系统算法发现提供了有效的解决方案，在性能和效率方面均优于现有方法。
...
### [40] [RAP: KV-Cache Compression via RoPE-Aligned Pruning](https://arxiv.org/abs/2602.02599)
*Jihao Xin,Tian Lvu,Hatem Ltaief,David Keyes,Marco Canini*

Main category: cs.LG

TL;DR: RoPE-Aligned Pruning (RAP) 通过剪枝对齐RoPE的列对，解决了低秩分解在RoPE-based LLMs中KV-Cache压缩失效的问题，实现了KV-Cache、注意力参数和FLOPs的联合减少。


<details>
  <summary>Details</summary>
Motivation: 长上下文推理在大型语言模型中越来越受KV-Cache的内存和计算成本限制。低秩分解通过将权重矩阵近似为两个低秩矩阵的乘积来压缩KV投影，但在基于RoPE的现代LLMs中，由于RoPE强制将潜在KV状态重构到完整维度，导致吸收失败并重新引入大量内存和计算开销。

Method: 提出RoPE-Aligned Pruning (RAP)方法，通过剪枝整个RoPE对齐的列对来保留RoPE的2x2旋转结构，恢复低秩矩阵B的吸收能力，并消除重构开销。该方法能够同时减少KV-Cache、注意力参数和计算量。

Result: 在LLaMA-3-8B和Mistral-7B上的评估显示，RAP能够同时减少KV-Cache、注意力参数和FLOPs达20-30%，同时保持较强的准确性。特别地，RAP将注意力延迟降低到基线的83%（预填充）和77%（解码）。

Conclusion: RAP方法有效解决了RoPE-based LLMs中低秩分解的局限性，通过结构化的剪枝策略实现了KV-Cache压缩、参数减少和计算效率提升的三重优化，为长上下文推理提供了实用的解决方案。
...
### [41] [Step-Wise Refusal Dynamics in Autoregressive and Diffusion Language Models](https://arxiv.org/abs/2602.02600)
*Eliron Rahimi,Elad Hirshel,Rom Himelstein,Amit LeVi,Avi Mendelson,Chaim Baskin*

Main category: cs.LG

TL;DR: 本文提出了一种分析扩散语言模型拒绝行为的框架，揭示了采样策略对安全性的关键作用，并引入了SRI信号来改进安全性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 尽管扩散语言模型在生成质量上表现出色，但其采样机制如何影响拒绝行为和越狱鲁棒性仍不清楚。需要理解采样策略在安全行为中的作用，并开发有效的安全检测方法。

Method: 提出了逐步拒绝动态分析框架，比较自回归和扩散采样。引入了SRI信号来捕获内部恢复动态，并基于此开发轻量级推理时检测器。

Result: SRI的几何结构能识别有害生成中的异常行为（不完全内部恢复），检测器能泛化到未见攻击，推理开销降低100倍以上，性能匹配或优于现有防御方法。

Conclusion: 采样策略本身是影响安全行为的关键因素，SRI信号为语言模型提供了可解释的安全分析工具，轻量级检测器在保持高性能的同时大幅降低了计算开销。
...
### [42] [Discovering Data Manifold Geometry via Non-Contracting Flows](https://arxiv.org/abs/2602.02611)
...
<details>
  <summary>Details</summary>
Motivation: 传统等距目标隐含假设流形平坦性，无法处理复杂流形结构。需要一种能够构建全局参考系统的方法，学习流形上的切向量场，将样本映射到共同参考点，从而定义内在坐标。

Method: 学习环境空间中的向量场，其流将样本传输到共同可学习参考点。沿这些流的弧长定义内在坐标。为防止退化，施加非收缩约束，并推导受流匹配启发的可扩展、无需积分的优化目标。

Result: 理论证明最小化该目标可在全局坐标图存在时恢复该图。实验显示在合成流形上获得正确的切向对齐和连贯的全局坐标结构。在CIFAR-10上展示可扩展性，学习坐标在下游分类任务中达到竞争性能。

Conclusion: 提出了一种无监督构建全局参考系统的方法，通过学习向量场将样本映射到共同参考点，定义内在坐标。该方法在理论和实验上均验证了有效性，并在实际数据集上展示了可扩展性和实用价值。
...
### [43] [A Semi-Supervised Pipeline for Generalized Behavior Discovery from Animal-Borne Motion Time Series](https://arxiv.org/abs/2602.02618)
*Fatemeh Karimi Nejadasl,Judy Shamoun-Baranes,Eldar Rakhimberdiev*

Main category: cs.LG

TL;DR: 提出一种半监督行为发现方法，用于从海鸥传感器数据中发现新行为类别，使用KDE+HDR包含度评分来量化新发现类簇的新颖性。


<details>
  <summary>Details</summary>
Motivation: 动物传感器数据的行为分类面临标签稀缺、类别高度不平衡以及标注集中可能缺少某些行为的挑战。需要一种能够从有限标注中发现新行为类别的方法。

Method: 提出三步半监督发现流程：1) 从标注子集学习嵌入函数；2) 对标注和未标注样本的嵌入进行标签引导聚类，形成候选行为组；3) 使用包含度评分决定发现组是否真正新颖。核心贡献是KDE+HDR包含度评分，衡量发现类簇分布与已知类别分布的包含关系。

Result: 在实验中，当某个行为完全从监督中移除并仅出现在未标注池中时，方法能够恢复出明显类簇，且包含度评分通过低重叠度标记其新颖性。在无新行为的负控制设置中，重叠度始终较高。

Conclusion: HDR基包含度评分提供了一种实用的量化测试方法，可用于生态运动时间序列在有限标注和严重类别不平衡条件下的广义类别发现。
...
### [44] [daVinci-Agency: Unlocking Long-Horizon Agency Data-Efficiently](https://arxiv.org/abs/2602.02619)
...
Method: daVinci-Agency通过重新构想数据合成视角，从真实世界软件演化中获取监督信号。核心洞察：Pull Request序列天然体现了长时程学习所需的监督信号。该方法系统地从PR链中挖掘结构化监督，通过三个相互关联的机制：(1) 通过连续提交实现渐进式任务分解，(2) 通过统一功能目标强制执行长期一致性，(3) 从真实bug修复轨迹中获取可验证的细化。与将每个步骤独立处理的合成轨迹不同，daVinci-Agency基于PR的结构固有地保留了因果依赖和迭代细化。

Result: 生成的轨迹规模可观（平均85k tokens和116个工具调用），但数据效率显著：在239个daVinci-Agency样本上微调GLM-4.6，在多个基准测试中取得广泛改进，特别是在Toolathlon上实现了47%的相对增益。分析证实了该方法在基准性能之外的优越性。

Conclusion: 通过利用真实软件演化中的Pull Request序列，daVinci-Agency为长时程智能体工作流提供了可扩展、高质量的训练数据。该方法不仅解决了现有合成方法的局限性，还通过保留因果依赖和迭代细化模式，实现了与项目级、全周期任务建模的自然对齐。
...
### [45] [A Reduction from Delayed to Immediate Feedback for Online Convex Optimization with Improved Guarantees](https://arxiv.org/abs/2602.02634)
*Alexander Ryabchenko,Idan Attias,Daniel M. Roy*

Main category: cs.LG

TL;DR: 提出基于约简的在线学习延迟反馈框架，改进了一阶和bandit凸优化的现有结果，通过连续时间模型将遗憾分解为延迟无关学习项和延迟诱导漂移项，实现了延迟自适应的约简方法。


<details>
  <summary>Details</summary>
Motivation: 现有在线学习延迟反馈方法存在局限性，特别是bandit凸优化的延迟相关遗憾项不够理想。需要开发统一框架来改进延迟反馈下的遗憾界，同时简化分析过程。

Method: 提出连续时间模型，将遗憾分解为延迟无关学习项和延迟诱导漂移项。开发延迟自适应约简框架，可将任何在线线性优化算法转换为处理轮次相关延迟的算法。使用连续时间分析和延迟分解技术。

Result: 对于bandit凸优化，获得O(√d_tot + T^{3/4}√k)遗憾界，将延迟相关项从O(min{√T d_max,(Td_tot)^{1/3}})改进为O(√d_tot)。在强凸性下，获得O(min{σ_max ln T, √d_tot} + (T^2 ln T)^{1/3} k^{2/3})，将延迟相关项从O(d_max ln T)改进为O(min{σ_max ln T, √d_tot})。

Conclusion: 提出的约简框架统一且简化了延迟反馈在线学习的分析，显著改进了bandit凸优化的遗憾界，延迟相关项达到与一阶反馈相当的最优状态，且框架具有延迟自适应特性。
...
### [46] [hSNMF: Hybrid Spatially Regularized NMF for Image-Derived Spatial Transcriptomics](https://arxiv.org/abs/2602.02638)
*Md Ishtyaq Mahmud,Veena Kochat,Suresh Satpati,Jagan Mohan Reddy Dwarampudi,Humaira Anzum,Kunal Rai,Tania Banerjee*

Main category: cs.LG

TL;DR: 该研究针对Xenium等高分辨率空间转录组学数据的高维特性，提出了两种空间正则化非负矩阵分解方法：SNMF和hSNMF，以改善细胞聚类和空间模式识别。


<details>
  <summary>Details</summary>
Motivation: 高分辨率空间转录组学平台（如Xenium）生成包含分子和空间信息的单细胞图像，但其极高的维度给表示学习和聚类带来了重大挑战。现有方法在处理这种高维空间数据时存在局限性，需要开发能够有效整合空间信息的降维和聚类方法。

Method: 研究提出两种空间正则化非负矩阵分解（NMF）变体：1）空间NMF（SNMF）：通过将每个细胞的NMF因子向量在其空间邻域内扩散来强制局部空间平滑性；2）混合空间NMF（hSNMF）：在空间正则化NMF后，对混合邻接矩阵进行Leiden聚类，该矩阵通过可调参数α整合空间邻近性（通过接触半径图）和转录组相似性。

Result: 在胆管癌数据集上的评估显示，SNMF和hSNMF相比其他空间基线方法显著改善了空间紧凑性（CHAOS < 0.004，Moran's I > 0.96）、聚类可分性（Silhouette > 0.12，DBI < 1.8）和生物学一致性（CMC和富集分析）。

Conclusion: 该研究提出的空间正则化NMF方法能够有效处理高分辨率空间转录组学数据，在保持空间结构的同时改善细胞聚类性能，为空间转录组学分析提供了新的计算工具。
...
### [47] [MARA: Continuous SE(3)-Equivariant Attention for Molecular Force Fields](https://arxiv.org/abs/2602.02671)
*Francesco Leonardi,Boris Bonev,Kaspar Riesen*

Main category: cs.LG

TL;DR: MARA模块将球面注意力扩展到分子领域，通过灵活加权局部几何相互作用提升机器学习力场的表达能力和稳定性
...
Method: 提出模块化角向-径向注意力(MARA)，将球面注意力扩展到SE(3)等变分子建模，直接操作相邻原子的角向和径向坐标，实现灵活、几何感知的局部环境加权

Result: MARA在分子基准测试中改善了能量和力预测，减少了高误差事件，增强了模型鲁棒性，且能以即插即用方式集成到现有架构中

Conclusion: 连续球面注意力是有效且可泛化的几何算子，能提高原子模型的表达能力、稳定性和可靠性
...
### [48] [Expert-Data Alignment Governs Generation Quality in Decentralized Diffusion Models](https://arxiv.org/abs/2602.02685)
*Marcos Villagra,Bidhan Roy,Raihan Seraj,Zhiying Jiang*

Main category: cs.LG

TL;DR: 研究发现去中心化扩散模型（DDMs）中，生成质量主要取决于专家-数据对齐而非数值稳定性，挑战了传统认为最小化轨迹敏感性决定质量的假设。


<details>
  <summary>Details</summary>
Motivation: 研究去中心化扩散模型（DDMs）中生成质量的决定因素。DDMs通过在不同数据簇上独立训练的专家进行去噪，但这些专家的预测可能存在强烈分歧。传统观点认为最小化去噪轨迹敏感性（即采样过程中扰动的放大程度）应该决定生成质量，但这一假设需要验证。

Method: 通过系统实验验证专家-数据对齐假设：1）使用数据簇距离分析，验证稀疏路由选择与当前去噪状态最接近的数据簇对应的专家；2）进行专家分析，比较被选专家与未被选专家的预测准确性；3）分析专家分歧对生成质量的影响。在两个不同的DDM系统上进行验证。

Result: 发现稳定性与质量存在分离：全集成路由（所有专家预测组合）具有最稳定的采样动态和最佳数值收敛性，但生成质量最差（FID 47.9 vs. 稀疏Top-2路由的22.6）。专家-数据对齐是决定因素：生成质量取决于将输入路由到其训练分布覆盖当前去噪状态的专家。

Conclusion: 对于DDM部署，路由策略应优先考虑专家-数据对齐而非数值稳定性指标。这一发现挑战了传统关于去噪轨迹敏感性决定生成质量的假设，为DDM系统的优化提供了新的指导原则。
...
### [49] [Sparsely Supervised Diffusion](https://arxiv.org/abs/2602.02699)
...
<details>
  <summary>Details</summary>
Motivation: 扩散模型在广泛生成任务中表现出色，但常存在空间不一致生成问题，这源于其去噪机制的局部性，导致样本局部合理但全局不一致

Method: 提出稀疏监督学习方法，采用简单有效的掩码策略，只需几行代码即可实现，可在扩散模型训练期间安全地掩码高达98%的像素

Result: 方法在实验中提供有竞争力的FID分数，最重要的是在小数据集上避免了训练不稳定性，掩码策略减少了记忆化并促进生成过程中使用必要的上下文信息

Conclusion: 稀疏监督学习是一种简单有效的扩散模型改进方法，通过掩码策略解决空间不一致问题，提高生成质量并增强模型鲁棒性
...
### [50] [Every Bit Counts: A Theoretical Study of Precision-Expressivity Tradeoffs in Quantized Transformers](https://arxiv.org/abs/2602.02707)
*Sayak Chakrabarti,Toniann Pitassi,Josh Alman*

Main category: cs.LG

TL;DR: 论文证明了Transformer量化精度与表达能力之间存在精细的理论权衡：对于每个p位精度，存在一个函数Γ（受等式函数启发），单层softmax Transformer可以用p位精度计算Γ，但不能用p-1位精度计算。
...
Method: 结合显式的有限精度Transformer构造和通信复杂度下界证明，建立紧致的"一位"阈值理论。通过构造受等式函数启发的函数Γ，证明单层softmax Transformer在不同精度下的计算能力差异。

Result: 证明了量化精度与表达能力之间的精细权衡：对于每个精度p，存在函数Γ，单层softmax Transformer可以用p位精度计算，但不能用p-1位精度计算。这为量化导致的表达能力损失提供了理论解释。

Conclusion: 量化精度降低会损失Transformer的特定表达能力，特别是需要等式比较的任务（精确匹配、成员检查等）对量化敏感。研究结果为实践中量化精度选择提供了理论依据：应根据具体任务所需的等式检查长度来选择精度。
...
### [51] [BinaryPPO: Efficient Policy Optimization for Binary Classification](https://arxiv.org/abs/2602.02708)
...
Method: 提出BinaryPPO框架，将二元分类重构为奖励最大化问题。采用基于置信度加权的奖励函数变体Proximal Policy Optimization，惩罚不确定或错误的预测，使模型能够从静态数据集中学习鲁棒的决策策略，无需在线交互。

Result: 在八个领域特定基准测试和多种不同架构的模型上，BinaryPPO将准确率提高了40-60个百分点，最高达到99%，显著优于监督基线。深入分析了奖励塑造、优势缩放和策略稳定性在实现这一改进中的作用。

Conclusion: 基于置信度的奖励设计为二元分类提供了比监督微调更鲁棒的替代方案。BinaryPPO框架在现实世界具有挑战性的分类场景中表现出色。
...
### [52] [Maximum Likelihood Reinforcement Learning](https://arxiv.org/abs/2602.02710)
...
TL;DR: MaxRL：一种基于采样的强化学习框架，通过计算索引目标函数逼近最大似然优化，在正确性导向任务中实现比传统RL更好的性能和缩放效率


<details>
  <summary>Details</summary>
Motivation: 传统强化学习在基于采样的二元结果反馈设置（如导航、代码生成、数学问题求解）中，仅优化了正确rollout似然的下阶近似，而非最大化真实似然。需要一种能更好利用采样计算资源的方法。

Method: 提出最大似然强化学习（MaxRL），定义计算索引的基于采样的目标函数族，在标准RL和精确最大似然之间插值。随着计算资源增加，目标函数收敛到最大似然优化。使用简单、无偏的策略梯度估计器。

Result: 在所有测试的模型和任务中，MaxRL帕累托优于现有方法，相比GRPO训练的对应模型，测试时缩放效率提升高达20倍。MaxRL在额外数据和计算资源下表现出更好的缩放性。

Conclusion: MaxRL是扩展正确性导向设置中RL训练的有前景框架，通过更好地利用采样计算资源，实现了比传统强化学习方法更优的性能和效率。
...
### [53] [Towards Understanding Steering Strength](https://arxiv.org/abs/2602.02712)
*Magamed Taimeskhanov,Samuel Vaiter,Damien Garreau*

Main category: cs.LG

TL;DR: 该论文首次对大型语言模型表示引导的强度进行理论分析，揭示了引导强度对模型输出的精确影响规律，包括非单调效应


<details>
  <summary>Details</summary>
Motivation: 当前后训练控制LLM的方法主要关注表示引导方向的选择，但对引导强度的理解不足。强度选择至关重要：太小则期望行为不出现，太大会严重损害模型性能。需要理论分析来指导强度选择

Method: 提出首个关于引导强度的理论分析框架，推导引导强度对下一个token概率、概念存在性和交叉熵的精确影响规律，建立定性法则。在11个语言模型上进行实证验证，从小型GPT架构到现代模型

Result: 理论分析揭示了引导强度的非单调效应等令人惊讶的行为。实证验证表明理论预测在11个不同规模的语言模型上均成立，从GPT小模型到现代大模型

Conclusion: 该研究为LLM表示引导提供了首个理论分析框架，揭示了引导强度的精确影响规律，为后训练控制中的强度选择提供了理论指导，有助于更精确地控制模型行为
...
### [54] [RLAnything: Forge Environment, Policy, and Reward Model in Completely Dynamic RL System](https://arxiv.org/abs/2602.02488)
...
<details>
  <summary>Details</summary>
Motivation: 当前强化学习系统在LLM和智能体场景中面临学习信号弱、系统性能有限的问题，需要一种能够动态优化环境、策略和奖励模型的综合框架来增强整体学习效果。

Method: 提出闭环优化框架：1) 策略通过整合步进信号和结果信号的反馈进行训练；2) 奖励模型通过一致性反馈联合优化；3) 理论驱动的自动环境适应利用批评反馈改进奖励和策略模型的训练。
...
Conclusion: RLAnything通过动态优化环境、策略和奖励模型的闭环框架，显著增强了强化学习系统在LLM和智能体任务中的性能，证明了综合优化方法的有效性。
...
TL;DR: 提出神经概率幅度整形(NPAS)框架，用于相干光纤系统，在单跨距205km链路上，相比序列选择方法，为双偏振64-QAM传输提供0.5dB信噪比增益


<details>
  <summary>Details</summary>
Motivation: 传统序列选择方法在光纤传输系统中存在性能限制，需要开发更有效的联合分布学习框架来提升传输性能

Method: 提出神经概率幅度整形(NPAS)框架，这是一种用于相干光纤系统的联合分布学习方法

Result: 在单跨距205km链路上，相比序列选择方法，为双偏振64-QAM传输提供0.5dB信噪比增益
...
### [56] [Hierarchical Entity-centric Reinforcement Learning with Factored Subgoal Diffusion](https://arxiv.org/abs/2602.02722)
...
Result: 在图像基长时程稀疏奖励任务上持续提升底层强化学习智能体性能，在最难任务上实现超过150%的成功率提升，并能泛化到更长的时程和更多实体
...
### [57] [Automated Dysphagia Screening Using Noninvasive Neck Acoustic Sensing](https://arxiv.org/abs/2602.02725)
*Jade Chng,Rong Xing,Yunfei Luo,Kristen Linnemeyer-Risser,Tauhidur Rahman,Andrew Yousef,Philip A Weissbrod*

Main category: cs.LG

TL;DR: 提出基于便携式非侵入声学传感与机器学习的吞咽障碍自动检测框架，通过捕捉吞咽时颈部细微声学信号识别异常模式


<details>
  <summary>Details</summary>
Motivation: 咽部健康对呼吸、吞咽和发声等基本功能至关重要。吞咽障碍的早期检测对及时干预很关键，但现有诊断方法多依赖放射成像或侵入性操作，需要更便捷的非侵入检测方案

Method: 采用便携式非侵入声学传感结合机器学习的方法，通过捕捉吞咽任务中颈部的细微声学信号，识别与异常生理状况相关的模式

Result: 在5次独立训练-测试分割下，测试时异常检测性能表现优异，AUC-ROC达到0.904

Conclusion: 非侵入声学传感作为咽部健康监测的实用且可扩展工具具有可行性，为吞咽障碍检测提供了新的技术途径
...
<details>
  <summary>Details</summary>
Motivation: 深度学习模型在隐藏表示中编码了丰富的语义信息，但难以理解模型在预测时实际依赖哪些信息。现有的基于聚类的概念解释方法存在可扩展性问题：层次聚类计算不可行，K-Means产生浅层或频率主导的簇。

Method: 提出VQLC方法，基于VQ-VAE架构构建框架，学习离散码本将连续表示映射到概念向量。该方法通过向量量化变分自编码器实现高效的离散概念学习。

Result: VQLC在保持人类可理解解释质量的同时显著提高了可扩展性。通过全面评估验证了该方法在大规模数据集上的有效性。

Conclusion: VQLC为深度学习模型提供了一种可扩展的概念解释框架，解决了传统聚类方法在大规模应用中的计算瓶颈，同时保持了概念解释的质量。
...
<details>
  <summary>Details</summary>
Motivation: 标准离散扩散模型主要针对基于似然的训练目标，匹配数据分布但缺乏强制执行硬约束或优化不可微分属性的机制，限制了其在需要满足特定约束的场景中的应用

Method: 提出Search-Augmented Masked Diffusion (SearchDiff)，在反向去噪过程的每一步，模型预测定义了一个候选集，然后在用户指定的属性满足条件下进行优化，产生修改后的反向转移，引导采样朝向既可能又可行的解决方案

Result: 在生物设计和符号推理任务上的实验表明，SearchDiff显著改善了约束满足和属性遵循，同时持续优于离散扩散和自回归基线方法

Conclusion: SearchDiff通过将知情搜索集成到离散扩散的反向过程中，提供了一种有效的训练免费框架，能够在保持生成质量的同时显著提高约束满足能力，为结构化符号生成提供了更强大的工具
...
TL;DR: CAPS是一种用于时间序列预测的结构化注意力机制，通过解耦全局趋势、局部冲击和季节模式三种时间结构，结合SO(2)旋转进行相位对齐，使用三种加性门控路径和Clock机制，实现线性复杂度的竞争性性能。


<details>
  <summary>Details</summary>
Motivation: 标准softmax注意力通过全局归一化将三种时间结构（全局趋势、局部冲击、季节模式）纠缠在一起，而最近的循环模型为了顺序依赖的因果结构牺牲了长期、顺序无关的选择能力。需要一种能够有效解耦这些时间结构的注意力机制。

Method: CAPS结合SO(2)旋转进行相位对齐，采用三种加性门控路径：Riemann softmax、前缀乘积门控和Clock基线。引入Clock机制作为学习的时间权重，通过共享的时间重要性概念来调制这些路径。整个注意力层保持线性复杂度。

Result: 在长短期预测基准测试中，CAPS超越了标准softmax和线性注意力机制，并在与七个强基线的比较中展现出竞争性性能，同时保持线性复杂度。

Conclusion: CAPS通过结构化解耦时间序列中的不同时间模式，提供了一种有效的注意力机制，在保持线性复杂度的同时实现了竞争性的预测性能，为时间序列分析提供了新的结构化注意力框架。
...
Method: 基于TabPFN的零样本生成框架，通过有限参考样本实现条件设计生成，无需任务特定模型训练或微调。该方法按顺序生成条件于目标性能指标的设计参数，为传统生成模型提供灵活替代方案。

Result: 在三个工程设计数据集（船体设计、BlendedNet飞机、UIUC翼型）上验证，方法在高度结构化的参数设计空间中实现竞争性多样性，对采样、分辨率和几何生成参数维度变化保持鲁棒性，性能误差低（如生成船体设计性能误差小于2%）。相比基于扩散的生成模型，显著降低计算开销和数据需求，同时保持可靠的生成性能。

Conclusion: 零样本、数据高效的生成方法具有作为工程设计实用高效工具的潜力，能够实现快速部署、灵活适应新设计设置，并易于集成到真实工程工作流程中。
...
TL;DR: EDT-Former是一种熵引导的动态令牌Transformer，通过生成与信息丰富的分子片段对齐的令牌，实现冻结图编码器与LLMs的对齐，无需调整LLM主干，在多个分子理解基准上达到SOTA性能。
...
Method: 提出EDT-Former（熵引导的动态令牌Transformer），生成与信息丰富的分子片段对齐的令牌，从而保留局部和全局结构特征。该方法实现了冻结图编码器与LLMs的对齐，无需调整LLM主干（不包括嵌入层），实现计算高效的微调。

Result: 在MoleculeQA、面向分子的Mol-Instructions和属性预测基准（TDC、MoleculeNet）上取得了最先进的结果，证明了其在可扩展和可泛化的多模态分子理解方面的有效性。

Conclusion: EDT-Former通过熵引导的动态令牌生成方法，有效解决了现有图-LLM桥接方法的局限性，实现了高效、泛化的分子图理解，为分子科学发现提供了有力的多模态理解工具。
...
TL;DR: 本文分析了半监督模仿学习中的逆动力学模型方法，揭示了VM-IDM和IDM标注在极限情况下学习相同策略，并指出IDM策略优于行为克隆的原因是IDM学习具有更好的样本效率，这源于IDM假设空间复杂度更低且确定性更强。
...
Method: 首先理论分析VM-IDM和IDM标注在极限情况下的等价性，然后通过统计学习理论分析和实验验证IDM学习样本效率优势的两个原因：IDM假设空间复杂度更低和确定性更强，最后基于这些见解改进LAPO算法。

Result: 证明了VM-IDM和IDM标注在极限情况下学习相同的IDM-based策略；通过理论和实验验证了IDM学习样本效率优势的两个原因；提出了改进的LAPO算法用于潜在动作策略学习。

Conclusion: 基于IDM的半监督模仿学习方法优于行为克隆的主要原因是IDM学习具有更好的样本效率，这源于IDM的假设空间复杂度更低且确定性更强，这些见解可用于改进现有算法如LAPO。
...
TL;DR: PATH：基于差分隐私的纵向表格数据合成框架，使用私有微调的大型语言模型处理用户级时序数据，相比传统边际机制显著提升时序连贯性


<details>
  <summary>Details</summary>
Motivation: 现有差分隐私合成表格数据研究主要关注独立同分布行数据，忽略了纵向数据集（如电子健康记录）的时序复杂性，其中每个用户贡献整个时序事件子表。传统方法将用户历史扁平化为高维向量使用标准边际机制，但无法保持时序连贯性。

Method: 引入PATH框架，将完整表格作为合成单元，利用私有微调的大型语言模型的自回归能力。该方法将整个用户时序表作为处理对象，而非扁平化处理，从而更好地捕捉长期依赖关系。

Result: PATH能有效捕捉传统方法遗漏的长期依赖关系。实证表明，相比领先的边际机制，该方法将真实轨迹的分布距离减少60%以上，状态转移错误减少近50%，同时保持相似的边际保真度。

Conclusion: PATH框架为纵向差分隐私数据合成提供了有效解决方案，通过将完整表格作为合成单元并利用私有微调LLM的自回归能力，显著提升了时序数据的连贯性和质量，填补了现有研究在时序数据合成方面的空白。
...
TL;DR: 该论文从特征学习角度建立了全数据回放持续学习的理论框架，发现即使在全数据回放下，当后续任务的累积噪声主导先前任务的信号时仍会发生遗忘，但通过足够的信号积累可以恢复早期任务，并提出优先学习高信号任务有助于防止灾难性遗忘。


<details>
  <summary>Details</summary>
Motivation: 持续学习（CL）面临灾难性遗忘的核心挑战。数据回放方法虽然简单有效，但全数据回放（所有历史数据可访问）的理论有效性尚未充分探索。本文旨在从特征学习角度建立全数据回放持续学习的理论分析框架。

Method: 采用多视图数据模型，以信噪比（SNR）为关键因素分析遗忘现象。聚焦于M个任务的增量二元分类问题，从理论上分析全数据回放训练。通过合成和真实世界实验验证理论发现，可视化不同SNR和任务相关性机制下信号学习与噪声记忆的相互作用。

Result: 1. 即使在全数据回放下，当后续任务的累积噪声主导先前任务的信号时，遗忘仍会发生；2. 通过足够的信号积累，数据回放可以恢复早期任务，即使其初始学习效果不佳；3. 发现任务排序的新见解：优先学习高信号任务不仅有助于学习低信号任务，还能帮助防止灾难性遗忘。

Conclusion: 本文建立了全数据回放持续学习的理论框架，揭示了信噪比在遗忘中的关键作用。研究结果表明，即使在全数据可访问的理想条件下，灾难性遗忘仍可能发生，但通过适当的任务排序和信号积累可以缓解。这些发现为设计更有效的持续学习算法提供了理论指导。
...
<details>
  <summary>Details</summary>
Motivation: 现有多模态融合方法大多忽略或需要昂贵的时间扭曲来处理EEG与周边生理信号的时间异步问题，而中枢神经系统与自主神经系统之间的耦合关系需要在时间维度上进行建模。

Method: 提出Cross-Temporal Attention Fusion (CTAF)模块，包含：1) 时间感知交叉注意力学习软双向对齐；2) 轻量级融合门；3) 对齐正则化的对比学习目标（可选弱监督）。

Result: 在K-EmoCon数据集上，CTAF在留一法交叉验证中：1) 匹配对获得更高的余弦边界；2) 一秒内实现更好的跨模态token检索；3) 在三分类准确率和宏F1上与基线竞争，同时使用较少标签。

Conclusion: CTAF通过直接建模时间对应关系的时间感知融合机制、针对EEG与生理信号的对齐驱动自监督目标、以及测量对齐质量的评估协议，为时间异步下的标签高效、可泛化的EEG-周边生理信号融合提供了有效方案。
...
### [67] [Structure-Preserving Learning Improves Geometry Generalization in Neural PDEs](https://arxiv.org/abs/2602.02788)
*Benjamin D. Shaffer,Shawn Koohy,Brooks Kinch,M. Ani Hsieh,Nathaniel Trask*

Main category: cs.LG

TL;DR: Geo-NeW：一种数据驱动的有限元方法，通过联合学习微分算子和兼容的降维有限元空间，在未见几何上保持物理守恒定律和精度。


<details>
  <summary>Details</summary>
Motivation: 开发用于科学和工程的物理基础模型，能够实时求解偏微分方程，并在适应未见几何时保持结构和精度。

Method: 提出General-Geometry Neural Whitney Forms (Geo-NeW)：一种数据驱动的有限元方法。联合学习微分算子和在底层几何上定义的兼容降维有限元空间。通过有限元外微积分精确保持物理守恒定律。几何通过离散网格以两种方式进入模型：基于transformer的编码和作为学习有限元空间的基础。提供本构模型的新参数化以确保解的存在性和唯一性。

Result: 在多个稳态PDE基准测试中达到最先进性能，在分布外几何上相比传统基线有显著改进。模型能够泛化到未见域。

Conclusion: Geo-NeW通过显式连接底层几何和边界条件与解，为学习神经PDE提供了强大的归纳偏置，在未见几何上实现了更好的泛化能力。
...
### [68] [Causality--Δ: Jacobian-Based Dependency Analysis in Flow Matching Models](https://arxiv.org/abs/2602.02793)
*Reza Rezvan,Gustav Gille,Moritz Schauer,Richard Torkar*

Main category: cs.LG

TL;DR: 该论文研究流匹配中潜在扰动如何通过流传播，利用雅可比向量积分析生成特征中的依赖结构，在合成和图像数据集上验证了方法的有效性。


<details>
  <summary>Details</summary>
Motivation: 研究流匹配中潜在扰动如何传播，探索生成特征中的依赖结构，为理解流模型的局部线性结构提供理论框架。

Method: 推导高斯和高斯混合模型下最优漂移及其雅可比矩阵的闭式解，利用雅可比向量积分析特征依赖结构，在合成和图像数据集上进行数值验证。

Result: 在低维合成基准测试中，数值雅可比向量积能恢复解析雅可比矩阵；在图像领域，通过组合流与属性分类器构建的属性级雅可比向量积估计器在MNIST和CelebA上恢复了经验相关性。

Conclusion: 流匹配模型即使全局非线性也表现出局部仿射结构，雅可比向量积为分析生成特征依赖提供了实用工具，但基于分类器-雅可比范数的条件化并非正式的do干预。
...
### [69] [Joint Learning of Hierarchical Neural Options and Abstract World Model](https://arxiv.org/abs/2602.02799)
...
<details>
  <summary>Details</summary>
Motivation: 构建能够通过组合现有技能执行新技能的智能体是AI研究的长期目标，但现有模型无关的分层强化学习算法需要大量数据，样本效率低下
...
### [70] [From Tokens to Numbers: Continuous Number Modeling for SVG Generation](https://arxiv.org/abs/2602.02820)
*Michael Ogezi,Martin Bell,Freda Shi,Ethan Smith*

Main category: cs.LG

TL;DR: 提出Continuous Number Modeling (CNM)方法，将SVG中的数值参数作为连续值而非离散token直接建模，解决了向量图形生成中数值序列编码效率低的问题，结合多模态Transformer和强化学习微调，在训练速度和视觉质量上均有提升。


<details>
  <summary>Details</summary>
Motivation: 向量图形（如SVG）在图像生成任务中具有灵活性、尺寸效率和编辑便利性等优势，但相比基于栅格的方法研究较少。核心挑战在于SVG中大量数值几何参数被编码为长token序列，导致训练缓慢、准确性降低和泛化能力差。

Method: 提出Continuous Number Modeling (CNM)方法，将数字作为连续值而非离散token直接建模，消除基于token编码引入的离散化伪影。首先在200万个栅格到SVG样本上训练多模态Transformer，然后通过基于感知反馈的强化学习进行微调以提升视觉质量。

Result: CNM方法将训练速度提升超过30%，同时保持比替代方法更高的感知保真度。该方法为高质量向量图形生成建立了实用高效的解决方案。

Conclusion: CNM通过将数字作为连续值直接建模，恢复了表示形式的数学优雅性，使模型输入与数据的连续性质对齐。该方法为高质量向量图形生成提供了实用高效的方法，并具有更广泛应用的潜力。
...
### [71] [A Single Revision Step Improves Token-Efficient LLM Reasoning](https://arxiv.org/abs/2602.02828)
*Yingchuan Zhang,Terry Ma,Wenxuan Zhong,Ping Ma*

Main category: cs.LG

TL;DR: PACER框架通过让推理轨迹相互"同行评审"来解决LLM推理中的"盲点"问题，显著提升复杂数学问题的准确性


<details>
  <summary>Details</summary>
Motivation: 传统多数投票或基于置信度过滤的方法存在根本性"盲点"：它们孤立评估每个推理轨迹。随着问题难度增加，模型常生成具有误导性高置信度的幻觉路径，导致真实解在传统投票中被微弱优势压制

Method: 提出无需训练的推理框架PACER：1) 对生成轨迹进行初步筛选；2) 构建包含候选答案、聚合置信度和代表性推理摘要的共识包；3) 各轨迹基于此包进行针对性自我审查，识别与共识分歧的逻辑节点并修正；4) 通过置信度加权投票获得最终预测

Result: 在AIME和BRUMO等具有挑战性的数学基准测试中，PACER匹配或超越256样本多数投票的准确性，显著优于原始集成基线，将简单共识转化为协作逻辑精炼过程

Conclusion: 通过让推理轨迹相互"同行评审"，PACER能够解决传统聚合方法的盲点，显著提升LLM在复杂推理任务中的准确性，为多轨迹推理提供了更有效的协作框架
...
### [72] [SC3D: Dynamic and Differentiable Causal Discovery for Temporal and Instantaneous Graphs](https://arxiv.org/abs/2602.02830)
...
Method: 提出Stable Causal Dynamic Differentiable Discovery (SC3D)两阶段可微分框架：第一阶段通过节点预测进行边预选，获得滞后边和瞬时边的掩码；第二阶段通过优化稀疏性似然并强制瞬时块的无环性来细化这些掩码

Result: 在合成和基准动态系统上的数值结果表明，SC3D相比现有时间基线方法，在滞后和瞬时因果结构的恢复上具有更高的稳定性和准确性

Conclusion: SC3D框架能够有效发现多元时间序列中的跨滞后因果结构和瞬时依赖关系，解决了动态图搜索空间的组合性问题
...
### [73] [Koopman Autoencoders with Continuous-Time Latent Dynamics for Fluid Dynamics Forecasting](https://arxiv.org/abs/2602.02832)
...
Conclusion: 连续时间Koopman框架通过数值积分实现可变时间步长推理，提高了时间分辨率鲁棒性和长期预测能力，为湍流模拟提供了有效的物理基础替代方案
...
TL;DR: RASA通过添加边类型嵌入和稀疏注意力掩码，改进了Transformer在多跳关系推理任务上的表现，在MetaQA和WebQuestionsSP上超越标准Transformer并匹配GPT-4


<details>
  <summary>Details</summary>
Motivation: 标准Transformer在处理需要结构化数据上进行多跳关系推理的任务时表现不佳，这源于其电路复杂性的限制：标准Transformer是TC⁰完全的，需要Ω(k)层来进行k跳推理

Method: 提出RASA（关系感知稀疏注意力），包含两个关键修改：(1) 边类型嵌入：将关系结构注入注意力分数中；(2) 稀疏掩码：将注意力限制在图相邻位置上。稀疏掩码将注意力搜索空间从O(2^{n²})减少到O(2^m)，边偏置提供显式的关系路由

Result: 在MetaQA（1/2/3跳）和WebQuestionsSP上，RASA优于标准Transformer，并以更低的成本匹配GPT-4的性能，随着推理深度的增加优势更明显（在3跳任务上提升7.1分）

Conclusion: 虽然RASA具有相同的渐近深度要求，但通过最小结构修改（边类型嵌入和稀疏注意力）显著改善了多跳关系推理能力，提供了经验验证而非形式化可学习性保证
...
<details>
  <summary>Details</summary>
Motivation: 深度学习在数据丰富时表现良好，但在实际常见的数据稀缺场景中表现不佳。基础模型虽然通过大规模数据训练具有强泛化能力，但在下游微调时仍受限于标注数据稀缺问题。

Method: 提出GeLDA框架：1）在基础模型诱导的潜在空间中进行数据生成，该空间维度低且集中任务相关信息；2）使用条件扩散模型；3）利用捕捉类别或子域间语义关系的辅助特征向量作为生成条件；4）支持低资源领域的数据增强。

Result: 1）在零样本语言特定语音情感识别任务中，GeLDA将Whisper-large基线的未加权平均召回率提升6.13%；2）在长尾图像分类任务中，在ImageNet-LT上达到74.7%的尾部类别准确率，创下新的最先进结果。

Conclusion: GeLDA通过语义感知的潜在空间数据增强有效解决了数据稀缺问题，在两个大规模识别任务中显著提升了性能，证明了其在低资源场景下的实用价值。
...
TL;DR: 该论文提出了一种基于因果视角的鲁棒离线强化学习方法，用于处理像素演示数据中的混淆观测问题，通过流匹配策略和深度判别器实现，在25个像素任务中性能提升120%


<details>
  <summary>Details</summary>
Motivation: 当演示者与学习者的感知能力不匹配时，像素演示数据中存在未测量的混淆变量，导致传统策略梯度方法基于的无混淆假设不成立，从而产生隐式的混淆偏差

Method: 1. 从因果视角分析离线RL中的混淆观测问题；2. 开发新的因果离线RL目标，优化策略在混淆偏差下的最坏情况性能；3. 提出实际实现方案：学习基于流匹配的表达性策略，使用深度判别器评估目标策略与名义行为策略之间的差异

Result: 在25个像素任务上的实验表明，所提出的混淆鲁棒增强程序实现了120%的成功率提升，显著优于未考虑混淆的先进离线RL方法

Conclusion: 通过因果视角处理像素演示数据中的混淆观测问题，提出的混淆鲁棒离线RL方法能有效提升策略在最坏情况下的性能，显著优于现有方法
...
TL;DR: ZS-SVD是一种后训练SVD压缩方法，通过激活白化和一阶校准损失估计进行全局奇异分量选择，使用零和规则保持累积预测损失变化接近零，自动产生异构秩分配，无需显式优化。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型的内存和计算成本阻碍部署，SVD压缩可以减少存储并加速推理，但性能取决于全局压缩比下的秩分配策略。现有方法要么对相似尺寸矩阵使用同质秩（忽略损失敏感性差异），要么依赖昂贵的迭代预截断优化来确定每矩阵秩。

Method: 提出ZS-SVD后训练方法：1）使用激活白化和白化坐标中的一阶校准损失估计进行全局奇异分量选择；2）采用零和规则在整个模型中剪枝分量，保持累积预测损失变化接近零，自动产生异构秩；3）可选轻量级校正：应用单次投影梯度更新后重新截断。

Result: 在多种LLM架构上的广泛实验显示，在不同基准测试和压缩比下均获得一致性能提升。

Conclusion: ZS-SVD通过零和规则实现高效的全局奇异分量选择，无需显式秩分配优化，在保持性能的同时实现有效的模型压缩。
...
### [78] [Recurrent Equivariant Constraint Modulation: Learning Per-Layer Symmetry Relaxation from Data](https://arxiv.org/abs/2602.02853)
*Stefanos Pertigkiozoglou,Mircea Petrache,Shubhendu Trivedi,Kostas Daniilidis*

Main category: cs.LG

TL;DR: RECM是一种学习性约束调制机制，能够根据训练信号和每层输入-目标分布的对称性自动调整等变约束的松弛程度，无需预设任务相关的松弛水平。


<details>
  <summary>Details</summary>
Motivation: 严格的等变约束虽然能利用任务对称性改善泛化，但可能导致更复杂的优化动态而阻碍学习。现有方法通过放松等变约束来解决这一问题，但通常需要预设、显式或隐式的每层松弛水平，这些是任务依赖且调优成本高昂的。

Method: 提出循环等变约束调制(RECM)，这是一种层级的约束调制机制，仅从训练信号和每层输入-目标分布的对称性属性中学习适当的松弛水平，无需任何关于任务相关目标松弛水平的先验知识。

Result: 在RECM更新下，每层的松弛水平可证明收敛到由其对称间隙上界限定的值。处理对称分布的层恢复完全等变性，而具有近似对称性的层保留足够的灵活性来学习数据支持的非对称解。在包括GEOM-Drugs数据集上具有挑战性的分子构象生成在内的多种精确和近似等变任务中，RECM优于先前方法。

Conclusion: RECM提供了一种自适应机制，能够根据数据的内在对称性自动调整等变约束的松弛程度，在保持对称性优势的同时避免严格约束带来的优化困难，在多种等变任务中表现出优越性能。
...
### [79] [When pre-training hurts LoRA fine-tuning: a dynamical analysis via single-index models](https://arxiv.org/abs/2602.02855)
*Gibbs Nwemadji,Bruno Loureiro,Jean Barbier*

Main category: cs.LG

TL;DR: 预训练并不总是加速下游任务微调，过度预训练可能计算性地减慢LoRA微调优化


<details>
  <summary>Details</summary>
Motivation: 传统观点认为预训练能促进下游任务微调，但本文旨在揭示这种直觉并不总是成立，特别是当预训练过度时可能反而阻碍微调优化

Method: 使用单指数模型和单次SGD训练，通过总结统计量描述微调动态，分析LoRA微调收敛速率与初始对齐度和目标任务非线性程度的关系

Result: 即使预训练与下游任务对齐良好，强预训练也可能导致搜索阶段延长并阻碍收敛，预训练强度和任务难度共同塑造LoRA微调动态

Conclusion: 预训练对微调的影响是复杂的，过度预训练可能计算性地减慢优化，这为理解预训练强度和任务难度如何共同影响LoRA微调提供了统一的理论框架
...
### [80] [A Geometry-Aware Efficient Algorithm for Compositional Entropic Risk Minimization](https://arxiv.org/abs/2602.02877)
*Xiyuan Wei,Linli Zhou,Bokun Wang,Chih-Jen Lin,Tianbao Yang*

Main category: cs.LG

TL;DR: 提出SCENT算法解决组合熵风险最小化问题，使用随机近端镜像下降和负指数函数诱导的Bregman散度，在凸问题上达到O(1/√T)收敛率，并在多个机器学习任务中优于现有基线。


<details>
  <summary>Details</summary>
Motivation: 组合熵风险最小化问题（Log-E-Exp函数）广泛存在于机器学习中，但现有优化算法存在非收敛、数值不稳定和收敛速度慢等基本限制，需要新的几何感知算法来解决这些问题。

Method: 提出SCENT算法：将熵风险最小化转化为min-min对偶问题，使用随机近端镜像下降更新对偶变量，采用负指数函数诱导的Bregman散度来准确捕捉目标函数的几何结构。

Result: 理论证明：1) SCENT在凸问题上达到O(1/√T)收敛率；2) 理论上证明了SPMD相对于标准SGD更新对偶变量的优势；3) 在极端分类、部分AUC最大化、对比学习和分布鲁棒优化等任务中，SCENT一致优于现有基线方法。

Conclusion: SCENT算法通过几何感知的随机优化方法有效解决了组合熵风险最小化问题，在理论和实证上都表现出优越性能，为Log-E-Exp类目标函数提供了稳定高效的优化方案。
...
### [81] [Mixture of Concept Bottleneck Experts](https://arxiv.org/abs/2602.02886)
...
TL;DR: M-CBEs扩展概念瓶颈模型，通过混合多个专家和多样化功能形式来提升预测准确性和适应性，平衡可解释性与性能的权衡。


<details>
  <summary>Details</summary>
Motivation: 现有概念瓶颈模型通常将任务预测器固定为单一线性或布尔表达式，这限制了预测准确性和适应不同用户需求的能力。需要更灵活的设计空间来平衡可解释性与性能。

Method: 提出混合概念瓶颈专家框架，从两个维度扩展现有模型：专家数量和每个专家的功能形式。实例化两种新模型：线性M-CBE（学习有限线性表达式集合）和符号M-CBE（利用符号回归在用户指定运算符词汇下从数据中发现专家函数）。

Result: 实证评估表明，通过改变混合规模和功能形式，为导航准确性与可解释性权衡提供了稳健框架，能够适应不同用户和任务需求。

Conclusion: M-CBEs框架通过暴露设计空间中未被充分探索的区域，为概念瓶颈模型提供了更灵活和适应性强的解决方案，在保持可解释性的同时提升了预测性能。
...
### [82] [Self-Soupervision: Cooking Model Soups without Labels](https://arxiv.org/abs/2602.02890)
*Anthony Fuller,James R. Green,Evan Shelhamer*

Main category: cs.LG

TL;DR: 该论文提出了Self-Souping方法，将模型汤（model soups）技术从监督学习扩展到自监督学习，通过在未标记数据上训练不同成分模型并混合参数来提升模型鲁棒性和准确性。


<details>
  <summary>Details</summary>
Motivation: 现有模型汤方法仅限于监督学习，且需要在相同损失函数和标记数据上优化。作者希望将模型汤技术扩展到自监督学习领域，以利用未标记数据（如任务迁移数据或分布偏移数据）来提升模型鲁棒性。

Method: 提出Self-Souping方法：1）在自监督学习框架下，使用不同数据源（如未标记任务数据或分布偏移数据）训练多个模型成分；2）允许成分模型在SSL超参数甚至SSL算法上存在差异；3）将不同成分模型的参数混合形成最终模型汤；4）在测试数据上应用Self-Souping后，再在干净训练数据上进行微调。

Result: 1）在ImageNet-C上提升鲁棒性3.5%，在LAION-C上提升7%；2）首次证明不同SSL算法（如MAE、MoCoV3、MMCR）的成分可以混合形成更准确的模型汤；3）Self-Souping模型汤比任何单一SSL成分模型都更准确。

Conclusion: Self-Souping成功将模型汤技术扩展到自监督学习领域，通过混合不同SSL算法和超参数的成分模型，显著提升了模型鲁棒性和准确性，为利用未标记数据提升模型性能提供了新途径。
...
### [83] [Controlled disagreement improves generalization in decentralized training](https://arxiv.org/abs/2602.02899)
*Zesen Wang,Mikael Johansson*

Main category: cs.LG

TL;DR: DSGD-AC通过保留非零共识误差作为结构化扰动，引导优化趋向平坦最小值，在图像分类和机器翻译任务中超越标准DSGD和集中式SGD。
...
Method: 提出去中心化SGD与自适应共识（DSGD-AC），通过时间相关的缩放机制有意保留非消失的共识误差，这些误差与主导Hessian子空间对齐，作为结构化扰动引导优化。

Result: 在图像分类和机器翻译基准测试中，DSGD-AC在测试准确率和解平坦度方面一致超越标准DSGD和集中式SGD。

Conclusion: 共识误差是有用的隐式正则化器，为去中心化学习算法设计提供了新视角，证明了去中心化训练可以通过利用共识误差获得比集中式训练更好的泛化性能。
...
### [84] [Manifold-Constrained Energy-Based Transition Models for Offline Reinforcement Learning](https://arxiv.org/abs/2602.02900)
*Zeyu Fang,Zuyuan Zhang,Mahdi Imani,Tian Lan*

Main category: cs.LG

TL;DR: MC-ETM通过基于能量的流形约束转移模型解决离线强化学习中的分布偏移问题，使用流形投影-扩散负采样训练，并通过能量信号进行悲观策略优化


<details>
  <summary>Details</summary>
Motivation: 基于模型的离线强化学习在分布偏移下很脆弱：策略改进将rollout驱动到数据集支持较弱的区域，模型误差累积导致严重的价值高估问题

Method: 提出流形约束能量转移模型(MC-ETM)：1) 使用流形投影-扩散负采样训练条件能量转移模型，学习下一个状态的潜在流形；2) 通过扰动潜在编码和在潜在空间中运行Langevin动力学生成近流形硬负样本；3) 使用学习到的能量作为可靠性信号：当采样下一个状态的最小能量超过阈值时截断rollout；4) 基于能量引导样本的Q值离散度进行悲观惩罚稳定Bellman备份

Result: MC-ETM提高了多步动态保真度，在标准离线控制基准测试中获得了更高的归一化回报，特别是在不规则动态和稀疏数据覆盖情况下表现更好

Conclusion: MC-ETM通过混合悲观MDP公式化，分离了支持内评估误差和截断风险，提供了一种有效的离线强化学习方法，能够处理分布偏移问题并提高策略性能
...
### [85] [Weighted Temporal Decay Loss for Learning Wearable PPG Data with Sparse Clinical Labels](https://arxiv.org/abs/2602.02917)
...
TL;DR: 提出一种基于PPG信号的生物标志物监测方法，通过学习样本权重随时间衰减的规律来解决临床标签稀疏性问题，提高健康监测算法的准确性。


<details>
  <summary>Details</summary>
Motivation: 可穿戴设备和AI技术的发展推动了基于PPG信号的健康监测，但临床标签稀疏性导致距离实验室抽血时间较远的生物信号监督不可靠，需要解决这一时间间隔带来的监督信号衰减问题。

Method: 提出一种训练策略，学习生物标志物特定的样本权重衰减函数，根据PPG信号段与其真实标签之间的时间间隔调整样本权重，并在损失函数中加入正则化项防止平凡解。在450名参与者的智能手表PPG数据上评估了10种生物标志物。

Result: 该方法在受试者层面的平均AUPRC达到0.715，优于微调的自监督基线（0.674）和基于特征的随机森林（0.626）。比较四种衰减函数家族发现，简单的线性衰减函数平均表现最稳健。

Conclusion: 提出的方法不仅提高了生物标志物监测的准确性，而且学习到的衰减率提供了生物标志物PPG证据随时间失效速度的可解释性视图，揭示了不同生物标志物的时间敏感性差异。
...
### [86] [A Reproducible Framework for Bias-Resistant Machine Learning on Small-Sample Neuroimaging Data](https://arxiv.org/abs/2602.02920)
...
<details>
  <summary>Details</summary>
Motivation: 传统交叉验证框架在模型选择和性能评估中重用相同数据折叠，导致乐观偏差结果，限制了可重复性和泛化能力，特别是在小样本生物医学数据中。

Method: 集成领域知识特征工程、嵌套交叉验证和校准决策阈值优化的机器学习框架，通过重要性引导排序选择紧凑可解释的特征子集。
...
### [87] [How Does the Lagrangian Guide Safe Reinforcement Learning through Diffusion Models?](https://arxiv.org/abs/2602.02924)
*Xiaoyuan Cheng,Wenxuan Yuan,Boyang Li,Yuanchao Xu,Yiming Yang,Hao Liang,Bei Peng,Robert Loftin,Zhuo Sun,Yukun Hu*

Main category: cs.LG

TL;DR: 提出ALGD算法，通过增强拉格朗日方法稳定扩散策略在安全强化学习中的训练过程


<details>
  <summary>Details</summary>
Motivation: 现有基于扩散的RL方法主要关注离线奖励最大化，对在线环境中的安全性考虑有限，且原始对偶方法在非凸拉格朗日景观中不稳定

Method: 提出增强拉格朗日引导扩散(ALGD)算法，通过引入增强拉格朗日函数局部凸化能量景观，稳定策略生成和训练过程，而不改变最优策略分布

Result: 理论分析和大量实验表明ALGD具有理论依据且实证有效，在多样化环境中实现强大而稳定的性能

Conclusion: ALGD成功解决了扩散基安全RL中的训练不稳定问题，为在线安全强化学习提供了有效的解决方案
...
### [88] [Distance Marching for Generative Modeling](https://arxiv.org/abs/2602.02928)
...
<details>
  <summary>Details</summary>
Motivation: 无条件时间生成模型学习时间独立的去噪向量场，但同一噪声输入可能对应多个噪声水平和不同的去噪方向，这会干扰监督信号。需要解决这一根本问题。

Method: 提出Distance Marching方法，受距离场建模启发，设计两种原则性推理方法。关键创新是设计聚焦更近目标的损失函数，使去噪方向更好地指向数据流形。

Result: 在CIFAR-10和ImageNet上，Distance Marching相比近期无条件时间基线持续改善FID 13.5%。对于类别条件ImageNet生成，尽管移除了时间输入，但超越流匹配性能：使用60%采样步骤达到流匹配最终性能的更低FID，在不同骨干大小上平均FID降低13.6%。距离预测还有助于采样中的早期停止和OOD检测。

Conclusion: 距离场建模可以作为生成建模的原则性视角，Distance Marching方法在无条件时间设置中表现出色，为生成模型提供了新的设计思路。
...
### [89] [RPG-AE: Neuro-Symbolic Graph Autoencoders with Rare Pattern Mining for Provenance-Based Anomaly Detection](https://arxiv.org/abs/2602.02929)
*Asif Tauhid,Sidahmed Benabderrahmane,Mohamad Altrabulsi,Ahamed Foisal,Talal Rahwan*

Main category: cs.LG

TL;DR: 提出一种结合图自编码器与稀有模式挖掘的神经符号异常检测框架，用于在系统级溯源数据中识别APT类活动，在DARPA透明计算数据集上验证了性能提升。


<details>
  <summary>Details</summary>
Motivation: 高级持续性威胁（APTs）具有隐蔽性强、持续时间长、难以检测的特点，它们常伪装成正常系统行为。现有检测方法在识别APT类活动方面存在局限性，需要结合多种技术手段提高检测效果和可解释性。

Method: 1. 基于特征相似性使用k近邻构建进程行为图；2. 使用图自编码器学习正常关系结构；3. 通过观测图与重构图之间的偏差识别异常候选；4. 集成稀有模式挖掘模块，发现不频繁的行为共现模式，并用这些模式提升具有稀有特征的进程的异常分数。

Result: 在DARPA透明计算数据集上的评估表明：1. 稀有模式增强相比基线图自编码器在异常排名质量上有显著提升；2. 与相同基准上的现有无监督方法相比，该单一统一模型持续优于基于上下文的独立检测器；3. 性能与需要多个独立检测器的集成聚合方法相当。

Conclusion: 将基于图的表示学习与经典模式挖掘相结合，能够有效提升溯源安全异常检测的效果和可解释性，为APT检测提供了有前景的神经符号融合方法。
...
### [90] [Rare Event Early Detection: A Dataset of Sepsis Onset for Critically Ill Trauma Patients](https://arxiv.org/abs/2602.02930)
*Yin Jin,Tucker R. Stewart,Deyi Zhou,Chhavi Gupta,Arjita Nema,Scott C. Brakenridge,Grant E. O'Keefe,Juhua Hu*

Main category: cs.LG

TL;DR: 该论文针对ICU创伤患者脓毒症早期检测问题，创建了首个公开的标准化创伤后脓毒症发病数据集，并建立了基准测试框架。


<details>
  <summary>Details</summary>
Motivation: 现有公共数据集将ICU患者视为同质群体，忽视了创伤患者特有的挑战：损伤相关炎症和器官功能障碍可能与脓毒症临床特征重叠，需要针对创伤后脓毒症进行专门识别以开发早期检测方法。

Method: 从MIMIC-III数据库中提取、重新标注并验证创伤后脓毒症发病数据集，采用标准化创伤后临床事实进行标注；根据ICU临床工作流程，将创伤后脓毒症早期检测框架化为每日一次的罕见事件检测问题。

Result: 创建了首个公开可用的标准化创伤后脓毒症发病数据集，并通过全面实验建立了通用基准，结果表明使用该新数据集需要进一步的技术进步。

Conclusion: 针对创伤患者脓毒症早期检测需要专门的数据集和方法，提出的标准化数据集为开发更准确的创伤后脓毒症检测算法提供了基础，数据代码已公开可用。
...
### [91] [3D-Learning: Diffusion-Augmented Distributionally Robust Decision-Focused Learning](https://arxiv.org/abs/2602.02943)
...
<details>
  <summary>Details</summary>
Motivation: 预测-优化管道中的机器学习模型在测试时面临分布外样本的挑战，导致预测误差增大和决策性能显著下降。现有方法难以在分布外条件下实现良好的泛化性能。

Method: 提出分布鲁棒决策聚焦学习框架，其中3D-Learning方法利用扩散模型的强大分布建模能力，在其参数化空间中搜索最坏情况分布，而非依赖经典分布鲁棒优化技术。

Result: 在LLM资源供应任务上的实证结果表明，3D-Learning在分布外泛化性能方面优于现有的分布鲁棒优化和数据增强方法。

Conclusion: 3D-Learning框架通过在扩散模型参数化空间中搜索与真实数据一致的最坏情况分布，实现了平均情况和最坏情况之间的良好平衡，有效提升了预测-优化管道在分布外条件下的决策性能。
...
### [92] [Variational Sparse Paired Autoencoders (vsPAIR) for Inverse Problems and Uncertainty Quantification](https://arxiv.org/abs/2602.02948)
...
<details>
  <summary>Details</summary>
Motivation: 逆问题在许多科学和工程领域中至关重要，但传统方法通常只提供点估计而缺乏可解释的不确定性。现有方法难以同时实现快速推理和不确定性估计，特别是在需要结构化、可解释的不确定性时面临挑战。

Method: 提出变分稀疏配对自编码器(vsPAIR)，将标准VAE编码观测数据与稀疏VAE编码感兴趣量配对，通过学习的潜在映射连接。采用硬混凝土尖峰-平板松弛实现可微分训练，使用beta超先验实现自适应稀疏度控制。

Result: 在盲修复和计算机断层扫描实验中验证了vsPAIR的有效性，证明其能够作为有效的逆问题求解器，提供可解释和结构化的不确定性估计。

Conclusion: vsPAIR通过配对架构、稀疏编码和变分框架，成功解决了逆问题中快速推理与可解释不确定性估计的挑战，为科学和工程应用提供了实用的解决方案。
...
### [93] [Quant VideoGen: Auto-Regressive Long Video Generation via 2-Bit KV-Cache Quantization](https://arxiv.org/abs/2602.02958)
...
<details>
  <summary>Details</summary>
Motivation: 自回归视频扩散模型中KV缓存内存随生成历史增长，通常超过30GB，限制了在广泛可用硬件上的部署。更重要的是，受限的KV缓存预算限制了有效工作内存，直接降低了身份、布局和运动方面的长时一致性。

Method: QVG通过语义感知平滑利用视频时空冗余，产生低幅度、量化友好的残差。进一步引入渐进残差量化，这是一种从粗到细的多阶段方案，减少量化误差同时实现平滑的质量-内存权衡。

Result: 在LongCat Video、HY WorldPlay和Self Forcing基准测试中，QVG建立了质量与内存效率之间的新帕累托前沿，将KV缓存内存减少高达7.0倍，端到端延迟开销小于4%，同时在生成质量上持续优于现有基线。

Conclusion: QVG通过创新的量化框架有效解决了自回归视频扩散模型中KV缓存内存瓶颈问题，实现了高质量视频生成与内存效率的平衡，为实际部署提供了可行方案。
...
### [94] [Human-Centric Traffic Signal Control for Equity: A Multi-Agent Action Branching Deep Reinforcement Learning Approach](https://arxiv.org/abs/2602.02959)
*Xiaocai Zhang,Neema Nassir,Lok Sang Chan,Milad Haghani*

Main category: cs.LG

TL;DR: MA2B-DDQN：一种以人为中心的多智能体行动分支双DQN框架，通过分解交通信号控制为局部和全局决策，优化旅行者层面的公平性，显著减少受影响出行者数量。


<details>
  <summary>Details</summary>
Motivation: 现有多智能体深度强化学习方法多为车辆中心主义，难以处理高维离散动作空间，且缺乏对行人、车辆乘客和公交乘客等不同出行者群体的公平性考虑。

Method: 提出MA2B-DDQN框架，采用行动分支离散控制策略：将走廊控制分解为（i）局部交叉口动作（分配下一两个相位的绿灯时间）和（ii）全局动作（选择这些相位的总持续时间）。设计以人为中心的奖励函数，惩罚走廊中延迟的个体数量。

Result: 在澳大利亚墨尔本的七个现实交通场景中进行广泛评估，结果显示该方法显著减少了受影响出行者数量，优于现有DRL和基准方法。实验证实模型具有鲁棒性，在不同设置下方差最小。

Conclusion: 该框架不仅倡导更公平的交通信号系统，还提供了可扩展的解决方案，能够适应各种城市交通条件，为多模态走廊协调控制提供了有效方法。
...
### [95] [Q-ShiftDP: A Differentially Private Parameter-Shift Rule for Quantum Machine Learning](https://arxiv.org/abs/2602.02962)
...
<details>
  <summary>Details</summary>
Motivation: 量子机器学习虽然具有计算优势，但训练数据隐私保护面临挑战。经典的差分隐私随机梯度下降方法无法充分利用量子梯度估计的独特性质，需要专门针对QML的隐私保护机制。

Method: 提出Q-ShiftDP方法，利用参数平移规则计算的量子梯度的固有有界性和随机性，结合精心校准的高斯噪声和固有量子噪声，提供形式化的隐私和效用保证。

Result: Q-ShiftDP在基准数据集上的实验表明，该方法在量子机器学习中持续优于经典的差分隐私方法，通过利用量子噪声进一步改善了隐私-效用权衡。

Conclusion: Q-ShiftDP是首个专门为量子机器学习设计的差分隐私机制，通过利用量子梯度估计的独特性质实现了更严格的敏感性分析和更低的噪声需求，为QML提供了有效的隐私保护解决方案。
...
### [96] [Co2PO: Coordinated Constrained Policy Optimization for Multi-Agent RL](https://arxiv.org/abs/2602.02970)
*Shrenik Patel,Christine Truong*

Main category: cs.LG

TL;DR: Co2PO是一种新型多智能体强化学习框架，通过风险感知通信实现协调驱动的安全性，避免传统约束方法的探索抑制和过度保守问题


<details>
  <summary>Details</summary>
Motivation: 传统约束多智能体强化学习方法（如拉格朗日方法）依赖全局惩罚或集中式批评器，通常在违规发生后做出反应，抑制探索并导致过度保守。需要在探索和安全约束优化之间找到更好的平衡

Method: 提出Co2PO框架，采用通信增强架构：1) 共享黑板架构广播位置意图和让行信号；2) 学习型危险预测器主动预测长期时间范围内的潜在违规；3) 将预测整合到约束优化目标中，实现前瞻性安全导航

Result: 在复杂多智能体安全基准测试中，Co2PO相比领先的约束基线方法获得更高回报，同时收敛到符合成本约束的策略。消融研究验证了风险触发通信、自适应门控和共享内存组件的必要性

Conclusion: Co2PO通过选择性风险感知通信和前瞻性危险预测，实现了协调驱动的安全性，避免了传统反应式约束方法的性能权衡，为约束多智能体强化学习提供了更有效的解决方案
...
### [97] [Learning to Repair Lean Proofs from Compiler Feedback](https://arxiv.org/abs/2602.02990)
*Evan Wang,Simon Chess,Daniel Lee,Siyuan Ge,Ajit Mallavarapu,Vasily Ilin*

Main category: cs.LG

TL;DR: APRIL数据集为Lean定理证明器中的证明修复提供监督学习数据，包含26万个错误证明、编译器反馈、修复目标和解释的元组，训练语言模型可显著提升修复准确性和反馈条件推理能力。


<details>
  <summary>Details</summary>
Motivation: 现有Lean数据集几乎只包含正确证明，缺乏对理解和修复失败证明的监督。随着神经定理证明器变得更加自主，理解和响应编译器反馈的能力变得至关重要。

Method: 将Lean证明修复构建为监督学习问题：给定错误证明和编译器反馈，预测修正后的证明和基于相同反馈的自然语言诊断。引入APRIL数据集，包含26万个系统生成的证明失败与编译器诊断、对齐的修复和解释目标的监督元组。

Result: 在APRIL上训练语言模型显著提高了修复准确性和反馈条件推理能力。在单次修复评估设置中，微调的40亿参数模型优于最强的开源基线。

Conclusion: 诊断条件监督可作为使用反馈的证明器的补充训练信号。APRIL数据集为定理证明中的证明修复和反馈理解提供了有价值的监督学习资源。
...
### [98] [Adaptive Batch Sizes Using Non-Euclidean Gradient Noise Scales for Stochastic Sign and Spectral Descent](https://arxiv.org/abs/2602.03001)
*Hiroki Naganuma,Shagun Gupta,Youssef Briki,Ioannis Mitliagkas,Irina Rish,Parameswaran Raman,Hao-Jun Michael Shi*

Main category: cs.LG

TL;DR: 提出非欧几里得梯度噪声尺度用于signSGD和specSGD优化器，通过分布式系统中的局部小批量梯度方差估计实现自适应批量大小调度，在160M参数Llama模型上减少66%训练步骤


<details>
  <summary>Details</summary>
Motivation: 现代机器学习系统通常使用大型恒定或手动调优的批量大小调度，依赖启发式方法且成本高昂。现有的基于梯度噪声尺度的自适应策略假设欧几里得几何，与基于广义范数的优化器（如signSGD/Signum和specSGD/Muon）存在根本性不匹配

Method: 推导了signSGD和specSGD的梯度噪声尺度，这些尺度自然地从各自对偶范数的几何中产生。提出高效的方差估计程序，利用分布式数据并行系统中不同rank上的局部小批量梯度来估计这些非欧几里得度量

Result: 实验表明，使用非欧几里得梯度噪声尺度的自适应批量大小策略能够在160M参数Llama模型上，为Signum和Muon优化器匹配恒定批量基线的验证损失，同时减少高达66%的训练步骤

Conclusion: 通过从优化器的几何结构中推导梯度噪声尺度并设计高效的分布式估计方法，实现了对非欧几里得优化器的自适应批量大小调度，显著提高了训练效率
...
### [99] [From Zero to Hero: Advancing Zero-Shot Foundation Models for Tabular Outlier Detection](https://arxiv.org/abs/2602.03018)
*Xueying Ding,Haomin Wen,Simon Klütterman,Leman Akoglu*

Main category: cs.LG

TL;DR: OUTFORMER是一个基于基础模型的异常检测框架，通过合成先验混合和自演进课程训练提升FoMo-0D性能，实现零样本快速推理


<details>
  <summary>Details</summary>
Motivation: 异常检测在实际部署中面临缺乏标记异常样本的挑战，导致算法和超参数选择困难。虽然基础模型已改变机器学习领域，但异常检测领域仍需要更高效的零样本解决方案

Method: 1) 使用合成先验混合方法进行预训练；2) 采用自演进课程训练策略；3) 完全在合成标记数据集上预训练；4) 通过上下文学习使用训练数据作为输入进行零样本推理

Result: 在AdBench基准测试中达到最先进性能，同时在作者新引入的两个大规模异常检测基准（包含超过1,500个数据集）上表现优异，且保持快速推理速度

Conclusion: OUTFORMER实现了真正的即插即用异常检测部署，无需额外训练或模型选择，为零样本异常检测提供了高效解决方案
...
### [100] [FedKRSO: Communication and Memory Efficient Federated Fine-Tuning of Large Language Models](https://arxiv.org/abs/2602.03019)
*Guohao Yang,Tongle Wu,Yuanxiong Guo,Ying Sun,Yanmin Gong*

Main category: cs.LG

TL;DR: FedKRSO是一种用于联邦学习环境下大语言模型微调的新方法，通过随机低维子空间更新和模型更新累加器传输，在保持全参数微调性能的同时大幅降低通信和内存开销。


<details>
  <summary>Details</summary>
Motivation: 联邦学习在LLM微调中面临高通信成本和内存消耗的挑战，而现有的参数高效微调方法虽然降低了开销，但往往牺牲了模型性能。需要一种既能保持全参数微调性能，又能降低通信和内存开销的联邦学习微调方法。

Method: FedKRSO采用两种核心策略：1) 客户端在服务器生成的共享随机低维子空间内更新模型，减少内存使用；2) 客户端只传输沿子空间的模型更新累加器而非完整模型参数，实现高效的全局模型聚合和分发。该方法还进行了严格的收敛性分析。

Result: 在GLUE基准测试上的广泛实验表明，FedKRSO在多种联邦学习场景下都能实现卓越的性能，同时保持较低的通信和内存开销，性能接近联邦全参数微调，优于参数高效微调方法。

Conclusion: FedKRSO通过创新的子空间优化和高效通信机制，成功解决了联邦学习中LLM微调的性能与开销权衡问题，为资源受限边缘设备上的联邦LLM微调提供了可行方案。
...
### [101] [CoBA-RL: Capability-Oriented Budget Allocation for Reinforcement Learning in LLMs](https://arxiv.org/abs/2602.03048)
...
<details>
  <summary>Details</summary>
Motivation: 现有强化学习验证奖励（RLVR）框架如GRPO采用均匀的rollout预算分配，导致资源效率低下。现有自适应方法依赖实例级指标（如任务通过率），无法捕捉模型动态学习状态。

Method: 提出CoBA-RL算法：1）使用能力导向价值函数将任务映射到潜在训练收益；2）采用基于堆的贪心策略自适应校准计算资源分布，优先分配给高训练价值的样本。

Result: 实验表明CoBA-RL能有效协调探索与利用的权衡，在多个具有挑战性的基准测试中实现一致的泛化改进。

Conclusion: 量化样本训练价值并优化预算分配对提升LLM后训练效率至关重要。CoBA-RL通过自适应资源分配机制解决了现有方法的局限性。
...
### [102] [Fedcompass: Federated Clustered and Periodic Aggregation Framework for Hybrid Classical-Quantum Models](https://arxiv.org/abs/2602.03052)
*Yueheng Wang,Xing He,Zinuo Cai,Rui Zhang,Ruhui Ma,Yuan Liu,Rajkumar Buyya*

Main category: cs.LG

TL;DR: FEDCOMPASS是一个用于混合经典-量子联邦学习的层次化聚合框架，通过谱聚类按类别分布相似性分组客户端，采用聚类级聚合处理经典特征提取器，结合循环均值聚合和自适应优化更新量子参数，显著提升非独立同分布数据下的性能。


<details>
  <summary>Details</summary>
Motivation: 混合经典-量子联邦学习虽然能利用量子计算减轻计算和通信负担，但在非独立同分布数据下仍面临性能下降问题。现有方法缺乏有效处理客户端数据分布异质性的机制，需要设计专门针对混合架构的联邦学习框架。

Method: 提出FEDCOMPASS框架：1) 使用谱聚类根据客户端类别分布相似性进行分组；2) 对经典特征提取器执行聚类级聚合；3) 对量子参数采用循环均值聚合结合自适应优化，确保全局更新的稳定性。

Result: 在三个基准数据集上的实验表明，FEDCOMPASS在非独立同分布设置下将测试准确率提升高达10.22%，增强了收敛稳定性，并优于六个强联邦学习基线方法。

Conclusion: FEDCOMPASS通过层次化聚合策略有效解决了混合经典-量子联邦学习在非独立同分布数据下的性能退化问题，为量子增强的联邦学习提供了稳健的解决方案。
...
### [103] [FlashSinkhorn: IO-Aware Entropic Optimal Transport](https://arxiv.org/abs/2602.03067)
*Felix X. -F. Ye,Xingjie Li,An Yu,Ming-Ching Chang,Linsong Chu,Davis Wertheimer*

Main category: cs.LG

TL;DR: FlashSinkhorn：一种基于FlashAttention风格融合和分块的IO感知熵最优传输求解器，通过将Sinkhorn迭代重写为类似Transformer注意力的行归一化操作，大幅减少GPU内存带宽需求，实现高达32倍前向加速和161倍端到端加速。


<details>
  <summary>Details</summary>
Motivation: 现有GPU上的熵最优传输求解器存在效率问题：张量化实现因稠密n×m交互产生二次HBM流量；现有在线后端虽避免存储稠密矩阵，但仍依赖通用分块map-reduce核，融合能力有限。需要开发更高效的GPU求解器。

Method: 将稳定化对数域Sinkhorn更新重写为带偏置点积分数的行级LogSumExp归约，这与Transformer注意力归一化相同。采用FlashAttention风格融合和分块：融合的Triton核通过片上SRAM流式处理分块，在单次遍历中更新对偶势，显著减少每次迭代的HBM IO，同时保持线性内存操作。还提供传输应用的流式核，支持可扩展的一阶和二阶优化。

Result: 在A100 GPU上，FlashSinkhorn在点云最优传输任务上相比最先进的在线基线实现高达32倍前向加速和161倍端到端加速，在基于OT的下游任务上提升可扩展性。开源实现已发布。

Conclusion: 通过将Sinkhorn迭代重新表述为类似注意力的操作并应用FlashAttention风格优化，FlashSinkhorn显著提升了GPU上熵最优传输的计算效率，为大规模机器学习应用提供了实用的高性能求解器。
...
### [104] [Neural Predictor-Corrector: Solving Homotopy Problems with Reinforcement Learning](https://arxiv.org/abs/2602.03086)
*Jiayao Mai,Bangyan Liao,Zhenjun Zhao,Yingping Zeng,Haoang Li,Javier Civera,Tailin Wu,Yi Zhou,Peidong Liu*

Main category: cs.LG

TL;DR: 提出Neural Predictor-Corrector (NPC)框架，用强化学习自动学习预测-校正方法中的启发式策略，实现一次离线训练、在线高效推理的通用同伦问题求解器。


<details>
  <summary>Details</summary>
Motivation: 同伦方法在多个领域有广泛应用，但传统预测-校正求解器依赖手工设计的启发式策略（步长选择、迭代终止等），这些策略通常次优且任务特定，需要更通用、自动化的解决方案。

Method: 1. 将多种同伦问题统一到单一框架下；2. 提出Neural Predictor-Corrector (NPC)，将策略选择建模为序列决策问题；3. 使用强化学习自动发现高效策略；4. 引入摊销训练机制，实现一次离线训练、在线高效推理。

Result: 在四个代表性同伦问题上的实验表明：1. NPC能有效泛化到未见实例；2. 在效率上持续优于经典和专用基线方法；3. 在不同任务间表现出更好的稳定性。

Conclusion: 将同伦方法统一到单一神经框架中具有重要价值，NPC通过自动学习策略取代手工启发式，实现了更高效、更稳定的通用同伦问题求解。
...
### [105] [TextME: Bridging Unseen Modalities Through Text Descriptions](https://arxiv.org/abs/2602.03098)
...
<details>
  <summary>Details</summary>
Motivation: 传统多模态表示扩展依赖于大规模配对数据集（如文本-图像、文本-音频等），这些数据获取成本高，在医学影像和分子分析等需要专家标注的领域往往不可行。需要一种无需配对监督的模态扩展方法。

Method: 提出TextME框架，利用预训练对比编码器的几何结构，将图像、视频、音频、3D、X射线、分子等多种模态投影到LLM嵌入空间作为统一锚点。仅使用文本描述进行训练，无需配对监督，实现零样本跨模态迁移。

Result: 实证验证了跨图像、视频、音频、3D、X射线和分子领域存在一致的模态间隙，仅文本训练能保持预训练编码器的显著性能。框架实现了训练期间未显式对齐的模态对之间的新兴跨模态检索（如音频到图像、3D到图像）。

Conclusion: 仅文本训练可作为配对监督的实用替代方案进行模态扩展，为数据稀缺领域的多模态表示学习提供了新途径。
...
### [106] [Consensus Group Relative Policy Optimization for Text Generation](https://arxiv.org/abs/2602.03102)
*Yuki Ichihara,Yuu Jinnai,Kaito Ariu,Eiji Uchibe*

Main category: cs.LG

TL;DR: C-GRPO是一种通过训练过程将MBR解码蒸馏到模型中的方法，避免了推理时重复采样和评分的计算开销，仅需效用函数和策略样本，无需黄金参考或显式偏好标签。


<details>
  <summary>Details</summary>
Motivation: 现有的采样-重排序解码方法（如MBR）虽然有效，但推理时计算成本高，需要重复采样和评分。先前尝试分摊推理计算的方法通常依赖黄金参考、教师标签或精心策划的偏好数据，增加了数据集构建工作和对高保真奖励模型的需求。

Method: 提出共识组相对策略优化（C-GRPO），通过将共识效用表述为GRPO中的组相对目标，将MBR解码蒸馏到训练中。该方法仅需效用函数和策略样本，无需黄金参考或显式偏好标签。在理想条件下，C-GRPO的目标函数与MBR解码底层期望效用目标的梯度方向对齐，从而提供收敛保证。

Result: 在机器翻译（WMT 2024）和文本摘要（XSum）任务上的实验表明，C-GRPO成功实现了与MBR解码相当的性能，同时避免了相关的推理时间开销，并且优于无参考基线方法。

Conclusion: C-GRPO提供了一种有效的训练时蒸馏方法，将MBR解码的优势整合到模型中，显著减少了推理时的计算负担，同时保持了性能优势，为高效文本生成提供了有前景的解决方案。
...
### [107] [Function-Space Empirical Bayes Regularisation with Large Vision-Language Model Priors](https://arxiv.org/abs/2602.03119)
...
Method: 提出VLM-FS-EB框架：1) 利用大规模视觉语言模型生成语义有意义的上下文点；2) 使用VLMs嵌入构建表达性强的函数空间先验；3) 采用经验贝叶斯正则化方法

Result: 实验表明该方法相比基线方法：1) 持续提升预测性能；2) 产生更可靠的不确定性估计；3) 在分布外检测任务和数据稀缺场景中表现尤其突出

Conclusion: VLM-FS-EB框架通过利用视觉语言模型构建语义丰富的函数空间先验，有效解决了高维贝叶斯深度学习中的先验设计挑战，提升了模型的可靠性和泛化能力
...
### [108] [Quantized Evolution Strategies: High-precision Fine-tuning of Quantized LLMs at Low-precision Cost](https://arxiv.org/abs/2602.03120)
*Yinggan Xu,Risto Miikkulainen,Xin Qiu*

Main category: cs.LG

TL;DR: QES是一种直接在量化空间中执行全参数微调的优化范式，通过误差反馈和种子重放技术，解决了量化LLMs无法使用标准反向传播进行微调的问题。


<details>
  <summary>Details</summary>
Motivation: PTQ使LLMs能在内存受限设备上部署，但量化后的模型参数空间离散且不可微，无法使用依赖反向传播和高精度权重的标准微调方法（包括RL）。虽然进化策略（ES）提供了无反向传播的替代方案，但量化参数优化仍可能因梯度消失或不准确而失败。

Method: 提出量化进化策略（QES），包含两个创新：1）集成累积误差反馈以保持高精度梯度信号；2）使用无状态种子重放将内存使用降至低精度推理水平。QES直接在量化空间中进行全参数微调。

Result: QES在算术推理任务上显著优于最先进的零阶微调方法，使量化模型的直接微调成为可能，为完全在量化空间中扩展LLMs开辟了可能性。

Conclusion: QES通过误差反馈和种子重放技术，成功解决了量化LLMs的微调难题，实现了直接在量化空间中的全参数优化，为量化模型的实际部署和持续改进提供了有效解决方案。
...
### [109] [Contrastive Concept-Tree Search for LLM-Assisted Algorithm Discovery](https://arxiv.org/abs/2602.03132)
...
<details>
  <summary>Details</summary>
Motivation: 尽管LLM辅助算法发现取得了有希望的结果，但如何最大化利用LLM对可能程序空间的内部表示来提升性能仍然是一个开放性问题。现有方法主要依赖算法谱系而非显式的概念层次结构。

Method: CCTS从生成的程序中提取层次化概念表示，学习对比概念模型，通过重加权父代（使用高表现与低表现解决方案之间的似然比分数）来引导搜索，偏向有用的概念组合并远离误导性概念。

Result: CCTS在开放Erdős型组合问题基准测试中，相比基于适应度的基线方法提高了搜索效率，并产生了可解释的、任务特定的概念树。分析表明收益主要来自于学习应避免哪些概念。

Conclusion: CCTS通过显式的概念层次结构而非LLM构建的算法谱系来提供指导，能够有效提升LLM辅助算法发现的搜索效率，并在受控合成环境中验证了其搜索动态的定性再现性。
...
### [110] [SATORIS-N: Spectral Analysis based Traffic Observation Recovery via Informed Subspaces and Nuclear-norm minimization](https://arxiv.org/abs/2602.03138)
*Sampad Mohanty,Bhaskar Krishnamachari*

Main category: cs.LG

TL;DR: SATORIS-N框架利用交通密度矩阵的低秩特性和相邻天之间的稳定相关子空间，通过显式子空间感知的半定规划方法改进部分观测数据的填补，在高缺失率下显著优于传统方法。


<details>
  <summary>Details</summary>
Motivation: 在智能车辆和V2X系统中，基础设施传感器或车辆报告观测数据常因通信中断、传感器遮挡或连接车辆渗透率低而不完整，可靠的交通密度填补对于安全高效的自主导航至关重要。

Method: 提出SATORIS-N框架：1）显式子空间感知半定规划方法，通过核范数公式化明确利用先验奇异子空间信息，联合强制低秩和子空间对齐；2）轻量级隐式子空间对齐策略，通过连接连续天的矩阵鼓励空间或时间奇异方向对齐。

Result: 在两个真实数据集（北京和上海）上，SATORIS-N在高遮挡水平下持续优于标准矩阵填补方法（SoftImpute、IterativeSVD、统计方法甚至深度学习基线）。显式SDP方法在大比例缺失时明显更稳健。
...
### [111] [MemCast: Memory-Driven Time Series Forecasting with Experience-Conditioned Reasoning](https://arxiv.org/abs/2602.03164)
*Xiaoyu Tao,Mingyue Cheng,Ze Guo,Shuo Yu,Yaguo Liu,Qi Liu,Shijin Wang*

Main category: cs.LG

TL;DR: MemCast：一个学习到记忆的框架，将时间序列预测重新定义为经验条件推理任务，通过分层记忆组织经验，实现持续演化
...
Method: 提出MemCast框架，从训练集学习经验并组织为分层记忆：将预测结果总结为历史模式，推理轨迹提炼为推理智慧，提取的时间特征归纳为通用规律；推理时利用历史模式指导推理过程，用推理智慧选择更好的轨迹，通用规律作为反思迭代标准；设计动态置信度适应策略实现持续演化

Result: 在多个数据集上的广泛实验表明，MemCast始终优于先前方法，验证了方法的有效性
...
### [112] [StepScorer: Accelerating Reinforcement Learning with Step-wise Scoring and Psychological Regret Modeling](https://arxiv.org/abs/2602.03171)
*Zhe Xu*

Main category: cs.LG

TL;DR: 本文提出心理遗憾模型（PRM），通过在每个决策步骤后引入基于遗憾的反馈信号，将稀疏奖励转化为密集反馈，加速强化学习收敛速度约36%。
...
Method: 提出心理遗憾模型（PRM），在每个状态计算基于最优行动期望值与实际采取行动值之间差异的遗憾信号，将稀疏奖励转化为密集的步进式反馈框架。

Result: PRM在Lunar Lander等基准环境中比传统PPO算法收敛速度快约36%，在连续控制任务和延迟反馈环境中表现尤为有效。

Conclusion: PRM通过将人类启发的反事实思维形式化为可计算的遗憾信号，连接了行为经济学和强化学习，适用于机器人、金融和自适应教育等需要快速策略适应的实际应用。
...
### [113] [Prompt Augmentation Scales up GRPO Training on Mathematical Reasoning](https://arxiv.org/abs/2602.03190)
*Wenquan Lu,Hai Huang,Randall Balestriero*

Main category: cs.LG

TL;DR: 通过提示增强策略稳定GRPO训练，避免熵崩溃，实现长时间训练，在数学推理任务上达到SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有强化学习方法在数学推理任务中存在熵崩溃现象，导致训练不稳定且只能进行短期训练（5-20轮），限制了策略的持续改进。同时，现有方法通常使用单一固定的推理提示模板，缺乏多样性。

Method: 提出提示增强训练策略，指导模型在多样化的模板和格式下生成推理轨迹，增加rollout多样性。该方法无需KL正则化项，能够在固定数据集上稳定扩展训练时长，使模型能够容忍低熵状态而不发生早期崩溃。

Result: 在MATH Level 3-5数据集上训练的Qwen2.5-Math-1.5B模型实现了最先进的性能：在AIME24、AMC、MATH500、Minerva和OlympiadBench等标准数学推理基准测试中，分别达到44.5%的每基准准确率和51.3%的每问题准确率。

Conclusion: 提示增强策略有效解决了GRPO训练中的熵崩溃问题，实现了稳定的大规模训练，显著提升了数学推理能力，为强化学习在语言模型中的应用提供了新思路。
...
### [114] [Probe-then-Commit Multi-Objective Bandits: Theoretical Benefits of Limited Multi-Arm Feedback](https://arxiv.org/abs/2602.03175)
*Ming Shi*

Main category: cs.LG

TL;DR: 提出PtC-P-UCB算法解决在线资源选择问题，支持探测-提交模式，在多目标优化中实现有限多臂反馈下的帕累托前沿探索


<details>
  <summary>Details</summary>
Motivation: 解决多无线电接入选择和移动边缘计算卸载中的在线资源选择问题，现有多目标学习理论主要关注经典多臂赌博机（q=1）和完全信息专家（q=K）两种极端情况，缺乏对中间有限多臂反馈（1<q<K）的研究

Method: 提出PtC-P-UCB算法，核心是帕累托模式下的前沿感知探测：1）通过最大化超体积启发式前沿覆盖潜力选择q个探测臂；2）通过边际超体积增益提交执行臂以直接扩展获得的帕累托区域；3）扩展到多模态探测，每个探测返回M个模态

Result: 理论证明：1）支配超体积前沿误差为Õ(K_P d/√(qT))，其中K_P是帕累托前沿大小；2）标量化遗憾为Õ(L_φ d√((K/q)T))，其中φ是标量化函数；3）通过不确定性融合实现方差自适应版本

Conclusion: PtC-P-UCB算法在探测-提交模式下实现了1/√q的加速，填补了经典多臂赌博机和完全信息专家之间的理论空白，为多目标在线资源选择提供了有效的解决方案
...
### [115] [Reinforcement Learning with Promising Tokens for Large Language Models](https://arxiv.org/abs/2602.03195)
...
<details>
  <summary>Details</summary>
Motivation: 传统RL方法将LLM视为策略并在完整词汇空间上应用RL，但其中包含大量上下文无关的令牌，这会分散策略对真正合理令牌的决策注意力。

Method: RLPT利用基础模型的语义先验识别动态的"有前景的令牌"集合，通过掩码将策略优化限制在这个精炼的子集上，将战略决策与令牌生成解耦。

Result: RLPT有效降低了梯度方差，稳定了训练过程，提高了样本效率。在数学、编程和电信推理任务上优于标准RL基线，并在不同模型大小（4B和8B）和RL算法（GRPO和DAPO）上有效集成。

Conclusion: RLPT通过将策略优化限制在语义相关的令牌子空间上，解决了LLM强化学习中动作空间过大的问题，为更高效稳定的对齐方法提供了新思路。
...
### [116] [DynSplit-KV: Dynamic Semantic Splitting for KVCache Compression in Efficient Long-Context LLM Inference](https://arxiv.org/abs/2602.03184)
*Jiancai Ye,Jun Liu,Qingchen Li,Tianlang Zhao,Hanbin Zhang,Jiayi Pan,Ningyi Xu,Guohao Dai*

Main category: cs.LG

TL;DR: DynSplit-KV：一种动态语义分割的KV缓存压缩方法，通过重要性感知的分隔符选择和统一映射策略，在长上下文场景中实现高效压缩，相比现有方法显著提升准确性和推理速度。


<details>
  <summary>Details</summary>
Motivation: KV缓存在大语言模型推理中至关重要，但在长上下文场景中其内存占用急剧增长成为瓶颈。现有压缩方法采用固定间隔或预定义分隔符的刚性分割策略，在不同场景下会导致显著的准确性下降（5.5%到55.1%），因为语义边界具有场景依赖性，需要动态语义分割来匹配实际语义。

Method: 提出DynSplit-KV方法：1）动态重要性感知分隔符选择策略：基于语义重要性动态识别分割点，避免语义错位，提升准确性49.9%；2）统一映射策略：将变长语义块转换为固定长度格式，减少73.1%的推理开销，降低推理开销4.9倍。

Result: 实验表明DynSplit-KV在长上下文场景中达到最高准确性，相比FlashAttention实现2.2倍加速，峰值内存减少2.6倍。有效解决了分隔符选择不当导致的28.6%准确性损失和变长块带来的73.1%额外推理开销问题。

Conclusion: DynSplit-KV通过动态语义分割解决了KV缓存压缩中的关键挑战，实现了准确性、推理速度和内存效率的显著提升，为长上下文LLM推理提供了有效的压缩解决方案。
...
### [117] [Lookahead Sample Reward Guidance for Test-Time Scaling of Diffusion Models](https://arxiv.org/abs/2602.03211)
*Yeongmin Kim,Donghyeok Shin,Byeonghu Na,Minsang Park,Richard Lee Kim,Il-Chul Moon*

Main category: cs.LG

TL;DR: 提出LiDAR采样方法，通过前向采样计算期望未来奖励，实现无神经反向传播的闭式指导，显著提升扩散模型与人类意图的对齐效率
...
Method: 提出期望未来奖励(EFR)的新公式，仅使用预训练扩散模型的边际样本计算EFR，解耦神经依赖；引入前向采样收集边际样本；使用精确求解器将粒子引导至高奖励前向样本

Result: LiDAR仅需3个样本和3步前向求解器即实现显著性能提升；达到最新SDXL梯度指导方法的GenEval性能，速度提升9.5倍；性能随前向精度和样本数量增加而急剧提升

Conclusion: 提出的LiDAR采样方法通过高效计算期望未来奖励，实现了无神经反向传播的闭式指导，显著提升了扩散模型与人类意图对齐的效率和性能
...
### [118] [Topology Matters: A Cautionary Case Study of Graph SSL on Neuro-Inspired Benchmarks](https://arxiv.org/abs/2602.03217)
*May Kristine Jonson Carlon,Su Myat Noe,Haojiong Wang,Yasuo Kuniyoshi*

Main category: cs.LG

TL;DR: SSL模型在连接组数据上存在根本性失败，因其拓扑不变性目标与社区结构相冲突，被经典拓扑感知方法显著超越


<details>
  <summary>Details</summary>
Motivation: 研究局部相互作用如何产生全局脑组织需要多尺度信息表示模型，探索通用图自监督学习在连接组类数据上的适用性

Method: 引入分层自监督学习框架，联合学习节点、边和图级嵌入；构建可控合成基准模拟连接组拓扑特性；采用四阶段评估协议

Result: 发现SSL模型存在关键失败：基于不变性的SSL模型与基准拓扑特性根本不对齐，被经典拓扑感知启发式方法灾难性地超越；消融实验证实目标不匹配

Conclusion: 通用图SSL应用于连接组类数据存在根本性缺陷，需要开发新的拓扑感知SSL目标，明确奖励结构保留（如模块化或模体）
...
### [119] [From Scalar Rewards to Potential Trends: Shaping Potential Landscapes for Model-Based Reinforcement Learning](https://arxiv.org/abs/2602.03201)
...
Conclusion: SLOPE通过构建势能景观而非简单回归标量奖励，有效解决了稀疏奖励下MBRL的梯度缺失问题，显著提升了样本效率和性能
...
### [120] [Unveiling Covert Toxicity in Multimodal Data via Toxicity Association Graphs: A Graph-Based Metric and Interpretable Detection Framework](https://arxiv.org/abs/2602.03268)
*Guanzong Wu,Zihao Zhu,Siwei Lyu,Baoyuan Wu*

Main category: cs.LG

TL;DR: 提出基于毒性关联图(TAGs)的多模态毒性检测框架，引入首个可量化的隐蔽毒性指标MTC，构建隐蔽毒性数据集，实现可解释的多模态毒性检测
...
Method: 提出基于毒性关联图(TAGs)的检测框架，系统建模无害实体与潜在毒性含义之间的语义关联。引入多模态毒性隐蔽性(MTC)量化指标，衡量毒性多模态表达的隐蔽程度。构建隐蔽毒性数据集作为评估基准。

Result: 实验表明该方法在低隐蔽性和高隐蔽性毒性检测中均优于现有方法，同时提供清晰、可解释、可审计的检测结果。隐蔽毒性数据集成为首个专门针对高隐蔽性毒性多模态实例的基准。

Conclusion: 该研究推动了可解释多模态毒性检测的技术发展，为未来上下文感知和可解释方法奠定了基础。TAGs框架和MTC指标共同增强了多模态毒性检测的透明度和准确性。
...
### [121] [Spectral Evolution Search: Efficient Inference-Time Scaling for Reward-Aligned Image Generation](https://arxiv.org/abs/2602.03208)
*Jinyan Ye,Zhongjie Duan,Zhiwen Li,Cen Chen,Daoyuan Chen,Yaliang Li,Yingda Chen*

Main category: cs.LG

TL;DR: 提出Spectral Evolution Search (SES)框架，通过低频率子空间进化搜索优化初始噪声，解决视觉生成模型推理时缩放效率低的问题
...
Method: 基于生成动态中的频谱偏差洞察，提出Spectral Evolution Search (SES)框架，在低频率子空间执行无梯度进化搜索，并推导出Spectral Scaling Prediction理论

Result: SES显著推进了生成质量与计算成本的帕累托前沿，在相同计算预算下始终优于强基线方法

Conclusion: 通过识别生成模型对初始扰动的频谱敏感性模式，提出的SES框架为推理时缩放提供了一种高效、即插即用的解决方案
...
### [122] [R1-SyntheticVL: Is Synthetic Data from Generative Models Ready for Multimodal Large Language Model?](https://arxiv.org/abs/2602.03300)
*Jingyi Zhang,Tianyi Lin,Huanjin Yao,Xiang Lan,Shunyu Liu,Jiaxing Huang*

Main category: cs.LG

TL;DR: 提出CADS方法，通过集体对抗数据合成技术自动生成高质量、多样化、具有挑战性的多模态训练数据，以增强多模态大语言模型在复杂现实任务中的表现。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型需要大量高质量训练数据来解决复杂现实任务，但现有数据往往质量不足、多样性有限且缺乏挑战性。需要开发能够自主合成高质量多模态训练数据的技术来有效提升模型性能。

Method: 提出集体对抗数据合成（CADS）方法，包含两个循环阶段：1）集体对抗数据生成（CAD-Generate）利用集体知识联合生成新颖多样的多模态数据；2）集体对抗数据判断（CAD-Judge）协作评估合成数据质量。同时引入对抗上下文优化机制来优化生成上下文，鼓励生成具有挑战性和高价值的数据。

Result: 使用CADS构建了MMSynthetic-20K数据集，并训练了R1-SyntheticVL模型。该模型在各种基准测试中表现出优越性能，验证了CADS方法的有效性。

Conclusion: CADS是一种新颖通用的方法，能够合成高质量、多样化且具有挑战性的多模态数据，有效驱动多模态大语言模型的性能提升。该方法通过集体智能确保生成质量与多样性，通过对抗学习合成挑战性样本，为多模态模型训练提供了有效的数据合成解决方案。
...
### [123] [Periodic Regularized Q-Learning](https://arxiv.org/abs/2602.03301)
...
<details>
  <summary>Details</summary>
Motivation: 传统Q学习在表格设置下收敛性有保证，但在线性函数逼近下收敛性无法保证，需要正则化技术来确保稳定收敛

Method: 提出周期性正则化Q学习算法（PRQ）：1）在投影算子层面引入正则化，构建正则化投影值迭代（RP-VI）；2）将正则化投影扩展到随机设置，建立PRQ算法；3）通过适当正则化投影算子使投影值迭代成为收缩映射

Result: 提供了严格的理论分析，证明了PRQ在线性函数逼近下的有限时间收敛保证

Conclusion: PRQ算法通过正则化投影算子解决了线性函数逼近下Q学习的收敛性问题，为强化学习中的函数逼近提供了理论保证
...
### [124] [Entropy-Gated Selective Policy Optimization:Token-Level Gradient Allocation for Hybrid Training of Large Language Models](https://arxiv.org/abs/2602.03309)
...
<details>
  <summary>Details</summary>
Motivation: 现有混合训练方法在样本级别结合监督微调和强化学习，但缺乏对模型内部不同令牌的差异化处理。高熵令牌需要更多探索，低熵令牌需要减少方差并保护已有知识。

Method: 三阶段框架：1) SFT专家学习建立可靠预热策略；2) RL轨迹生成并计算每个令牌的预测熵；3) EGSPO机制应用熵门控梯度分配：高熵令牌进行完整PPO更新以鼓励探索，低熵令牌进行衰减PPO更新以减少方差并保护知识。

Result: 在数学推理基准测试中取得一致改进：AIME上提升3.8%，MATH上提升2.9%，相比CHORD phi基线仅增加3.4%计算开销。

Conclusion: EGSPO通过令牌级梯度调制扩展了混合训练方法，在保持计算效率的同时显著提升数学推理性能，证明了基于熵的细粒度优化策略的有效性。
...
### [125] [Merging Beyond: Streaming LLM Updates via Activation-Guided Rotations](https://arxiv.org/abs/2602.03237)
...
<details>
  <summary>Details</summary>
Motivation: 现有模型合并技术通常作为后处理改进或专注于减轻任务干扰，往往无法捕捉监督微调（SFT）的动态优化优势。随着大语言模型规模的扩大，需要更高效的适应技术。

Method: 提出Streaming Merging范式，核心是ARM策略：将合并系数视为学习率，从激活子空间推导旋转向量，引导参数沿数据驱动轨迹更新。ARM对齐语义子空间以保持高维参数演化的几何结构，仅需早期SFT检查点进行迭代合并。

Result: 实验在1.7B到14B模型规模和数学、代码等多样领域进行，ARM通过迭代合并超越了完全收敛的SFT模型。ARM提供了一个可扩展且轻量级的高效模型适应框架。

Conclusion: Streaming Merging和ARM策略为高效模型适应提供了新范式，能够仅使用早期检查点就超越完全收敛的SFT模型，展示了模型合并作为迭代优化过程的有效性。
...
TL;DR: 研究发现模型在任务上达到高性能时，鲁棒性自然获得，两者呈强正相关，挑战了将鲁棒性视为独立能力的现有观点


<details>
  <summary>Details</summary>
Motivation: 鲁棒性被认为是实际应用的关键挑战，但当前方法通常将鲁棒性视为独立于任务性能的能力。研究者假设随着模型在任务上性能提升，鲁棒性会自然涌现，而非需要单独优化的独立属性

Method: 通过实证分析多个模型在不同数据集和配置（如释义、不同温度）下的表现，研究任务性能与鲁棒性之间的相关性，考察鲁棒性是由任务特定能力还是模型固有属性驱动

Result: 发现任务性能与鲁棒性之间存在强正相关关系，模型在任务上接近高性能时，鲁棒性实际上已经实现。鲁棒性主要由任务特定能力驱动，而非模型层面的固有属性

Conclusion: 随着新任务性能饱和，模型在这些任务上的鲁棒性将相应涌现。研究者可减少对鲁棒性单独测量的强调，实践者可将模型在简单任务上的可靠性视为实际部署的指标
...
### [127] [Causal Graph Learning via Distributional Invariance of Cause-Effect Relationship](https://arxiv.org/abs/2602.03353)
*Nang Hung Nguyen,Phi Le Nguyen,Thao Nguyen Truong,Trong Nghia Hoang,Masashi Sugiyama*

Main category: cs.LG

TL;DR: 提出基于因果不变性的新框架，通过检验效应-原因条件分布在多个下采样子集中的方差来恢复因果图，算法复杂度为变量数的平方，比现有方法快25倍。


<details>
  <summary>Details</summary>
Motivation: 现有因果发现方法在处理大规模观测数据时面临计算复杂度高的问题，需要更高效的算法来从观测数据中恢复因果图。

Method: 利用效应在给定原因时的分布对原因先验分布变化具有不变性的观察，通过检查效应-原因条件分布在反映不同原因先验分布的数据子集中的方差来直接测试潜在因果关系，结合因果图的稀疏性开发高效算法。

Result: 算法复杂度为观测变量数的平方，处理时间比最先进方法减少达25倍，在大规模数据集基准测试中表现出同等或更优的性能，同时实现更好的可扩展性。

Conclusion: 提出的基于因果不变性的框架能够高效地从观测数据中恢复因果图，在保持性能的同时显著提高了计算效率，为大规模因果发现提供了实用解决方案。
...
### [128] [Beyond Suffixes: Token Position in GCG Adversarial Attacks on Large Language Models](https://arxiv.org/abs/2602.03265)
*Hicham Eddoubi,Umar Faruk Abdullahi,Fadi Hassan*

Main category: cs.LG

TL;DR: 研究发现对抗性标记在提示中的位置（前缀vs后缀）显著影响LLM越狱攻击成功率，揭示了当前安全评估的盲点


<details>
  <summary>Details</summary>
Motivation: 随着LLM在各领域的广泛应用，需要强大的安全对齐机制。然而，通过对抗性提示绕过对齐的越狱攻击对鲁棒性构成挑战，特别是GCG攻击中对抗性标记位置的影响尚未充分探索

Method: 以GCG攻击为案例研究，优化攻击以生成前缀而非后缀，并在评估过程中变化对抗性标记的位置，分析其对攻击成功率的影响

Result: 对抗性标记的位置（前缀与后缀）对攻击成功率有显著影响，揭示了当前安全评估中对抗性标记位置因素的盲点

Conclusion: 在LLM对抗鲁棒性评估中需要考虑对抗性标记的位置因素，这对改进安全评估方法具有重要意义
...
TL;DR: MeKi提出了一种基于存储空间而非计算量扩展LLM容量的新范式，通过为Transformer层配备token级记忆专家，将知识存储在ROM中，实现零推理延迟开销的模型扩展


<details>
  <summary>Details</summary>
Motivation: 传统LLM扩展依赖增加参数数量或测试时计算量，但这在边缘设备上不实用，因为RAM和NPU资源有限。然而，在智能手机等边缘设备上部署高性能LLM对用户体验至关重要，需要解决硬件约束下的模型扩展问题

Method: 提出MeKi系统，为每个Transformer层配备token级记忆专家，注入预存储的语义知识；采用重参数化策略将训练期间使用的参数矩阵折叠成紧凑的静态查找表；将知识卸载到ROM中，解耦模型容量与计算成本

Result: 大量实验表明，MeKi在相同推理速度下显著优于密集LLM基线，验证了基于内存的扩展范式在设备端LLM上的有效性

Conclusion: MeKi通过存储空间而非FLOPs扩展LLM容量，为边缘设备部署提供了实用的解决方案，实现了模型容量与计算成本的解耦，为零延迟开销的模型扩展提供了新范式
...
TL;DR: 论文发现语法相似性是良性再学习的主要驱动因素，并提出语法多样化方法有效抑制遗忘信息的重新出现


<details>
  <summary>Details</summary>
Motivation: 现有机器遗忘方法存在根本性脆弱性，即使从良性微调数据中，被遗忘的信息也会重新出现（良性再学习现象），而传统基于主题相关性的解释不够充分

Method: 通过系统分析确定语法相似性是主要驱动因素，并提出语法多样化方法：在遗忘前将原始遗忘查询改写成异构结构

Result: 语法多样化有效抑制良性再学习，加速遗忘过程，并显著缓解遗忘效果与模型效用之间的权衡

Conclusion: 语法相似性而非主题相关性是良性再学习的主要驱动因素，语法多样化方法为构建更鲁棒的机器遗忘系统提供了有效途径
...
### [131] [BlockRR: A Unified Framework of RR-type Algorithms for Label Differential Privacy](https://arxiv.org/abs/2602.03277)
*Haixia Liu,Yi Ding*

Main category: cs.LG

TL;DR: BlockRR是一种新颖的统一随机响应机制，用于标签差分隐私，将现有RR型机制推广为特定参数设置下的特例，无需单独案例分析。


<details>
  <summary>Details</summary>
Motivation: 现有标签差分隐私中的随机响应机制需要针对不同情况进行单独分析，缺乏统一的框架。BlockRR旨在提供一个通用机制，简化分析并提高效率。

Method: BlockRR是一个统一的随机响应机制框架，基于权重矩阵设计分区方法，利用并行组合原理确保组合机制仍满足ε-标签差分隐私。

Result: 理论证明BlockRR满足ε-标签差分隐私。在CIFAR-10变体数据集上的实验表明，在高隐私和中度隐私区域（ε≤3.0），BlockRR在测试准确率和每类平均准确率之间取得更好平衡；在低隐私区域（ε≥4.0），所有方法都退化为标准RR而无额外性能损失。

Conclusion: BlockRR提供了一个统一的标签差分隐私框架，简化了机制设计，在隐私保护与分类性能之间实现了更好的权衡，特别是在高隐私和中度隐私区域。
...
### [132] [Universal Approximation of Continuous Functionals on Compact Subsets via Linear Measurements and Scalar Nonlinearities](https://arxiv.org/abs/2602.03290)
*Andrey Krylov,Maksim Penkin*

Main category: cs.LG

TL;DR: 该论文证明了在希尔伯特空间乘积的紧子集上，连续泛函可以通过"先进行有限个连续线性测量，再通过连续标量非线性组合"的模型来一致逼近，为算子学习和成像中的常见设计模式提供了紧集上的理论依据。


<details>
  <summary>Details</summary>
Motivation: 为算子学习和成像领域中广泛使用的"测量-应用标量非线性-组合"设计模式提供理论依据，特别是在希尔伯特空间乘积的紧子集上建立泛函逼近的数学基础。

Method: 采用函数逼近理论方法，首先证明在希尔伯特空间乘积的紧子集上，任何连续泛函都可以通过有限个连续线性测量与连续标量非线性组合的模型来一致逼近。然后将该逼近原理扩展到取值于巴拿赫空间的映射，得到有限秩逼近。

Result: 证明了在希尔伯特空间乘积的紧子集上，连续泛函可以通过"先进行有限个连续线性测量，再通过连续标量非线性组合"的模型实现一致逼近。该结果也适用于取值于巴拿赫空间的映射，得到有限秩逼近。

Conclusion: 该研究为算子学习和成像中常用的"测量-应用标量非线性-组合"设计模式提供了紧集上的数学理论支撑，证明了这种结构在希尔伯特空间乘积紧子集上具有通用逼近能力。
...
### [133] [Anomaly Detection via Mean Shift Density Enhancement](https://arxiv.org/abs/2602.03293)
*Pritam Kar,Rahul Bordoloi,Olaf Wolkenhauer,Saptarshi Bej*

Main category: cs.LG

TL;DR: 提出MSDE框架，通过密度驱动流形演化中的几何响应检测异常，在ADBench基准测试中表现优于13个基线方法


<details>
  <summary>Details</summary>
Motivation: 现有无监督异常检测算法在不同异常类型上表现不一致，在特定结构假设下表现良好，但在噪声环境下鲁棒性不足，需要一种能跨多种异常类型保持稳定性能的方法

Method: 提出Mean Shift Density Enhancement (MSDE)框架，基于密度增强流形演化原理：正常样本在局部密度支持下保持稳定，而异常样本在迭代密度增强中向附近密度模式移动。使用加权均值漂移过程，基于UMAP模糊邻域图计算样本特定密度权重，通过少量迭代中的累积位移定义异常分数

Result: 在ADBench基准测试（46个真实世界表格数据集，4种异常生成机制，6个噪声水平）中，与13个无监督基线相比，MSDE在AUC-ROC、AUC-PR和Precision@n指标上均表现出一致、平衡且鲁棒的性能，在不同噪声水平和异常类型上平均表现优异

Conclusion: 基于位移的评分方法为无监督异常检测提供了鲁棒的替代方案，能够跨多种异常类型和噪声环境保持稳定性能
...
### [134] [On the Entropy Dynamics in Reinforcement Fine-Tuning of Large Language Models](https://arxiv.org/abs/2602.03392)
*Shumin Wang,Yuexiang Xie,Wenhao Zhang,Yuchang Sun,Yanxi Chen,Yaliang Li,Yanyong Zhang*

Main category: cs.LG

TL;DR: 该论文建立了强化微调过程中熵动态的理论框架，推导了熵变化的判别表达式，并基于此设计了熵控制方法，为LLM微调中的探索-利用平衡提供了理论支持和实践策略。


<details>
  <summary>Details</summary>
Motivation: 熵作为衡量大语言模型输出多样性的关键指标，对理解其探索能力至关重要。虽然现有研究关注在强化微调中监控和调整熵以平衡探索和利用，但对该过程中熵动态的原理性理解尚未得到深入研究。本文旨在填补这一理论空白。

Method: 建立分析强化微调过程中熵动态的理论框架：1）从量化单个logit更新下熵变化的判别表达式出发；2）推导熵变化的一阶表达式；3）扩展到Group Relative Policy Optimization的更新公式；4）基于理论分析设计熵控制方法；5）提供实证证据支持分析结论。

Result: 理论分析得出了熵变化的判别表达式和一阶表达式，能够扩展到GRPO更新公式。基于理论洞察设计了熵控制方法，特别是熵判别器裁剪方法。实证结果支持了主要分析结论，并证明了所提方法的有效性。

Conclusion: 本研究为强化微调训练动态提供了新的理论见解，为优化LLM微调中的探索-利用平衡提供了理论支持和实用策略。理论框架为解释现有研究中各种基于熵的方法提供了统一视角，并启发了新的熵控制方法设计。
...
### [135] [Beyond Variance: Prompt-Efficient RLVR via Rare-Event Amplification and Bidirectional Pairing](https://arxiv.org/abs/2602.03452)
*Xin Sheng,Jiaxin Li,Yujuan Pang,Ran Peng,Yong Ma*

Main category: cs.LG

TL;DR: 本文提出了一种名为"正负配对"的提示选择方法，结合加权GRPO算法，通过同时提供可靠的正向锚点和显式的负向学习信号，显著提升了强化学习在确定性结果推理任务中的样本效率和性能。


<details>
  <summary>Details</summary>
Motivation: 现有基于验证奖励的强化学习（RLVR）在提示选择上主要依赖训练准确率的方差，这导致优化方向不稳定且迁移能力较弱。作者从机制层面重新审视提示选择，认为有效的minibatch应同时提供可靠的正向锚点和显式的负向学习信号。

Method: 提出"正负配对"方法：每次更新时采样一个难但可解的提示q⁺（低成功率）和一个易但脆弱的提示q⁻（高成功率但不完美）。进一步引入加权GRPO算法，在配对层面重新加权二元结果，使用组归一化优势来放大q⁺上的罕见成功为明确的正向指导，同时将q⁻上的罕见失败转化为强负向惩罚。

Result: 在Qwen2.5-Math-7B模型上，每个更新使用单个配对minibatch持续优于基于方差选择启发式的GRPO基线：AIME 2025 Pass@8从16.8提升至22.2，AMC23 Pass@64从94.0提升至97.0，同时与从1209个训练提示池训练的大规模RLVR保持竞争力。在Qwen2.5-Math-7B-Instruct模型上也观察到类似提升。

Conclusion: 通过正负配对和加权GRPO的双向信号机制，能够为成功和失败提供信息丰富的学习反馈，在不抑制探索的情况下提高样本效率，为强化学习在确定性推理任务中的提示选择提供了更有效的策略。
...
TL;DR: scDiVa：基于掩码离散扩散的scRNA-seq基础模型，通过双向去噪器联合建模离散基因身份和连续值，避免自回归方法的人工排序偏差


<details>
  <summary>Details</summary>
Motivation: 单细胞RNA-seq数据具有高维、稀疏、无序的特性，自回归生成方法会引入人工排序偏差并遭受误差累积问题，需要更符合生物学特性的生成方法

Method: 提出掩码离散扩散基础模型scDiVa，在标记空间定义连续时间前向掩码机制，使用双向去噪器联合建模离散基因身份和连续值，采用熵归一化序列化和潜在锚点标记最大化信息效率，通过深度不变时间采样和双重去噪目标进行训练

Result: 在5900万个细胞上预训练后，scDiVa在批次整合、细胞类型注释和扰动响应预测等主要基准测试中展现出强大的迁移性能

Conclusion: 掩码离散扩散是自回归方法的生物学一致且有效的替代方案，能够更好地处理scRNA-seq数据的特性
...
### [137] [medR: Reward Engineering for Clinical Offline Reinforcement Learning via Tri-Drive Potential Functions](https://arxiv.org/abs/2602.03305)
...
<details>
  <summary>Details</summary>
Motivation: 临床强化学习面临奖励工程瓶颈：需要定义在复杂、稀疏的离线环境中安全有效指导策略学习的信号。现有方法依赖手动启发式规则，难以在不同病理间泛化。

Method: 提出自动化流水线，利用大语言模型进行离线奖励设计和验证。奖励函数采用由生存、置信度和能力三个核心组件组成的势函数形式。引入定量指标在部署前严格评估和选择最优奖励结构。

Result: 通过集成LLM驱动的领域知识，该框架能够自动化设计针对特定疾病的奖励函数，同时显著提升最终策略的性能。

Conclusion: 该研究解决了临床强化学习中的奖励工程挑战，通过LLM驱动的自动化奖励设计框架，为动态治疗方案优化提供了更安全有效的解决方案。
...
TL;DR: RFO提出了一种基于流策略的重新参数化策略梯度方法，通过联合反向传播流生成过程和系统动力学来计算策略梯度，在保持高样本效率的同时避免了难以处理的对数似然计算。


<details>
  <summary>Details</summary>
Motivation: 现有的重新参数化策略梯度方法主要局限于高斯策略，限制了性能提升，未能充分利用生成模型的最新进展。流策略通过可微ODE积分生成动作，与RPG框架自然契合，但直接利用这种协同效应会导致训练不稳定和探索不足。

Method: 提出了重新参数化流策略优化方法，通过联合反向传播流生成过程和系统动力学来计算策略梯度，避免了难以处理的对数似然计算。包含两个专门设计的正则化项以确保稳定性和探索性，并提出了带有动作分块的变体。

Result: 在多种运动和控制任务上进行了广泛实验，包括刚体和软体控制，涉及状态和视觉输入。在控制软体四足机器人的挑战性运动任务中，RFO实现了接近最先进基线2倍的奖励。

Conclusion: RFO成功地将流策略与重新参数化策略梯度框架相结合，通过专门的稳定性和探索正则化，在保持高样本效率的同时显著提升了性能，特别是在复杂软体控制任务中表现出色。
...
TL;DR: 该研究首次将机制可解释性应用于符号回归变换器，开发了PATCHES算法来发现紧凑正确的电路，并通过因果评估框架验证了28个电路，为符号回归的机制理解提供了新方法。
...
Method: 提出了PATCHES算法，这是一种进化电路发现算法，用于识别符号回归变换器中的紧凑正确电路。采用基于忠实性、完整性和最小性等关键概念的因果评估框架来验证发现结果。

Result: 成功分离出28个电路，首次对符号回归变换器进行了电路级表征。发现平均修补结合基于性能的评估最能可靠地分离功能正确的电路，而直接对数归因和探测分类器主要捕获相关性特征而非因果特征。

Conclusion: 该研究确立了符号回归作为机制可解释性的高潜力应用领域，并为电路发现提供了原则性方法论，揭示了符号回归变换器的内部工作机制。
...
### [140] [Mitigating Staleness in Asynchronous Pipeline Parallelism via Basis Rotation](https://arxiv.org/abs/2602.03515)
*Hyunji Jung,Sungbin Shin,Namhoon Lee*

Main category: cs.LG

TL;DR: 异步流水线并行通过消除同步执行中的流水线气泡来最大化硬件利用率，但梯度延迟会引入优化噪声，且延迟随流水线深度线性增长，破坏了方法的可扩展性。本文通过基旋转修正延迟梯度，恢复可扩展的异步训练。


<details>
  <summary>Details</summary>
Motivation: 异步流水线并行虽然能提高硬件利用率，但存在梯度延迟问题，且延迟随流水线深度线性增长，这破坏了该方法原本旨在提供的可扩展性优势。梯度延迟会引入优化噪声，影响训练效果。

Method: 提出使用基旋转（basis rotation）来修正延迟梯度。研究发现，当Hessian特征基与标准坐标基不对齐时，梯度延迟的有害效应会被放大，导致Adam等坐标自适应方案无法有效利用曲率感知的自适应性。基旋转可以缓解这种对齐问题。

Result: 通过理论分析和实证评估验证了基旋转方法的有效性。在训练1B参数的LLM时，使用基旋转的方法相比最佳异步流水线并行基线，达到相同训练损失所需的迭代次数减少了76.8%。

Conclusion: 基旋转能够有效缓解异步流水线并行中的梯度延迟问题，恢复其可扩展性优势，同时保持训练性能。该方法通过解决Hessian特征基与坐标基的对齐问题，显著加速了异步设置下的收敛速度。
...
### [141] [From Inexact Gradients to Byzantine Robustness: Acceleration and Optimization under Similarity](https://arxiv.org/abs/2602.03329)
...
<details>
  <summary>Details</summary>
Motivation: 标准联邦学习算法容易受到拜占庭节点攻击，现有鲁棒分布式学习算法通常用鲁棒聚合代替参数平均，但分析较为临时性，阻碍了更复杂鲁棒算法（如加速算法）的发展。

Method: 1) 将拜占庭鲁棒分布式优化重新表述为具有不精确梯度的一般优化问题；2) 提出两种加速方案：基于Nesterov型加速方案（源自加速不精确梯度结果）和基于相似性优化的方案（服务器利用近似全局损失的辅助损失函数）。

Result: 理论证明标准鲁棒聚合上的梯度下降在拜占庭设置中获得最优渐近误差；两种加速方案相比先前方法显著降低通信复杂度，理论和实验均验证了这一点。

Conclusion: 通过将拜占庭鲁棒分布式优化重新表述为不精确梯度优化问题，为开发更复杂鲁棒算法提供了统一框架，提出的加速方案有效降低了通信复杂度。
...
Method: PNS通过反向强化学习训练专用模型，使用格式合规性、准确性反转、奖励模型评估和思维链评估的复合奖励，合成具有预期格式和结构连贯性但最终给出错误答案的高质量负样本

Result: 在七个数学推理基准测试中，PNS作为即插即用数据源在三个骨干模型上持续优于其他负样本合成方法，平均比RL训练模型提升2.03%
...
### [143] [Bayesian Conformal Prediction as a Decision Risk Problem](https://arxiv.org/abs/2602.03331)
*Fanyi Wu,Veronika Lohmanova,Samuel Kaski,Michele Caprio*

Main category: cs.LG

TL;DR: BCP使用贝叶斯后验预测密度作为非共形分数，结合贝叶斯积分估计和最小化预期预测集大小，在分割共形框架下提供有效覆盖保证，并在模型错误设定下表现出可靠的实证覆盖。


<details>
  <summary>Details</summary>
Motivation: 解决传统共形预测方法在模型错误设定下覆盖性能不稳定的问题，特别是降低预测集大小的运行间变异性，同时保持有效的覆盖保证。

Method: 将贝叶斯后验预测密度作为非共形分数，使用贝叶斯积分估计预期预测集大小并最小化，在分割共形预测框架下操作。

Result: 在回归和分类任务中，包括ImageNet-A等分布偏移场景，BCP产生与分割共形预测相当大小的预测集，但运行间变异性显著降低。在稀疏回归中，名义覆盖80%时，BCP在错误设定先验下实现81%实证覆盖，而贝叶斯可信区间仅覆盖49%。

Conclusion: BCP方法在保持有效覆盖保证的同时，显著降低了预测集大小的运行间变异性，在模型错误设定下表现出比传统贝叶斯方法更可靠的覆盖性能。
...
### [144] [Live or Lie: Action-Aware Capsule Multiple Instance Learning for Risk Assessment in Live Streaming Platforms](https://arxiv.org/abs/2602.03520)
*Yiran Qiao,Jing Chen,Xiang Ao,Qiwei Zhong,Yang Liu,Qing He*

Main category: cs.LG

TL;DR: AC-MIL：基于动作感知胶囊多示例学习框架，用于弱监督下的直播房间风险评估，通过结构化用户-时间槽胶囊建模个体行为与群体协调模式，实现房间级风险预测和行为段级可解释性。


<details>
  <summary>Details</summary>
Motivation: 直播已成为互联网社交的重要形式，但面临稀疏、协调的恶意行为风险。这些行为隐藏在正常活动中，难以实时准确检测。现有方法面临弱监督挑战，只有房间级标签可用，需要同时建模个体行为和群体协调模式。

Method: 将任务形式化为多示例学习问题：房间作为包，结构化用户-时间槽胶囊作为实例。提出AC-MIL框架，通过串并行架构联合编码时间动态性和跨用户依赖关系，捕捉多粒度语义和行为线索。胶囊表示特定时间窗口内的用户行为子序列，封装局部行为模式。

Result: 在抖音大规模工业数据集上的实验表明，AC-MIL显著优于多示例学习和序列基线方法，在直播房间风险评估中建立了新的最先进性能。同时提供胶囊级可解释性，能够识别风险行为段作为干预的可操作证据。

Conclusion: AC-MIL为弱监督下的直播风险评估提供了有效解决方案，通过多示例学习框架和胶囊表示，实现了准确的房间级风险预测和行为段级可解释性，为实时干预提供了可操作证据。
...
TL;DR: 提出新的单步逆合成基准框架，使用ChemCensor评估化学合理性，并引入CREED数据集用于LLM训练


<details>
  <summary>Details</summary>
Motivation: 现有逆合成评估方法依赖已发表的合成程序和基于单一真实结果的Top-K准确率，无法捕捉真实世界合成规划的开放性本质，需要更符合人类合成规划实践的评估框架

Method: 提出新的单步逆合成基准框架，使用ChemCensor（化学合理性度量）评估通用和专业化学LLM，强调合理性而非精确匹配；引入CREED数据集（数百万个ChemCensor验证的反应记录）用于LLM训练

Result: 使用CREED数据集训练的模型在基准测试中优于LLM基线，新框架能更好地评估逆合成性能

Conclusion: 提出的基准框架和ChemCensor度量能更准确地评估逆合成性能，CREED数据集有效提升了LLM在逆合成任务上的表现，为药物发现中的合成规划提供了更好的评估工具
...
TL;DR: 提出EVE方法，无需参与模型初始训练即可验证机器遗忘，通过扰动遗忘数据使模型预测发生变化作为验证信号


<details>
  <summary>Details</summary>
Motivation: 机器遗忘验证至关重要但研究不足，现有基于后门的方法需要参与初始训练，效率低且不实用
...
Conclusion: EVE为机器遗忘验证提供了一种高效实用的新工具，解决了现有方法需要参与初始训练的局限性
...
### [147] [Achieving Linear Speedup for Composite Federated Learning](https://arxiv.org/abs/2602.03357)
*Kun Huang,Shi Pu*

Main category: cs.LG

TL;DR: FedNMap：一种基于法向映射的复合联邦学习方法，通过法向映射更新方案处理非光滑正则项，并采用局部校正策略缓解客户端数据异构性影响，在非凸损失下实现了关于客户端数量和本地更新次数的线性加速。


<details>
  <summary>Details</summary>
Motivation: 复合联邦学习中的目标函数通常包含光滑损失项和可能非光滑的正则项，现有方法在处理非光滑项时存在挑战。同时，客户端间的数据异构性会严重影响联邦学习的收敛性能。需要开发一种能够有效处理非光滑正则项并缓解数据异构性影响的高效联邦学习算法。

Method: FedNMap采用法向映射（normal map）更新方案来处理非光滑正则项，通过将非光滑优化问题转化为光滑子问题。算法结合了局部校正策略来减轻客户端数据异构性的影响，在标准假设下（包括局部损失的光滑性、正则项的弱凸性、有界随机梯度方差）进行分析。

Result: FedNMap在非凸损失下实现了关于客户端数量n和本地更新次数Q的线性加速，无论是否满足Polyak-Łojasiewicz（PL）条件。据作者所知，这是首次在非凸复合联邦学习中建立线性加速的结果。

Conclusion: FedNMap是第一个在非凸复合联邦学习中实现线性加速的方法，通过法向映射方案有效处理非光滑正则项，并通过局部校正策略缓解数据异构性影响，为复合联邦学习提供了理论保证和实用算法。
...
### [148] [ContraLog: Log File Anomaly Detection with Contrastive Learning and Masked Language Modeling](https://arxiv.org/abs/2602.03678)
*Simon Dietz,Kai Klede,An Nguyen,Bjoern M Eskofier*

Main category: cs.LG

TL;DR: ContraLog：一种无需解析器的自监督日志异常检测方法，通过预测连续消息嵌入而非离散模板ID，结合掩码语言建模和对比学习，在多个基准数据集上表现有效。


<details>
  <summary>Details</summary>
Motivation: 现有日志异常检测方法大多依赖日志解析器，将消息折叠为离散模板，丢弃变量值和语义内容。这限制了检测能力，特别是在处理复杂多样的日志消息时。

Method: ContraLog采用无解析器、自监督的方法，将日志异常检测重构为预测连续消息嵌入而非离散模板ID。方法包含：1) 消息编码器为单个日志消息生成丰富嵌入；2) 序列编码器建模序列内的时间依赖关系；3) 结合掩码语言建模和对比学习进行训练，基于上下文预测被掩码的消息嵌入。

Result: 在HDFS、BGL和Thunderbird基准数据集上的实验表明，该方法在具有多样化日志消息的复杂数据集上有效。此外，ContraLog生成的消息嵌入携带有意义信息，即使没有序列上下文也能预测异常。

Conclusion: 嵌入级预测是日志异常检测的有效方法，具有应用于其他事件序列的潜力。ContraLog避免了传统解析器的局限性，保留了语义信息，为日志分析提供了新视角。
...
### [149] [Dynamic Topology Optimization for Non-IID Data in Decentralized Learning](https://arxiv.org/abs/2602.03383)
...
<details>
  <summary>Details</summary>
Motivation: 去中心化学习在非独立同分布数据分布和静态通信拓扑下难以训练高精度模型，需要解决数据异构性和拓扑优化问题。

Method: Morph采用基于最大模型差异的自适应对等节点选择，通过gossip-based peer discovery和diversity-driven neighbor selection动态重塑通信图，保持固定入度。

Result: 在CIFAR-10和FEMNIST上最多100个节点的实验中，Morph始终优于静态和epidemic基线，接近全连接上限。CIFAR-10上相对改进1.12x，FEMNIST上比Epidemic Learning高1.08x。50节点部署中，与全连接上限差距缩小到0.5个百分点内。
...
### [150] [QuAIL: Quality-Aware Inertial Learning for Robust Training under Data Corruption](https://arxiv.org/abs/2602.03686)
...
Method: QuAIL通过为现有模型添加可学习的特征调制层，该层的更新受到质量依赖的近端正则化器的选择性约束，从而诱导在不同可信度特征间的受控适应。这种方法在不进行显式数据修复或样本级重新加权的情况下，稳定了结构化损坏下的优化过程。

Result: 在50个分类和回归数据集上的实证评估表明，QuAIL在随机和值依赖损坏下持续改进神经基线的平均性能，在低数据和系统性偏差设置中表现出特别鲁棒的行为。

Conclusion: 将特征可靠性信息直接融入优化动态是构建弹性表格学习的一种实用有效方法，能够在不依赖显式数据清洗的情况下提升模型在损坏数据上的鲁棒性。
...
### [151] [LLM-Inspired Pretrain-Then-Finetune for Small-Data, Large-Scale Optimization](https://arxiv.org/abs/2602.03690)
...
<details>
  <summary>Details</summary>
Motivation: 解决企业在面对大规模产品组合等场景时，每个实例只有少量噪声数据的小数据大规模决策问题。传统方法难以从有限数据中学习有效决策策略。

Method: 设计基于Transformer的预训练-微调框架：1）在包含领域知识和决策环境结构特征的大规模合成数据上预训练；2）在实际观测数据上微调。采用问题特定的架构设计和定制化训练流程。

Result: 理论分析建立了Transformer学习的首个全面误差分析，证明了方法的有效性。分析显示预训练和微调共同决定性能，其中微调具有规模经济效应，即随着实例数量增加，迁移学习效果增强。

Conclusion: 提出的预训练-微调方法结合了Transformer的强大表示能力和领域知识注入，有效解决了小数据大规模决策问题，为运营决策提供了新的机器学习框架。
...
### [152] [Reward Redistribution for CVaR MDPs using a Bellman Operator on L-infinity](https://arxiv.org/abs/2602.03778)
*Aneri Muni,Vincent Taboga,Esther Derman,Pierre-Luc Bacon,Erick Delage*

Main category: cs.LG

TL;DR: 提出一种新的静态CVaR目标函数公式，通过状态增强实现具有密集奖励和收缩性质的Bellman算子，开发了风险厌恶值迭代和Q学习算法。
...
Method: 提出基于状态增强的新静态CVaR目标函数公式，构建具有密集每步奖励和全空间收缩性质的Bellman算子，开发风险厌恶值迭代和模型无关Q学习算法，使用离散化增强状态。

Result: 算法成功学习CVaR敏感策略，实现有效的性能-安全权衡，提供了收敛保证和离散化引起的近似误差界限。

Conclusion: 新公式解决了传统CVaR增强方法的稀疏奖励和退化固定点问题，为风险厌恶强化学习提供了理论基础和实用算法。
...
### [153] [Bridging Online and Offline RL: Contextual Bandit Learning for Multi-Turn Code Generation](https://arxiv.org/abs/2602.03806)
*Ziru Chen,Dongdong Chen,Ruinan Jin,Yingbin Liang,Yujia Xie,Huan Sun*

Main category: cs.LG

TL;DR: Cobalt是一种结合在线和离线RL优势的新方法，用于多轮代码生成任务，通过上下文赌博机学习和离线轨迹实现高效训练


<details>
  <summary>Details</summary>
Motivation: 虽然在线强化学习在多轮代码生成等现实任务中表现优于离线RL，但其高训练成本和不稳定性限制了广泛应用。本文旨在开发一种结合两者优势的高效稳定方法。

Method: Cobalt将多轮代码生成建模为一步可恢复马尔可夫决策过程，首先使用参考LLM收集代码生成轨迹并分割为部分轨迹作为上下文提示，然后在在线赌博机学习中训练LLM通过单步代码生成完成每个部分轨迹提示。

Result: Cobalt在LiveCodeBench上显著优于基于GRPO和VeRPO的多轮在线RL基线，将R1-Distill 8B和Qwen3 8B的Pass@1分数分别提升高达9.0和6.2个绝对百分点。通过分析上下文奖励攻击行为并添加扰动轨迹缓解该问题。

Conclusion: Cobalt是多轮代码生成等迭代决策任务的有前景解决方案，结合了在线和离线RL的优势，实现了高效稳定的训练效果。
...
### [154] [Least but not Last: Fine-tuning Intermediate Principal Components for Better Performance-Forgetting Trade-Offs](https://arxiv.org/abs/2602.03493)
*Alessio Quercia,Arya Bangun,Ira Assent,Hanno Scharr*

Main category: cs.LG

TL;DR: 该论文分析了低秩适应方法中性能与遗忘的权衡问题，提出使用中间主成分初始化LoRA的方法，相比现有方法能更好地平衡任务性能与预训练知识保留。


<details>
  <summary>Details</summary>
Motivation: LoRA方法在计算和内存受限条件下适配大预训练模型时面临基本挑战：平衡任务特定性能提升与预训练知识的灾难性遗忘。现有方法对此提供不一致的建议，需要系统分析性能-遗忘权衡。

Method: 使用主成分分析作为初始化基础，系统研究不同组件（首部、中间、尾部）对性能-遗忘权衡的影响。发现微调中间组件比现有方法（PiSSA的首部组件和MiLoRA的尾部组件）能获得更好的平衡，并对高学习率更鲁棒。基于此提出实用的LoRA初始化方法。

Result: 在多种计算机视觉和NLP任务上的实证研究表明，该方法提高了准确性并减少了遗忘，在持续学习场景中也表现良好。相比现有方法，中间组件初始化在性能-遗忘权衡方面表现更优。

Conclusion: 通过系统分析低秩适应中的性能-遗忘权衡，发现中间主成分初始化LoRA能提供更优的权衡效果。该方法为实际应用提供了有效的初始化策略，在保持预训练知识的同时提升下游任务性能。
...
### [155] [MatGPTQ: Accurate and Efficient Post-Training Matryoshka Quantization](https://arxiv.org/abs/2602.03537)
*Maximilian Kleinegger,Elvir Crnčević,Dan Alistarh*

Main category: cs.LG

TL;DR: MatGPTQ是一种后训练量化方法，通过单次处理生成支持多精度切片的模型，解决了Matryoshka量化需要昂贵训练和缺乏开源支持的问题。


<details>
  <summary>Details</summary>
Motivation: 现有的Matryoshka量化方法依赖昂贵的量化感知训练，缺乏开源和内核支持，使得单检查点多精度部署在实际应用中面临挑战。

Method: 提出后训练Matryoshka量化(MatGPTQ)管道，将量化建模为多精度目标，结合比特切片和跨比特误差补偿，通过单次处理生成可切片的模型，并包含预算感知的逐层比特宽度搜索和高效内核实现。

Result: 在标准LLM和基准测试中，MatGPTQ保持了高比特精度，同时显著提升了低比特宽度设置的性能，为Matryoshka风格的后训练量化建立了新的SOTA。

Conclusion: MatGPTQ使单检查点多精度部署变得开放实用，解决了Matryoshka量化的关键限制，为实际部署提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有时间序列预测方法存在局限性：自回归LLM擅长语义上下文建模但无法充分建模连续概率生成，扩散类模型擅长连续概率生成但语义理解不足。需要一种能同时建模这两个方面的统一框架。

Method: 1) 将预训练的解码器LLM重构为原生预测编码器-解码器骨干，仅修改注意力拓扑结构，实现双向上下文编码和因果表示生成；2) 集成流匹配机制建模时间演化，在自回归生成表示的基础上捕获连续随机动态；3) 支持多模态预测和跨域统一训练。

Result: 在多个基准测试上的广泛实验表明，CoGenCast始终优于先前比较的基线方法。代码已开源。
...
### [157] [Asymmetric Hierarchical Anchoring for Audio-Visual Joint Representation: Resolving Information Allocation Ambiguity for Robust Cross-Modal Generalization](https://arxiv.org/abs/2602.03570)
...
<details>
  <summary>Details</summary>
Motivation: 现有对称框架在跨模态泛化中存在信息分配模糊问题，缺乏结构性归纳偏置导致语义特定信息在模态间泄漏，需要更有效的跨模态知识迁移方法。

Method: 提出非对称层次锚定(AHA)：1)利用音频残差向量量化(RVQ)的层次离散表示作为结构化语义锚点；2)使用GRL对抗解耦器抑制模态特定分支中的语义泄漏；3)引入局部滑动对齐(LSA)促进细粒度时间对齐。

Result: 在AVE和AVVP基准测试中，AHA持续优于对称基线方法；说话人脸解缠实验进一步验证了学习到的表示具有更好的语义一致性和解缠性。

Conclusion: AHA框架通过非对称层次锚定有效解决了跨模态泛化中的信息分配模糊问题，实现了更优的跨模态知识迁移，具有更广泛的适用性。
...
### [158] [Optimization and Generation in Aerodynamics Inverse Design](https://arxiv.org/abs/2602.03582)
*Huaguan Chen,Ning Lin,Luxi Chen,Rui Zhang,Wenbing Huang,Chongxuan Li,Hao Sun*

Main category: cs.LG

TL;DR: 提出了一种结合优化和引导生成的新方法，用于物理目标驱动的逆向设计，通过改进成本预测器训练损失和密度梯度优化，在保持形状合理性的同时提升目标性能。


<details>
  <summary>Details</summary>
Motivation: 基于物理目标的逆向设计具有挑战性，因为它将高维几何与昂贵的仿真耦合在一起，例如空气动力学形状优化中的阻力减少问题。现有方法在优化和引导生成之间存在分离，且难以处理高维条件协方差估计。

Method: 1) 提出新的成本预测器训练损失；2) 提出密度梯度优化方法，在提升目标的同时保持形状合理性；3) 统一现有免训练引导生成方法；4) 开发时间-内存高效的近似协方差估计算法，解决高维条件协方差估计问题。

Result: 在受控2D研究和高保真3D空气动力学基准测试（汽车和飞机）中，通过OpenFOAM仿真和3D打印原型的小型风洞测试验证，在优化和引导生成方面均获得一致性能提升。离线强化学习结果进一步支持了方法的通用性。

Conclusion: 通过重新审视逆向设计的两个典型解（最优设计点和最优设计分布），并将其与优化和引导生成联系起来，提出了一种统一框架，在保持形状合理性的同时有效提升物理目标性能，为高维物理驱动逆向设计提供了新思路。
...
TL;DR: 该论文研究反事实解释如何扩大MLaaS系统的攻击面，增强成员推理攻击，并提出结合差分隐私和主动学习的防御框架来平衡隐私、性能和解释质量。


<details>
  <summary>Details</summary>
Motivation: MLaaS系统中集成的反事实解释虽然提高了透明度，但可能加剧隐私攻击风险，特别是成员推理攻击。目前对解释如何影响这一威胁格局的理解不足，需要研究防御机制以在不损害效用和可解释性的前提下降低风险。

Method: 1. 系统分析通过查询API暴露反事实解释如何增强基于影子模型的成员推理攻击；2. 提出结合差分隐私和主动学习的防御框架，共同减少模型记忆并限制有效训练数据暴露；3. 进行广泛的实证评估，量化隐私泄露、预测性能和解释质量之间的三方权衡。

Result: 研究发现反事实解释确实会扩大MLaaS攻击面，增强成员推理攻击效果。提出的DP+AL防御框架能有效降低隐私泄露，同时保持合理的预测性能和解释质量。实证结果揭示了透明度、效用和隐私之间需要仔细平衡。

Conclusion: 在负责任地部署可解释MLaaS系统时，必须仔细平衡透明度、效用和隐私。反事实解释虽然提高了可解释性，但也带来了新的隐私风险，需要采用适当的防御机制来缓解这些风险。
...
Method: 提出Phys-Instruct框架：1) 通过匹配生成器和先验扩散分布将预训练扩散PDE求解器压缩为少步生成器；2) 通过PDE蒸馏引导显式注入PDE知识增强物理一致性；基于坚实理论基础的物理约束训练目标
...
TL;DR: CTTVAE+TBS：基于条件Transformer的表格变分自编码器，通过类别感知三元组边界损失和训练时采样策略，在严重类别不平衡下生成更具代表性的合成表格数据，提升下游任务中少数类别的性能。


<details>
  <summary>Details</summary>
Motivation: 在类别严重不平衡的表格数据生成中，现有生成模型要么忽略少数类别，要么生成的样本对下游学习无实际效用。特别是在医疗、欺诈检测、预测性维护等领域，罕见但高影响的事件对决策至关重要，需要专门解决少数类别生成问题的模型。

Method: 提出CTTVAE+TBS框架：1) CTTVAE（条件Transformer表格变分自编码器）使用类别感知三元组边界损失，重构潜在空间以实现类内紧凑性和类间分离性；2) TBS（训练时采样策略）自适应增加对少数类别的训练曝光。两者结合确保生成更具代表性和实用性的样本。

Result: 在六个真实世界基准测试中，CTTVAE+TBS在少数类别上实现了最强的下游效用，通常优于在原始不平衡数据上训练的模型，同时保持竞争力的保真度。消融研究证实潜在空间重构和针对性采样都对性能提升有贡献。

Conclusion: 通过明确优先考虑少数类别的下游性能，CTTVAE+TBS为条件表格数据生成提供了稳健且可解释的解决方案，可直接应用于医疗、欺诈检测和预测性维护等行业，其中少数类别的微小改进都至关重要。
...
TL;DR: 本文提出了一种基于强化学习的检索器优化方法，通过将确定性检索替换为随机采样，并将检索历史纳入状态表示，解决了现有方法中的目标不匹配和状态混叠问题。


<details>
  <summary>Details</summary>
Motivation: 检索增强生成（RAG）的性能依赖于检索器与LLM的匹配度，但现有检索器优化方法存在目标不匹配问题。强化学习虽然提供了有前景的解决方案，但应用于检索器优化面临两个基本挑战：1）确定性检索与RL公式不兼容；2）在多跳推理中仅基于查询的检索会导致状态混叠。

Method: 1）将确定性检索替换为随机采样，将RAG建模为马尔可夫决策过程，使检索器可通过RL优化；2）在每个检索步骤中将检索历史纳入状态表示，以缓解状态混叠问题。

Result: 在多种RAG流水线、数据集和检索器规模上的广泛实验表明，该方法在RAG性能上实现了持续改进。

Conclusion: 通过将RAG重新表述为可通过RL优化的MDP，并解决状态混叠问题，本文提出的方法有效提升了检索增强生成的性能，为检索器优化提供了新的解决方案。
...
TL;DR: CoRSA框架通过参数高效的知识编辑方法，解决LLM知识更新中的泛化性、稳定性和知识冲突问题，在多个基准测试中显著优于现有方法。
...
Method: 提出CoRSA训练框架，采用参数高效的知识编辑方法，同时解决多个挑战：通过最小化损失曲率提高不同输入形式的泛化性和多轮更新的稳定性，通过最大化新旧知识间的间隔解决知识冲突。

Result: 在三个广泛使用的事实编辑基准测试中，CoRSA在泛化性方面显著优于基线方法，平均绝对改进分别为：比LoRA高12.42%，比模型编辑方法高10%。在多轮更新中，保持高更新效率的同时，灾难性遗忘比LoRA减少27.82%。在代码领域也表现出色，Pass@5更新效率比最强基线高5.48%。

Conclusion: CoRSA框架为大型语言模型的知识编辑提供了一种参数高效、全面的解决方案，有效解决了泛化性、稳定性和知识冲突等关键问题，在事实编辑和代码领域都表现出优越性能。
...
TL;DR: 提出一种名为离策略对数分散正则化（LDR）的新正则化框架，用于提高Boltzmann生成器的数据效率和训练效果，无需额外在线策略样本。


<details>
  <summary>Details</summary>
Motivation: Boltzmann生成器用于从物理系统的玻尔兹曼分布中独立采样，但实际应用受限于数据效率问题，因为模拟数据和目标能量评估都很昂贵。需要一种能够提高训练数据效率的方法。

Method: 提出离策略对数分散正则化（LDR）框架，基于对数方差目标的泛化。LDR作为能量景观的形状正则化器，利用目标能量标签的额外信息。该方法支持无偏或有偏模拟数据集，以及纯变分训练，无需目标样本。

Result: 在所有基准测试中，LDR都提高了最终性能和数据效率，样本效率提升高达一个数量级。该方法与标准基于数据的训练目标结合使用，无需额外在线策略样本。

Conclusion: LDR是一种有效的正则化框架，能够显著提高Boltzmann生成器的数据效率和训练效果，为从非归一化概率密度采样提供了更实用的解决方案。
...
TL;DR: 提出改进MWEM框架的方法，将每轮迭代的时间复杂度从Θ(m)降低到Θ(√m)，通过懒采样和Gumbel噪声实现


<details>
  <summary>Details</summary>
Motivation: MWEM是私有数据分析的基础迭代框架，但每轮执行指数机制需要Θ(m)时间，成为可扩展性的瓶颈

Method: 采用懒采样方法改进Report-Noisy-Max机制，使用Gumbel噪声和k-最近邻数据结构，避免线性扫描快速选择近似分数

Result: 将每轮迭代的运行时依赖改进为Θ(√m)期望复杂度，实验验证相比经典MWEM有显著运行时改进
...
TL;DR: 提出基于LSTM的软传感器，利用井口和地面测量数据估算流动井底压力，替代昂贵的永久井下压力计，在巴西盐下盆地实际数据上实现MAPE低于2%的精度。


<details>
  <summary>Details</summary>
Motivation: 永久井下压力计（PDGs）虽然能提供实时压力数据，但存在可靠性和成本问题，需要开发更经济可靠的替代方案来监测井底变量以优化生产、确保安全和减少排放。

Method: 提出基于长短期记忆（LSTM）模型的软传感器，使用井口和地面测量数据估算流动井底压力（BHP），并与多层感知器（MLP）和岭回归进行比较，同时开创性地应用迁移学习来适应不同操作环境。

Result: 在巴西盐下盆地的实际海上数据集测试中，该方法实现了平均绝对百分比误差（MAPE）持续低于2%，优于基准方法。

Conclusion: 该工作提供了一种成本效益高、精度准确的物理传感器替代方案，具有跨不同储层和流动条件的广泛适用性。
...
TL;DR: 离散扩散模型在推理时需要更多计算，但性能优于自回归模型。研究发现联合预测所有未知token是关键机制，将其视为潜在token并调节其数量可实现推理速度与样本质量的权衡。


<details>
  <summary>Details</summary>
Motivation: 离散扩散模型在语言建模中已能与自回归模型竞争，在需要规划和全局一致性的推理任务上甚至表现更优，但推理时需要更多计算。研究旨在理解这种权衡背后的机制，并探索如何改进两种模型。

Method: 通过消融实验分析扩散模型中的联合预测机制，将未解码token解释为潜在token，提出调节潜在token数量的方法。同时将潜在token概念引入自回归模型，通过辅助多token预测目标改进其性能。

Result: 研究发现扩散模型的联合预测机制是关键，消融该机制会加速推理但降低性能。调节潜在token数量可实现推理速度与样本质量的平滑权衡。将潜在token引入自回归模型能显著提升其在传统上表现不佳的推理任务上的性能。

Conclusion: 潜在token虽然在扩散模型中自然出现，但代表了改进需要全局一致性或前瞻性任务性能的通用机制。该发现为平衡推理效率与模型性能提供了新思路，并展示了跨模型架构的通用改进方法。
...
TL;DR: RC是一种迭代解码算法，通过利用LLM响应生成与总结能力的不对称性构建推理链，使模型能够在测试时持续改进并外推至远超训练时所见的长推理范围。
...
Method: 引入RC（迭代解码算法），在训练和推理时替代标准自回归解码。RC利用LLM响应生成与总结能力的不对称性，构建跨迭代持续改进的推理链。

Result: 使用RC训练的模型能够外推并持续改进推理范围，比训练时所见长一个数量级以上。4B模型在16k-token训练预算下，通过0.5m测试token将HMMT 2025性能从40%提升至近70%，优于同规模模型和许多更大的推理LLM。

Conclusion: RC使LLM能够在测试时持续改进并外推至更长的推理范围。训练后的模型能更有效地利用现有脚手架进一步扩展测试时性能，这得益于训练中学习的改进的总结条件生成能力。
...
TL;DR: 提出一种基于推理时间的遗忘框架，使用验证器和反馈迭代优化生成响应，无需更新模型参数，显著降低遗忘误差


<details>
  <summary>Details</summary>
Motivation: 现有遗忘方法通常需要重新训练模型参数，在现实应用中假设常被挑战，特别是生成模型，且会损害模型的通用能力

Method: 采用推理时间遗忘范式，为生成模型配备验证器判断响应是否满足遗忘保证，利用符合预测减少计算开销并提供分布无关的遗忘保证

Result: 在具有挑战性的遗忘基准测试中，该方法显著优于现有最先进方法，将遗忘误差降低高达93%

Conclusion: 推理时间遗忘框架通过验证器和反馈机制有效解决传统遗忘方法的局限性，在保持模型通用能力的同时提供可靠的遗忘保证
...
TL;DR: 提出了一种新的流形随机特征（MRF）方法，用于近似定义在一般流形上的双变量函数（特别是核函数），通过离散化流形和图随机特征技术来学习流形上的连续场。


<details>
  <summary>Details</summary>
Motivation: 在一般流形上，许多连续逼近机制无法通过解析方法推导。需要一种新方法来创建随机特征以近似流形上的双变量函数（特别是核函数），同时保证特征的正定性和有界性，这是实现准确、低方差逼近的关键特性。

Method: MRF方法结合了流形离散化和图随机特征（GRF）技术。首先将流形离散化，然后应用GRF来学习流形上的连续场。这些连续场用于找到在一般情况下无法解析推导的连续逼近机制。该方法还重新发现了高斯核近似机制，应用于改进线性注意力Transformer，通过简单的图随机游走绕过原始复杂的数学计算。

Result: MRF提供了正定且有界的特征，这是实现准确、低方差逼近的关键特性。研究展示了图随机特征（定义在离散图对象上）与用于正则核的连续随机特征之间的深度渐近联系。通过实验验证了算法的有效性。

Conclusion: MRF为流形上的核函数逼近提供了一种新的有效范式，将离散图方法与连续随机特征理论联系起来，为改进线性注意力Transformer等应用提供了新的数学工具和理论框架。
...
### [171] [SymPlex: A Structure-Aware Transformer for Symbolic PDE Solving](https://arxiv.org/abs/2602.03816)
*Yesom Park,Annie C. Lu,Shao-Ching Huang,Qiyang Hu,Y. Sungtaek Ju,Stanley Osher*

Main category: cs.LG

TL;DR: SymPlex：基于强化学习的符号PDE求解框架，通过树结构决策和语法约束Transformer直接发现解析符号解，无需真实表达式


<details>
  <summary>Details</summary>
Motivation: 现有数值和神经方法在离散化或隐函数空间中近似求解PDE，缺乏可解释性和显式参数依赖性；序列生成器表达能力有限，无法有效处理符号表达式的层次结构

Method: 将符号PDE求解建模为树结构决策问题，使用SymFormer（结构感知Transformer）通过树相对自注意力建模层次符号依赖，语法约束自回归解码确保句法有效性，仅利用PDE和边界条件优化候选解

Result: 成功精确恢复了非光滑和参数化PDE的解析解，实现了深度学习符号方法在PDE求解中的突破

Conclusion: SymPlex直接在符号表达式空间中操作，生成可解释、人类可读的解，能自然表示非光滑行为和显式参数依赖，为符号PDE求解提供了新范式
...
### [172] [Robust Intervention Learning from Emergency Stop Interventions](https://arxiv.org/abs/2602.03825)
*Ethan Pronovost,Khimya Khetarpal,Siddhartha Srinivasa*

Main category: cs.LG

TL;DR: 论文提出鲁棒干预学习（RIL）框架和残差干预微调（RIFT）算法，用于从噪声干预数据中学习策略，通过结合先验策略解决干预信号不完整的问题。


<details>
  <summary>Details</summary>
Motivation: 自主系统中的人类干预是测试期间常见的数据来源，这些干预提供了策略需要改进的重要信号，但通常存在噪声且不完整。现有方法假设避免干预就足以完成任务，但在现实场景中这通常只是必要条件而非充分条件，需要更鲁棒的学习方法。

Method: 提出残差干预微调（RIFT）算法，将干预学习视为微调问题，将干预反馈视为不完整的学习信号，并显式地将其与先验策略结合。通过利用先验策略编码的结构来解决干预信号对任务描述不充分时的歧义问题。

Result: 理论分析刻画了该框架能够实现原则性策略改进的条件，并识别了干预学习可能失败的机制。实验表明，残差微调能够在各种干预策略和先验策略质量下实现鲁棒且一致的策略改进。

Conclusion: 鲁棒干预学习是未来工作的有前景方向，残差干预微调方法能够有效处理干预信号的质量和信息量问题，通过结合先验策略解决干预信号不完整的问题，实现鲁棒策略改进。
...
### [173] [Uncertainty and Fairness Awareness in LLM-Based Recommendation Systems](https://arxiv.org/abs/2602.02582)
*Chandan Kumar Sah,Xiaoli Lian,Li Zhang,Tony Xu,Syed Shazaib Shah*

Main category: cs.AI

TL;DR: 该论文研究了大型语言模型在零样本推荐中的不确定性、公平性和偏见问题，提出了包含不确定性评估和人格感知公平性的评估框架，并在电影和音乐领域进行了实证分析。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型虽然能够利用广泛上下文知识实现强大的零样本推荐，但其预测不确定性和内嵌偏见威胁着推荐的可靠性和公平性。现有研究缺乏系统评估这些因素对LLM推荐准确性、一致性和可信度影响的方法。

Method: 1) 引入包含精心设计指标的基准测试和包含8个人口统计属性（31个分类值）的数据集，涵盖电影和音乐两个领域；2) 通过深度案例研究量化预测不确定性（通过熵计算）；3) 分析Google DeepMind的Gemini 1.5 Flash模型对敏感属性的系统性不公平；4) 在提示扰动（如拼写错误和多语言输入）下测试公平性；5) 将人格感知公平性整合到RecLLM评估流程中，揭示人格相关的偏见模式。

Result: 1) Gemini 1.5 Flash表现出对某些敏感属性的系统性不公平，相似性差距指标SNSR为0.1363，SNSV为0.0507；2) 这些不公平在提示扰动下持续存在；3) 揭示了人格个性化与群体公平性之间的权衡；4) 建立了不确定性感知的评估方法学，提供了深度不确定性案例研究的实证见解。

Conclusion: 该论文提出了一个新颖的不确定性感知评估方法学，引入了人格档案知情的公平性基准，提高了LLM推荐的可解释性和公平性。这些贡献为更安全、更可解释的RecLLMs奠定了基础，并激励未来在多模型基准测试和自适应校准方面的研究，以实现可信部署。
...
### [174] [A Positive Case for Faithfulness: LLM Self-Explanations Help Predict Model Behavior](https://arxiv.org/abs/2602.02639)
*Harry Mayne,Justin Singh Kang,Dewi Gould,Kannan Ramchandran,Adam Mahdi,Noah Y. Siegel*

Main category: cs.AI

TL;DR: 提出Normalized Simulatability Gain (NSG)作为评估LLM自我解释忠实度的新指标，发现自我解释能显著提升模型行为预测能力(11-37%)，优于外部模型解释，但5-15%的自我解释存在严重误导性。


<details>
  <summary>Details</summary>
Motivation: 现有LLM自我解释的忠实度评估方法存在局限性，主要依赖对抗性提示或检测推理错误，忽视了解释的预测价值。需要一种更通用、可扩展的忠实度评估指标。

Method: 引入Normalized Simulatability Gain (NSG)指标，基于"忠实解释应能让观察者学习模型的决策标准，从而更好预测相关输入上的行为"这一理念。在7,000个反事实样本上评估18个前沿专有和开源模型，涵盖健康、商业和伦理领域。

Result: 自我解释显著提升模型行为预测能力(11-37% NSG增益)，且比外部模型生成的解释提供更多预测信息，即使外部模型更强。这表明自我知识带来的优势是外部解释方法无法复制的。同时发现5-15%的自我解释存在严重误导性。

Conclusion: 尽管存在不完美之处，但自我解释确实编码了有助于预测模型行为的信息，为自我解释提供了积极案例。NSG为评估解释忠实度提供了通用且可扩展的框架。
...
### [175] [Dynamic Mix Precision Routing for Efficient Multi-step LLM Interaction](https://arxiv.org/abs/2602.02711)
*Yuanzhe Li,Jianing Deng,Jingtong Hu,Tianlong Chen,Song Wang,Huanrui Yang*

Main category: cs.AI

TL;DR: 提出动态混合精度路由框架，在长时程决策任务中自适应选择高精度或低精度LLM，以降低推理成本同时保持任务成功率


<details>
  <summary>Details</summary>
Motivation: 长时程决策任务中，使用大型LLM进行多步交互推理成本过高，而实践中通常认为高任务成功率需要使用更大更强的LLM模型，这导致了推理成本与性能之间的权衡问题

Method: 提出动态混合精度路由框架，基于观察到不同交互步骤对精度的敏感性差异，自适应地在每个决策步骤选择高精度或低精度LLM。采用两阶段训练流程：1) KL散度监督学习识别精度敏感步骤；2) 组相对策略优化(GRPO)进一步提高任务成功率

Result: 在ALFWorld上的实验表明，该方法相比单精度基线和启发式路由方法，在准确率-成本权衡方面取得了显著改进

Conclusion: 通过动态混合精度路由框架，可以在保持长时程决策任务成功率的同时，显著降低LLM推理成本，为实际应用提供了有效的解决方案
...
### [176] [Scaling-Aware Adapter for Structure-Grounded LLM Reasoning](https://arxiv.org/abs/2602.02780)
*Zihao Jing,Qiuhao Zeng,Ruiyi Fang,Yan Yi Li,Yan Sun,Boyu Wang,Pingzhao Hu*

Main category: cs.AI

TL;DR: Cuttlefish是一个统一的全原子LLM，通过自适应缩放的结构补丁和几何接地适配器，在保持几何线索的同时实现多模态结构推理，解决了现有方法的结构幻觉和模态融合瓶颈问题。
...
Method: 提出Cuttlefish框架，包含两个核心技术：1）缩放感知补丁：使用指令条件门控机制在结构图上生成可变大小的补丁，根据结构复杂度自适应缩放查询标记预算；2）几何接地适配器：通过跨注意力机制细化自适应标记并注入模态嵌入，将显式几何线索暴露给LLM以减少结构幻觉。

Result: 在多样化的全原子基准测试中，Cuttlefish在异构结构接地推理方面表现出优越性能，代码已在项目仓库中公开。

Conclusion: Cuttlefish通过自适应缩放结构标记和显式几何接地，实现了语言推理在几何线索上的有效基础，为通用全原子推理提供了有前景的解决方案。
...
### [177] [AutoSizer: Automatic Sizing of Analog and Mixed-Signal Circuits via Large Language Model (LLM) Agents](https://arxiv.org/abs/2602.02849)
...
<details>
  <summary>Details</summary>
Motivation: 模拟混合信号集成电路设计严重依赖专家知识，晶体管尺寸优化面临非线性行为、高维设计空间和严格性能约束等挑战。现有EDA方法通常将尺寸优化视为静态黑盒优化，导致效率低下且鲁棒性不足。大语言模型虽然具备强大推理能力，但不适合AMS尺寸优化的精确数值优化。

Method: 提出AutoSizer反射式LLM驱动的元优化框架，统一了电路理解、自适应搜索空间构建和优化协调的闭环系统。采用双循环优化架构：内循环进行电路尺寸优化，外循环分析优化动态和约束，根据仿真反馈迭代精炼搜索空间。同时建立了AMS-SizingBench开放基准，包含SKY130 CMOS技术中的24个多样化AMS电路。

Result: AutoSizer在实验中表现出更高的解决方案质量、更快的收敛速度和更高的成功率，在不同电路难度下均优于传统优化方法和现有基于LLM的智能体。

Conclusion: AutoSizer通过将LLM的推理能力与自适应优化策略相结合，有效解决了AMS电路尺寸优化的核心挑战，为自动化集成电路设计提供了新的有效框架。
...
### [178] [STEER: Inference-Time Risk Control via Constrained Quality-Diversity Search](https://arxiv.org/abs/2602.02862)
*Eric Yang,Jong Ha Lee,Jonathan Amar,Elissa Ye,Yugang Jia*

Main category: cs.AI

TL;DR: STEER框架通过进化多样性搜索构建自然语言角色库，实现无需训练的决策保守性可调控制，解决LLM在临床分诊等序数决策任务中的模式坍缩问题。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型在训练追求平均正确性时容易出现模式坍缩，在临床分诊等序数决策任务中无法根据上下文约束灵活调整特异性和敏感性（ROC操作点），标准对齐方法移除了这种可调控制能力。

Method: STEER框架：1）通过离线约束质量-多样性搜索构建自然语言角色群体，在保证最低安全、推理和稳定性阈值的同时促进行为覆盖；2）推理时通过单一可解释控制参数将用户指定的风险百分位数映射到选定角色，实现决策保守性的单调调整。

Result: 在两个临床分诊基准测试中：1）相比基于温度的采样和静态角色集成方法，STEER获得更广泛的行为覆盖；2）相比代表性后训练方法，在明确紧急病例上保持显著更高的准确性，同时在模糊决策上提供可比较的控制能力。

Conclusion: STEER作为一种安全保持的风险控制范式，能够在无需训练的情况下实现行为引导，且不损害领域能力，为LLM在临床决策等关键应用中提供了可调的风险控制机制。
...
### [179] [Aligning Language Model Benchmarks with Pairwise Preferences](https://arxiv.org/abs/2602.02898)
*Marco Gutierrez,Xinyi Leng,Hannah Cyberey,Jonathan Richard Schwarz,Ahmed Alaa,Thomas Hartvigsen*

Main category: cs.AI

TL;DR: 提出BenchAlign方法，通过有限的实际部署信息自动更新离线基准测试，使其能更好地预测模型在真实场景中的偏好排序


<details>
  <summary>Details</summary>
Motivation: 当前语言模型基准测试虽然计算高效，但往往无法准确预测模型在真实世界中的实际效用。许多研究发现基准测试与真实性能之间存在差距，需要建立更有效的评估方法

Method: 提出BenchAlign方法，利用模型在基准测试问题上的表现数据以及部署期间收集的模型排序对，学习与偏好对齐的基准问题权重，生成能根据特定偏好对未见模型进行排序的新基准

Result: 实验表明，对齐后的基准能够准确根据人类偏好模型对未见模型进行排序，且在不同规模模型上保持有效性，同时保持可解释性

Conclusion: 该工作为基准测试与实际人类偏好的对齐提供了见解，有望加速模型开发向真实效用的转变，提出了基准对齐的新框架和首个解决方案
...
### [180] [Reasoning about Reasoning: BAPO Bounds on Chain-of-Thought Token Complexity in LLMs](https://arxiv.org/abs/2602.02909)
...
Method: 扩展了有界注意力前缀预言机模型，量化解决任务所需的信息流。针对三个BAPO-hard任务（二进制多数、三元组匹配、图可达性）证明了推理token数量的下界。通过显式构造提供了匹配或接近匹配的上界，并用前沿推理模型进行了实验验证。

Result: 证明了三个任务都需要Ω(n)个推理token（输入规模为n）。实验显示前沿推理模型在这些任务上表现出近似线性的推理token扩展，当约束在较小的推理预算时会失败，与理论下界一致。

Conclusion: 研究识别了通过思维链进行推理时计算的基本瓶颈，提供了一个分析最优推理长度的原则性工具。结果表明线性推理token扩展是这些任务的基本要求。
...
### [181] [UAT-LITE: Inference-Time Uncertainty-Aware Attention for Pretrained Transformers](https://arxiv.org/abs/2602.02952)
*Elias Hossain,Shubhashis Roy Dipta,Subash Neupane,Rajib Rana,Ravid Shwartz-Ziv,Ivan Garibay,Niloofar Yousefi*

Main category: cs.AI

TL;DR: UAT-LITE：基于蒙特卡洛dropout的推理时框架，通过使自注意力机制具备不确定性感知能力来改进预训练transformer分类器的校准，无需修改权重或训练目标


<details>
  <summary>Details</summary>
Motivation: 神经NLP模型通常校准不佳，对错误预测赋予高置信度，这影响了选择性预测和高风险部署。现有后处理校准方法仅调整输出概率而不改变内部计算，而集成和贝叶斯方法虽然能改善不确定性但需要高昂的训练或存储成本。

Method: 提出UAT-LITE推理时框架，通过蒙特卡洛dropout进行近似贝叶斯推断，使预训练transformer分类器的自注意力机制具备不确定性感知能力。从随机前向传播中估计token级认知不确定性，并在上下文化过程中调制自注意力，无需修改预训练权重或训练目标。还引入层间方差分解来诊断预测不确定性如何在transformer深度中累积。

Result: 在SQuAD 2.0可回答性、MNLI和SST-2数据集上，UAT-LITE相对于微调的BERT-base基线平均减少约20%的期望校准误差，同时保持任务准确性，并改善了选择性预测和分布偏移下的鲁棒性。

Conclusion: UAT-LITE通过推理时使自注意力机制具备不确定性感知能力，有效改进了预训练transformer分类器的校准性能，为选择性预测和分布偏移下的鲁棒性提供了实用解决方案，且无需修改模型权重或训练目标。
...
### [182] [Structuring Value Representations via Geometric Coherence in Markov Decision Processes](https://arxiv.org/abs/2602.02978)
*Zuyuan Zhang,Zeyu Fang,Tian Lan*

Main category: cs.AI

TL;DR: GCR-RL：通过偏序集理论重构强化学习，提出几何一致性正则化方法，通过超偏序集细化序列确保价值函数学习的几何一致性，显著提升样本效率和稳定性。


<details>
  <summary>Details</summary>
Motivation: 现有方法利用几何特性（如对称结构、几何感知数据增强、结构限制）来稳定和加速强化学习，但缺乏从序理论角度系统处理价值函数估计的几何一致性。

Method: 将价值函数估计重构为学习期望的偏序集，提出GCR-RL方法，通过计算超偏序集细化序列——细化先前步骤中的偏序集并从时序差分信号学习额外序关系，确保支撑学习价值函数的偏序集序列的几何一致性。开发了基于Q学习和actor-critic的两种新算法来实现这些超偏序集细化。

Result: 理论分析了算法的性质和收敛速率。在多个任务上的实证评估表明，GCR-RL相比强基线在样本效率和稳定性能方面有显著提升。

Conclusion: 从序理论视角重新审视强化学习，提出的GCR-RL框架通过确保偏序集序列的几何一致性，有效提升了强化学习的样本效率和稳定性，为利用几何特性优化RL提供了新的理论和方法基础。
...
### [183] [Are LLMs Biased Like Humans? Causal Reasoning as a Function of Prior Knowledge, Irrelevant Information, and Reasoning Budget](https://arxiv.org/abs/2602.02983)
*Hanna M. Dettki,Charley M. Wu,Bob Rehder*

Main category: cs.AI

TL;DR: LLMs在因果推理任务中表现出比人类更规则化的推理策略，不显示人类特有的共因偏误，思维链提示能增强其鲁棒性。
...
Method: 使用共因结构(C₁→E←C₂)的11个因果判断任务，将20+个LLMs与匹配的人类基线对比，测试语义抽象和提示过载下的鲁棒性，并分析思维链提示的影响。

Result: 大多数LLMs表现出比人类更规则化的推理策略，不显示人类特有的弱解释消除和马尔可夫违背偏误；思维链提示能提高许多LLMs的鲁棒性；小型可解释模型能很好压缩LLMs的因果判断。

Conclusion: LLMs在已知人类偏误不受欢迎时可作为人类补充，但其规则化推理在不确定性固有情况下可能失效，需要表征LLMs推理策略以实现安全有效部署。
...
### [184] [Large Language Models Can Take False First Steps at Inference-time Planning](https://arxiv.org/abs/2602.02991)
*Haijiang Yan,Jian-Qiao Zhu,Adam Sanborn*

Main category: cs.AI

TL;DR: LLMs在训练中获得序列规划能力，但在推理时表现出短视和不一致的规划行为。研究提出贝叶斯解释：自生成上下文驱动规划偏移，导致看似受损的规划行为。


<details>
  <summary>Details</summary>
Motivation: 解释大型语言模型在推理时表现出的规划行为与训练获得的规划能力之间的差距。虽然LLMs在训练中获得了序列级规划能力，但在实际推理中往往表现出短视和不一致的规划行为，需要理论解释这一现象。

Method: 提出贝叶斯解释框架，将规划行为建立在不断演化的生成上下文基础上。通过两个受控实验验证：1) 随机生成任务展示人类提示下的受限规划以及自生成上下文积累时规划强度的增加；2) 高斯采样任务展示在条件于自生成序列时初始偏见的减少。

Result: 实验验证了提出的模型：随机生成任务显示LLMs在人类提示下规划受限，但随着自生成上下文积累，规划强度增加；高斯采样任务显示当条件于自生成序列时，初始偏见减少。这些结果为LLMs在推理时如何进行前瞻规划提供了理论和实证证据。

Conclusion: LLMs推理时的规划行为差距可以通过自生成上下文驱动的规划偏移来解释。自然语言与LLMs内部化语言之间的细微差异导致累积的自生成上下文在推理过程中引发规划转变，从而产生看似受损的规划行为。研究为理解LLMs的规划机制提供了理论框架和实证支持。
...
### [185] [Agent Alpha: Tree Search Unifying Generation, Exploration and Evaluation for Computer-Use Agents](https://arxiv.org/abs/2602.02995)
*Sizhe Tang,Rongqian Chen,Tian Lan*

Main category: cs.AI

TL;DR: Agent Alpha：通过步级MCTS实现GUI智能体主动规划的统一框架，在OSWorld基准上达到77%成功率
...
Method: 提出Agent Alpha统一框架，通过步级蒙特卡洛树搜索（MCTS）协同生成、探索和评估；集成alpha-UCT引导搜索实现主动规划；采用比较驱动评估缓解绝对评分偏差，多样性约束扩展保持紧凑信息搜索空间

Result: 在OSWorld基准上达到约77%的最先进成功率，在同等计算量下显著优于轨迹级基线方法

Conclusion: Agent Alpha通过步级MCTS实现了GUI智能体的主动规划能力，能够早期剪枝次优分支并高效重用前缀，显著提升了任务执行成功率
...
### [186] [Methods and Open Problems in Differentiable Social Choice: Learning Mechanisms, Decisions, and Alignment](https://arxiv.org/abs/2602.03003)
*Zhiyu An,Wan Du*
...
<details>
  <summary>Details</summary>
Motivation: 社会选择已成为现代机器学习系统的核心组成部分，从拍卖、资源分配到联邦学习、参与式治理和大语言模型对齐，机器学习系统越来越多地将异质偏好、激励和判断聚合为集体决策。然而，这些系统通常隐含地实现社会选择机制，缺乏明确的规范性审查。

Method: 采用可微分社会选择范式，将投票规则、机制和聚合过程构建为可学习、可微分的模型，通过数据驱动的方式进行优化。该方法综合了拍卖、投票、预算编制、流动民主、去中心化聚合和逆向机制学习等领域的研究。

Result: 展示了经典公理和不可能性定理如何重新表现为目标函数、约束条件和优化权衡。通过这一框架，社会选择理论的形式化概念可以直接集成到机器学习管道中，实现数据驱动的机制设计。

Conclusion: 提出了36个开放性问题，定义了一个新的研究议程，旨在连接机器学习、经济学和民主理论，推动可微分社会选择领域的发展，为构建更透明、公平和有效的集体决策系统提供理论基础。
...
### [187] [Distilling LLM Reasoning into Graph of Concept Predictors](https://arxiv.org/abs/2602.03006)
*Ziyang Yu,Liang Zhao*

Main category: cs.AI

TL;DR: GCP是一种推理感知的主动蒸馏框架，通过将教师模型的决策过程外化为有向无环图，并用模块化概念预测器在学生模型中镜像该图，从而提高样本效率和训练稳定性。


<details>
  <summary>Details</summary>
Motivation: 当前LLM在判别任务部署中存在推理延迟、计算和API成本问题。传统主动蒸馏方法仅蒸馏最终标签，丢弃了中间推理信号，且缺乏对缺失推理内容和错误来源的诊断能力。

Method: 提出Graph of Concept Predictors (GCP)：1) 将教师决策过程外化为有向无环图；2) 在学生模型中用模块化概念预测器镜像该图；3) 采用图感知采集策略，针对关键推理节点的不确定性和分歧；4) 执行针对性子模块重训练，将下游损失归因于特定概念预测器并仅更新最有影响的模块。

Result: 在八个NLP分类基准测试中，GCP在有限标注预算下提升了性能，同时产生了更可解释和可控的训练动态。

Conclusion: GCP框架通过外部化教师推理过程并采用模块化设计，显著提高了主动蒸馏的样本效率、训练稳定性和可解释性，为LLM的轻量化部署提供了有效解决方案。
...
### [188] [STAR: Similarity-guided Teacher-Assisted Refinement for Super-Tiny Function Calling Models](https://arxiv.org/abs/2602.03022)
*Jiliang Ni,Jiachen Pu,Zhongyi Yang,Jingfeng Luo,Conggang Hu*

Main category: cs.AI

TL;DR: STAR框架通过相似性引导的教师辅助精炼，将大语言模型的能力蒸馏到超小型模型，解决了现有方法中的过拟合、训练不稳定、二元奖励无效等问题，在函数调用任务上实现了最先进的性能。


<details>
  <summary>Details</summary>
Motivation: 大语言模型在函数调用中至关重要，但其大规模阻碍了广泛采用，需要将其能力转移到小型模型。现有方法存在过拟合、训练不稳定、多解任务中二元奖励无效以及技术协同困难等问题。

Method: STAR框架包含两个核心技术创新：1) 约束知识蒸馏(CKD)，通过增强top-k前向KL散度来抑制自信的错误预测，确保训练稳定性同时保留下游强化学习的探索能力；2) 相似性引导强化学习(Sim-RL)，引入细粒度、基于相似性的奖励，通过评估生成输出与真实值的相似性提供鲁棒、连续、丰富的信号优化策略。

Result: 在具有挑战性的基准测试中，STAR模型在其规模类别中建立了最先进性能，显著优于基线方法。特别是0.6B的STAR模型在所有1B以下的开源模型中表现最佳，甚至超越了多个更大规模的知名开源模型。
...
### [189] [RC-GRPO: Reward-Conditioned Group Relative Policy Optimization for Multi-Turn Tool Calling Agents](https://arxiv.org/abs/2602.03025)
*Haitian Zhong,Jixiu Zhai,Lei Song,Jiang Bian,Qiang Liu,Tieniu Tan*

Main category: cs.AI

TL;DR: RC-GRPO通过引入奖励条件令牌解决GRPO中组内奖励变化低导致的探索停滞问题，在BFCLv4多轮工具调用基准上取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 多轮工具调用对LLM具有挑战性，因为奖励稀疏且探索成本高。传统的SFT+GRPO方法在组内奖励变化低时（如组内多数rollout获得全0或全1奖励）会停滞，导致组归一化优势无信息量，更新消失

Method: 提出RC-GRPO（奖励条件组相对策略优化），将探索视为通过离散奖励令牌可控的转向问题。首先在混合质量轨迹上微调奖励条件轨迹策略（RCTP），在提示中注入奖励目标特殊令牌（如<|high_reward|>, <|low_reward|>），使模型能够按需生成不同质量的轨迹。然后在RL期间，在每个GRPO组内采样多样化的奖励令牌，并在采样的令牌上条件化rollout，以提高组内多样性，从而改善优势增益

Result: 在Berkeley Function Calling Leaderboard v4（BFCLv4）多轮基准测试中，该方法相比基线获得持续改进的性能，Qwen-2.5-7B-Instruct上的性能甚至超过了所有闭源API模型

Conclusion: RC-GRPO通过奖励条件令牌有效解决了GRPO中组内多样性不足导致的探索停滞问题，在多轮工具调用任务上取得了显著性能提升，证明了可控探索策略的有效性
...
TL;DR: MAS4TS是一个基于多智能体系统的通用时间序列分析框架，通过视觉推理和潜在重建实现跨任务泛化
...
Method: 基于Analyzer-Reasoner-Executor范式构建工具驱动的多智能体系统，集成视觉语言模型进行时间序列图表的视觉推理，在潜在空间重建预测轨迹，通过共享内存和门控通信协调三个专门智能体，路由器选择任务特定工具链

Result: 在多个基准测试中达到最先进性能，展现出强大的泛化能力和高效推理

Conclusion: MAS4TS通过整合视觉推理、多智能体协作和工具链选择，为通用时间序列任务提供了有效的解决方案
...
TL;DR: 对多智能体系统中过程验证的系统性实证研究，发现过程级验证并不能持续提升性能且方差高，LLM-as-a-Judge方法优于基于奖励的方法，但有效的MAS过程验证仍是一个开放挑战。


<details>
  <summary>Details</summary>
Motivation: 基于大语言模型的多智能体系统在推理轨迹上表现出高方差，过程验证（评估轨迹中的中间步骤）在一般推理场景中显示出潜力，但其在多智能体系统中的实际有效性尚不清楚。

Method: 提出了MAS-ProVe系统，涵盖三种验证范式（LLM-as-a-Judge、奖励模型、过程奖励模型），在两个验证粒度级别（智能体级和迭代级）进行评估，研究了五个代表性验证器和四种上下文管理策略，在六个不同的MAS框架和多个推理基准上进行实验。

Result: 过程级验证不能持续提升性能且经常表现出高方差；LLM-as-a-Judge方法总体上优于基于奖励的方法，训练过的法官优于通用LLM；LLM作为法官与作为单智能体之间存在小的性能差距；验证中存在上下文长度与性能的权衡。
...
### [192] [Risky-Bench: Probing Agentic Safety Risks under Real-World Deployment](https://arxiv.org/abs/2602.03100)
...
TL;DR: Risky-Bench：一个系统化的智能体安全评估框架，基于真实世界部署场景，通过领域无关的安全原则和上下文感知的安全标准来评估智能体在复杂交互任务中的安全风险。


<details>
  <summary>Details</summary>
Motivation: 现有智能体安全评估方法存在三个主要问题：1）依赖针对特定智能体设置的风险导向任务，导致安全风险空间覆盖有限；2）无法评估智能体在复杂真实世界部署中长期、交互式任务执行中的安全行为；3）对特定智能体设置的专门化限制了跨不同智能体配置的适应性。

Method: 提出Risky-Bench框架，围绕领域无关的安全原则组织评估，推导出上下文感知的安全标准来界定安全空间，并通过在不同威胁假设下的真实任务执行来系统评估整个安全空间的风险。

Result: 在生活辅助智能体设置中应用Risky-Bench，发现在真实执行条件下，最先进的智能体存在显著的安全风险。该框架不仅限于生活辅助场景，可以适应其他部署设置来构建环境特定的安全评估。

Conclusion: Risky-Bench提供了一个可扩展的智能体安全评估方法学，能够系统评估智能体在复杂真实世界部署中的安全风险，解决了现有评估方法的局限性，并为不同智能体配置提供了适应性强的评估框架。
...
### [193] [Understanding Multi-Agent LLM Frameworks: A Unified Benchmark and Experimental Analysis](https://arxiv.org/abs/2602.03128)
*Abdelghny Orogat,Ana Rostam,Essam Mansour*

Main category: cs.AI

TL;DR: MAFBench：首个统一评估多智能体LLM框架架构影响的基准测试套件，揭示了框架设计选择可导致100倍延迟差异和30%准确率下降


<details>
  <summary>Details</summary>
Motivation: 现有研究缺乏对多智能体LLM框架架构设计对系统性能影响的系统性理解。框架的架构选择（如智能体交互、信息存储、任务协调方式）可导致数量级的性能差异，但现有基准测试仅关注单一能力且缺乏框架层面的标准化评估。

Method: 提出多智能体LLM框架的架构分类法，从基本维度系统比较不同框架；开发MAFBench统一评估套件，将现有基准测试集成到标准化执行流程中；使用MAFBench对多个主流框架进行受控实证研究。

Result: 框架层面的设计选择单独可导致：延迟增加超过100倍；规划准确率下降高达30%；协调成功率从90%以上降至30%以下。这些差异完全由架构设计引起，而非智能体能力差异。

Conclusion: 多智能体LLM框架的架构设计对系统性能有决定性影响。研究提出了具体的架构设计原则和框架选择指南，并指出了未来研究方向。MAFBench为框架评估提供了标准化工具，填补了该领域的研究空白。
...
### [194] [General Agents Contain World Models, even under Partial Observability and Stochasticity](https://arxiv.org/abs/2602.03146)
*Santiago Cifuentes*

Main category: cs.AI

TL;DR: 该论文将先前关于智能体必然学习环境模型的定理扩展到随机智能体和部分可观测环境，证明随机化无法避免学习环境模型，并放宽了"通用性"要求。
...
Method: 通过理论扩展和证明，将原定理推广到随机智能体在部分可观测环境中的情况。同时通过放宽"通用性"概念的要求，证明能力较弱的智能体也包含其操作环境的世界模型。

Result: 成功证明了随机智能体在部分可观测环境中也无法避免学习其环境模型，随机化不能成为避免学习环境模型的策略。同时证明了更弱的通用性条件已足以保证智能体包含世界模型。

Conclusion: 随机智能体在部分可观测环境中仍然必然学习其环境模型，这从根本上表明随机化无法避免环境学习。放宽通用性要求后，该结论对更广泛的智能体类别成立，强化了智能体必然包含世界模型的理论基础。
...
### [195] [Enhancing Foundation VLM Robustness to Missing Modality: Scalable Diffusion for Bi-directional Feature Restoration](https://arxiv.org/abs/2602.03151)
*Wei Dai,Haoyu Wang,Honghao Chang,Lijun He,Fan Li,Jian Sun,Haixia Bi*

Main category: cs.AI

TL;DR: 提出一种用于视觉语言模型的通用缺失模态恢复策略，通过增强扩散模型作为可插拔模块，结合动态模态门控和跨模态互学习机制，有效恢复缺失特征并保持模型泛化能力。


<details>
  <summary>Details</summary>
Motivation: 当前视觉语言模型在推理时假设完整模态输入，当某些模态缺失时性能急剧下降。现有方法面临两大困境：基于提示的方法难以恢复缺失的关键特征且损害模型泛化；基于插补的方法缺乏有效指导，容易生成语义无关的噪声。如何在恢复精确语义的同时保持VLM的泛化能力仍是挑战。

Method: 提出通用缺失模态恢复策略，引入增强扩散模型作为可插拔的中期训练模块。包含两个关键创新：1) 动态模态门控，自适应利用条件特征引导生成语义一致的特征；2) 跨模态互学习机制，桥接双编码器的语义空间实现双向对齐。

Result: 在基准数据集上的零样本评估表明，该方法优于现有基线方法。大量实验和消融研究证实，该模型在缺失模态场景下是VLM的鲁棒且可扩展的扩展，确保在不同缺失率和环境下的可靠性。

Conclusion: 该方法为视觉语言模型提供了一种有效的缺失模态恢复解决方案，通过创新的动态门控和跨模态学习机制，在保持模型泛化能力的同时准确恢复缺失语义特征，具有实际应用价值。
...
### [196] [VALUEFLOW: Toward Pluralistic and Steerable Value-based Alignment in Large Language Models](https://arxiv.org/abs/2602.03160)
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM与人类价值观对齐存在三个关键问题：基于偏好的方法无法捕捉深层动机原则；价值观提取忽略层次结构；评估仅检测存在性而非校准强度；LLM在受控强度下的可引导性理解不足

Method: 提出VALUEFLOW统一框架，包含三个组件：1) HIVES层次化价值观嵌入空间，捕捉理论和跨理论价值观结构；2) VIDB大规模价值观标注文本数据库，基于排序聚合的强度估计；3) 基于锚点的评估器，通过将模型输出与VIDB面板排序产生一致的强度分数

Result: 在十个模型和四个价值观理论上的大规模研究，识别了可引导性的不对称性以及多价值观控制的组合规律，建立了评估和控制价值观强度的可扩展基础设施

Conclusion: VALUEFLOW为LLM的多元对齐提供了可扩展的基础设施，通过校准强度控制推进价值观对齐，解决了现有方法在层次结构、强度评估和可引导性理解方面的局限性
...
### [197] [Beyond Quantity: Trajectory Diversity Scaling for Code Agents](https://arxiv.org/abs/2602.03219)
...
<details>
  <summary>Details</summary>
Motivation: 随着代码大语言模型通过MCP协议演化为工具交互智能体，其泛化能力受到低质量合成数据和数量扩展收益递减的限制。数量为中心的扩展存在早期瓶颈，未能充分利用轨迹数据。

Method: TDScaling框架包含四个创新：1）业务集群机制捕捉真实服务逻辑依赖；2）蓝图驱动的多智能体范式确保轨迹连贯性；3）基于领域熵、推理模式熵和累积动作复杂度的自适应进化机制，引导合成朝向长尾场景；4）沙盒化代码工具缓解内在编码能力的灾难性遗忘。

Result: 在通用工具使用基准（BFCL、tau^2-Bench）和代码智能体任务（RebenchT、CodeCI、BIRD）上的实验表明，TDScaling实现了双赢：既提升了工具使用的泛化能力，又增强了固有的编码能力。

Conclusion: TDScaling通过轨迹多样性扩展而非数量扩展，为代码智能体训练提供了更优的性能-成本权衡方案，计划在发表后发布完整代码库和包含30,000+工具集群的合成数据集。
...
### [198] [TAME: A Trustworthy Test-Time Evolution of Agent Memory with Systematic Benchmarking](https://arxiv.org/abs/2602.03224)
*Yu Cheng,Jiuan Zhou,Yongkang Hu,Yihang Chen,Huichi Zhou,Mingang Chen,Zhizhong Zhang,Kun Shao,Yuan Xie,Zhaoxia Yin*

Main category: cs.AI

TL;DR: 该论文研究了智能体在测试时记忆演化过程中出现的信任度下降问题（Agent Memory Misevolution），提出了Trust-Memevo基准来评估这一现象，并开发了TAME双记忆演化框架来同时保持任务性能和安全性。


<details>
  <summary>Details</summary>
Motivation: 智能体通过测试时记忆演化积累经验是实现AGI的关键范式，但即使在良性任务演化过程中，智能体的安全对齐仍然脆弱，会出现信任度下降的现象（Agent Memory Misevolution）。目前缺乏对这一现象的系统评估方法。

Method: 提出了TAME双记忆演化框架：1）构建Trust-Memevo基准评估多维度信任度；2）分离演化执行器记忆（提升任务性能）和评估器记忆（基于历史反馈优化安全和任务效用评估）；3）通过记忆过滤、草稿生成、可信度精炼、执行和双轨记忆更新的闭环流程。

Result: 实验表明：1）Trust-Memevo基准揭示了在多种任务领域和评估设置下，智能体记忆演化过程中信任度普遍下降；2）TAME框架有效缓解了记忆误演化问题，在保持任务性能的同时显著提升了信任度。

Conclusion: 智能体记忆演化过程中的信任度下降是一个普遍存在的问题，TAME框架通过分离演化执行器和评估器记忆，实现了任务性能和信任度的联合优化，为解决智能体安全对齐问题提供了有效方案。
...
### [199] [The Necessity of a Unified Framework for LLM-Based Agent Evaluation](https://arxiv.org/abs/2602.03238)
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型驱动的通用智能体评估面临独特挑战：现有基准测试被系统提示、工具集配置、环境动态等外部因素严重混淆，评估框架碎片化且依赖研究者特定的提示工程，导致难以将性能提升归因于模型本身，同时缺乏标准化环境数据导致错误不可追溯、结果不可复现

Method: 提出标准化智能体评估框架的提案，旨在统一评估标准，解决当前评估中的混杂因素和碎片化问题

Result: 论文提出了标准化评估框架的提案，但未提供具体实施结果，主要贡献在于识别问题并提出解决方案方向

Conclusion: 标准化评估框架对于智能体评估的严谨发展至关重要，当前评估中的不公平性和不透明性需要通过统一标准来解决，以推动该领域的科学进步
...
### [200] [Accordion-Thinking: Self-Regulated Step Summaries for Efficient and Readable LLM Reasoning](https://arxiv.org/abs/2602.03249)
*Zhicheng Yang,Zhijiang Guo,Yinya Huang,Yongxin Wang,Wenlei Shi,Yiwei Wang,Xiaodan Liang,Jing Tang*

Main category: cs.AI

TL;DR: Accordion-Thinking框架让LLM学习通过动态总结来自我调节推理步骤的粒度，实现推理上下文的压缩，在保持准确性的同时显著提升推理效率


<details>
  <summary>Details</summary>
Motivation: 传统长链式思维推理面临KV缓存线性增长和注意力二次复杂度的实际限制，需要一种更高效的推理机制

Method: 引入Accordion-Thinking端到端框架，LLM学习通过动态总结自我调节推理步骤粒度，采用Fold推理模式定期总结思维过程并丢弃历史token，应用强化学习进一步激励这种能力

Result: 模型学会将关键推理信息编码到紧凑总结中，实现推理上下文的有效压缩；在48GB GPU内存配置下实现3倍吞吐量同时保持准确性；Fold模式与Unfold模式之间的准确率差距在训练过程中逐渐缩小直至消失

Conclusion: 通过学习的自我压缩，LLM能够以最小依赖token开销处理复杂推理任务而不影响解决方案质量，结构化步骤总结提供了人类可读的推理过程记录
...
### [201] [CSR-Bench: A Benchmark for Evaluating the Cross-modal Safety and Reliability of MLLMs](https://arxiv.org/abs/2602.03263)
*Yuxuan Liu,Yuntian Shi,Kun Wang,Haoting Shen,Kun Yang*

Main category: cs.AI

TL;DR: CSR-Bench是一个评估多模态大语言模型跨模态可靠性的基准测试，通过四种压力测试模式（安全性、过度拒绝、偏见、幻觉）覆盖61种细粒度类型，揭示MLLMs存在系统性跨模态对齐差距。


<details>
  <summary>Details</summary>
Motivation: 当前多模态大语言模型的安全行为可能由单模态捷径驱动而非真正的联合意图理解，需要评估其跨模态可靠性以揭示潜在问题。

Method: 构建CSR-Bench基准，包含四种压力测试交互模式（安全性、过度拒绝、偏见、幻觉），覆盖61种细粒度类型。每个实例需要图像-文本集成理解，并提供配对的纯文本对照以诊断模态诱导的行为变化。评估了16个最先进的MLLMs。

Result: 观察到系统性跨模态对齐差距：模型表现出弱安全意识、强语言主导性（在干扰下）、从纯文本对照到多模态输入的一致性能下降。还观察到减少过度拒绝与保持安全、非歧视行为之间的明显权衡。

Conclusion: 一些明显的安全改进可能来自拒绝导向的启发式方法而非鲁棒的意图理解。需要开发真正理解跨模态意图的MLLMs，而不仅仅是依赖单模态捷径。
...
### [202] [Memora: A Harmonic Memory Representation Balancing Abstraction and Specificity](https://arxiv.org/abs/2602.03315)
...
TL;DR: Memora是一种谐波记忆表示方法，通过抽象与具体性的结构平衡来扩展智能体记忆系统，在保持细节的同时支持高效检索。


<details>
  <summary>Details</summary>
Motivation: 智能体记忆系统需要处理持续增长的信息，同时支持下游任务的高效、上下文感知检索。抽象对扩展记忆至关重要，但通常以牺牲具体性为代价，模糊了有效推理所需的细粒度细节。

Method: 引入Memora谐波记忆表示，通过主要抽象索引具体记忆值并将相关更新整合为统一记忆条目，同时使用线索锚点扩展跨不同方面的检索访问并连接相关记忆。在此基础上采用检索策略，主动利用这些记忆连接来检索超出直接语义相似性的相关信息。

Result: 理论上证明标准检索增强生成(RAG)和知识图谱(KG)记忆系统是本框架的特殊情况。在LoCoMo和LongMemEval基准测试中建立了新的最先进水平，展示了随着记忆扩展，检索相关性和推理有效性更好。

Conclusion: Memora通过结构平衡抽象与具体性，为智能体记忆系统提供了一种可扩展的解决方案，在保持细节的同时支持高效检索，超越了传统RAG和KG方法的局限性。
...
### [203] [MentalSeek-Dx: Towards Progressive Hypothetico-Deductive Reasoning for Real-world Psychiatric Diagnosis](https://arxiv.org/abs/2602.03340)
*Xiao Sun,Yuming Yang,Junnan Zhu,Jiang Zhong,Xinyu Zhou,Kaiwen Wei*

Main category: cs.AI

TL;DR: 提出首个面向真实临床场景的精神障碍诊断基准MentalDx Bench，揭示LLM在粗粒度分类与细粒度诊断间的范式错配，并开发专门化模型MentalSeek-Dx实现SOTA性能


<details>
  <summary>Details</summary>
Motivation: 现有LLM在精神健康评估中缺乏生态效度和细粒度诊断监督，临床实用性受限，需要建立真实临床场景下的诊断基准

Method: 1) 构建MentalDx Bench基准：包含712份去标识化电子健康记录，由精神科医生按ICD-11标注，覆盖16个诊断类别的76种障碍；2) 开发MentalSeek-Dx模型：通过监督轨迹构建和课程式强化学习内化临床推理过程

Result: 评估18个LLM发现关键范式错配：粗粒度诊断分类表现良好，但在障碍级诊断中系统性失败；MentalSeek-Dx仅用14B参数即达到SOTA性能

Conclusion: 建立了首个临床基础的精神障碍诊断框架，揭示了LLM临床推理的局限性，并提出专门化模型解决方案，为可靠的精神科诊断提供了新范式
...
### [204] [Risk Awareness Injection: Calibrating Vision-Language Models for Safety without Compromising Utility](https://arxiv.org/abs/2602.03402)
*Mengxuan Wang,Yuxin Chen,Gang Xu,Tao He,Hongjie Jiang,Ming Li*

Main category: cs.AI

TL;DR: 提出RAI框架，通过构建不安全原型子空间和针对性调制高风险视觉token，在无需训练的情况下增强VLM的安全风险识别能力


<details>
  <summary>Details</summary>
Motivation: 现有VLM安全防御方法依赖安全微调或激进的token操作，成本高且会降低模型效用。研究发现LLM本身能识别文本不安全内容，但VLM中的视觉输入会稀释风险信号，因此需要恢复VLM的LLM式风险识别能力
...
Conclusion: RAI是一种轻量级、无需训练的安全校准框架，通过恢复VLM的LLM式风险识别能力，有效防御多模态越狱攻击，同时保持模型效用
...
### [205] [DiscoverLLM: From Executing Intents to Discovering Them](https://arxiv.org/abs/2602.03429)
*Tae Soo Kim,Yoonjoo Lee,Jaesang Yu,John Joon Young Chung,Juho Kim*

Main category: cs.AI

TL;DR: DiscoverLLM是一个训练大语言模型帮助用户形成和发现意图的框架，通过模拟用户认知状态和渐进具体化的意图层次，让模型学会在意图不明确时探索选项，在意图具体化时收敛实现，在多个交互基准测试中显著提升任务性能并减少对话长度。


<details>
  <summary>Details</summary>
Motivation: 当前大语言模型处理模糊和开放式请求时，通常通过询问澄清问题来揭示用户未表达的意图。然而，用户模糊往往是因为他们尚未形成明确意图，需要观察和探索结果来发现自己的需求。简单地询问"你想要什么语气？"在用户自己也不知道的情况下会失败。

Method: 引入DiscoverLLM框架，核心是一个新颖的用户模拟器，用层次化的意图建模认知状态，这些意图随着模型呈现相关选项而渐进具体化。具体化程度作为奖励信号，模型可以被训练来优化这个信号。模型学会通过与用户协作，在意图不明确时自适应地发散（探索选项），在意图具体化时收敛（细化和实现）。

Result: 在创意写作、技术写作和SVG绘图等交互基准测试中，DiscoverLLM实现了超过10%的任务性能提升，同时将对话长度减少了高达40%。在75名人类参与者的用户研究中，DiscoverLLM相比基线方法提高了对话满意度和效率。

Conclusion: DiscoverLLM提供了一个通用框架，训练大语言模型帮助用户形成和发现意图，通过建模渐进具体化的意图层次和优化具体化程度作为奖励，使模型能够更有效地与用户协作，在意图探索和实现之间取得平衡，显著提升交互效果。
...
### [206] [Ontology-to-tools compilation for executable semantic constraint enforcement in LLM agents](https://arxiv.org/abs/2602.03439)
...
TL;DR: 论文提出将本体编译为工具接口的机制，使LLM能够利用形式化领域知识，在知识图谱生成过程中强制执行语义约束而非事后验证。


<details>
  <summary>Details</summary>
Motivation: 解决LLM与形式化领域知识耦合的问题，减少手动模式工程和提示工程，通过语义约束指导LLM行为，确保生成的知识图谱符合领域规范。

Method: 在The World Avatar框架中，将本体规范编译为可执行工具接口，结合Model Context Protocol和智能体，实现从非结构化科学文本中提取、验证和修复结构化知识的迭代工作流。

Result: 以金属有机多面体合成文献为例，展示了可执行本体语义如何指导LLM行为，减少手动模式工程和提示工程，建立了将形式化知识嵌入生成系统的通用范式。

Conclusion: 本体到工具的编译机制为LLM与形式化领域知识的耦合提供了原理性证明，通过语义约束指导生成过程，为知识图谱生态系统中的生成模型、符号约束和外部资源的结构化交互奠定了基础。
...
### [207] [CRL-VLA: Continual Vision-Language-Action Learning](https://arxiv.org/abs/2602.03445)
*Qixin Zeng,Shuo Zhang,Hongyin Zhang,Renjie Wang,Han Zhao,Libang Zhao,Runze Li,Donglin Wang,Chao Huang*

Main category: cs.AI

TL;DR: CRL-VLA：一种用于视觉语言动作模型持续后训练的框架，通过非对称调节解决稳定性-可塑性权衡问题，在LIBERO基准测试中表现优异
...
Method: 提出CRL-VLA框架，通过非对称调节解决稳定性-可塑性权衡：约束先前任务的优势幅度，同时允许新任务上的受控增长。采用具有新颖目标条件价值公式的双评论家架构，其中冻结评论家锚定语义一致性，可训练估计器驱动适应。

Result: 在LIBERO基准测试中，CRL-VLA有效协调了冲突目标，在抗遗忘和正向适应方面均优于基线方法。推导了统一的性能边界，将稳定性-可塑性权衡与目标条件优势幅度（按策略散度缩放）联系起来。

Conclusion: CRL-VLA为VLA模型的持续后训练提供了一个具有严格理论边界的框架，通过非对称调节机制成功解决了持续强化学习中的稳定性-可塑性权衡问题，为终身机器人学习提供了有效解决方案。
...
### [208] [The Dual Role of Abstracting over the Irrelevant in Symbolic Explanations: Cognitive Effort vs. Understanding](https://arxiv.org/abs/2602.03467)
*Zeynep G. Saribatur,Johannes Langer,Ute Schmid*

Main category: cs.AI

TL;DR: 研究形式化抽象（移除和聚类）如何影响人类推理性能和认知负荷，使用答案集编程（ASP）作为形式框架，通过认知实验验证抽象化能增强以人为中心的符号解释


<details>
  <summary>Details</summary>
Motivation: AI系统产生的输出往往难以理解，而符号AI虽然提供了透明的可解释性基础，但原始逻辑轨迹通常会给人类带来较高的外在认知负荷。需要研究如何通过形式化抽象来改善人类对符号解释的理解和认知效率。

Method: 使用答案集编程（ASP）作为形式框架，定义需要抽象化的无关细节概念，通过移除和聚类两种抽象操作获得简化解释。设计认知实验，让参与者使用从答案集程序导出的解释对跨领域刺激进行分类。

Result: 认知实验表明：聚类细节显著提高参与者的理解能力，而移除细节则显著降低认知努力。这些结果支持了抽象化能增强以人为中心的符号解释的假设。

Conclusion: 形式化抽象（特别是移除和聚类操作）能有效改善人类对符号AI解释的理解并降低认知负荷，为设计更人性化的可解释AI系统提供了实证支持。
...
### [209] [IntentRL: Training Proactive User-intent Agents for Open-ended Deep Research via Reinforcement Learning](https://arxiv.org/abs/2602.03468)
*Haohao Luo,Zexi Li,Yuexiang Xie,Wenhao Zhang,Yaliang Li,Ying Shen*

Main category: cs.AI

TL;DR: IntentRL框架训练主动式智能体在开始长期研究前澄清用户潜在意图，通过两阶段强化学习解决深度研究中的自主性-交互性困境


<details>
  <summary>Details</summary>
Motivation: 深度研究（DR）智能体虽然能超越LLMs的参数知识进行自主检索和证据合成，但存在计算成本高、耗时长的问题，导致在模糊用户查询时出现自主性-交互性困境：高自主性常导致执行时间过长且结果不理想
...
Conclusion: IntentRL通过训练主动澄清用户意图的智能体，有效解决了深度研究中的自主性-交互性困境，为长视野研究任务提供了更高效的交互范式
...
### [210] [When Routing Collapses: On the Degenerate Convergence of LLM Routers](https://arxiv.org/abs/2602.03478)
*Guannan Lai,Han-Jia Ye*

Main category: cs.AI

TL;DR: EquiRouter解决现有LLM路由器的路由崩溃问题，通过直接学习模型排名而非标量性能分数，在保持GPT-4级别性能的同时降低约17%成本


<details>
  <summary>Details</summary>
Motivation: 现有LLM路由器存在路由崩溃问题：随着用户成本预算增加，路由器会系统性地默认选择最强大且最昂贵的模型，即使更便宜的模型已经足够。这导致小模型利用率不足，浪费计算和金钱成本，违背了路由的核心承诺。

Method: 提出EquiRouter，一种决策感知的路由器，直接学习模型排名而非预测标量性能分数。这种方法解决了目标-决策不匹配问题：现有路由器训练预测标量性能分数，而路由决策最终依赖于候选模型之间的离散比较。

Result: 在RouterBench上，EquiRouter在保持GPT-4级别性能的同时，相比之前最强的路由器降低了约17%的成本。该方法有效恢复了小模型的作用，缓解了路由崩溃现象。

Conclusion: 路由崩溃是现有LLM路由器中普遍存在但未被充分探索的失败模式。通过直接学习模型排名而非标量性能分数的决策感知方法，EquiRouter能够更好地利用小模型，实现更优的质量-成本权衡。
...
### [211] [Group Selection as a Safeguard Against AI Substitution](https://arxiv.org/abs/2602.03541)
*Qiankun Zhong,Thomas F. Eisenmann,Julian Garcia,Iyad Rahwan*

Main category: cs.AI

TL;DR: 研究AI使用对人类文化演化的长期影响，发现AI替代型用户（依赖AI生成大部分内容）在个体层面选择中占优但会减少文化多样性，而AI补充型用户（寻求建议但保持主要创作者角色）在群体选择中更有利，能维持文化创新所需的变化。


<details>
  <summary>Details</summary>
Motivation: 生成式AI的依赖会降低文化多样性和变化，这已经导致了模型性能问题（如模型崩溃和幻觉）。本研究旨在探讨AI使用对人类文化演化的长期后果，以及广泛使用AI可能导致"文化崩溃"的条件——即依赖AI生成内容会减少人类变化和创新，减缓累积文化演化。

Method: 使用基于主体的建模和演化博弈论，比较两种AI使用类型：补充型（寻求建议和指导，但仍是最终输出的主要生产者）和替代型（提供最小输入，依赖AI产生大部分输出）。研究这些使用策略在演化动态下的竞争和传播。

Result: AI替代型用户在个体层面选择中占优势，尽管会更强地减少文化变化。相比之下，AI补充型用户通过维持探索所需的变化而有益于群体，因此在群体边界较强时，可以通过文化群体选择得到青睐。

Conclusion: 研究结果揭示了AI采用对人口层面的长期影响，为政策和组织策略提供了信息，以减轻AI依赖导致文化多样性和创新减少的风险。维持文化多样性需要平衡个体利益与群体利益，考虑群体选择机制。
...
### [212] [EHRWorld: A Patient-Centric Medical World Model for Long-Horizon Clinical Trajectories](https://arxiv.org/abs/2602.03569)
*Linjie Mu,Zhongzhen Huang,Yannian Gu,Shengqian Qin,Shaoting Zhang,Xiaofan Zhang*

Main category: cs.AI

TL;DR: EHRWorld：基于因果序列范式训练的以患者为中心的医疗世界模型，使用真实电子健康记录数据，在长期临床模拟中显著优于基于LLM的基线方法


<details>
  <summary>Details</summary>
Motivation: 现有大型语言模型在静态医疗推理任务上表现良好，但作为动态医疗世界模型模拟疾病进展和治疗结果时，难以在序列干预下保持一致的患者状态，导致长期模拟中的误差累积

Method: 提出EHRWorld，一个在因果序列范式下训练的以患者为中心的医疗世界模型，同时构建了EHRWorld-110K——一个基于真实世界电子健康记录的大规模纵向临床数据集

Result: EHRWorld显著优于基于LLM的基线方法，实现了更稳定的长期模拟、对临床敏感事件的改进建模以及更优的推理效率

Conclusion: 可靠的医疗世界建模需要在因果基础、时间演化的临床数据上进行训练，EHRWorld展示了这种方法的必要性和有效性
...
### [213] [Mitigating Conversational Inertia in Multi-Turn Agents](https://arxiv.org/abs/2602.03664)
*Yang Wan,Zheng Cao,Zhenhao Zhang,Zhengwen Zeng,Shuheng Shen,Changhua Meng,Linchao Zhu*

Main category: cs.AI

TL;DR: 论文提出"对话惯性"概念，指出LLM在多轮智能体场景中会错误地将自身先前响应作为few-shot示例模仿，导致探索受限。通过上下文偏好学习框架校准模型偏好，降低惯性，提升智能体性能。


<details>
  <summary>Details</summary>
Motivation: 大型语言模型作为few-shot学习者在多轮智能体场景中存在严重问题：模型会错误地将自己之前的响应作为few-shot示例进行模仿，导致"对话惯性"现象，限制了智能体的探索能力。这揭示了将few-shot LLM转化为智能体时的内在矛盾：更长的上下文虽然提供了更丰富的环境反馈用于利用，但也放大了对话惯性从而损害探索。

Method: 通过注意力分析识别对话惯性现象，发现模型对先前响应表现出强烈的对角线注意力模式。关键洞察是：对于相同状态，使用更长上下文生成的动作表现出更强的惯性。基于此构建无需环境奖励的偏好对，提出上下文偏好学习框架来校准模型偏好，使其倾向于低惯性响应而非高惯性响应。同时提供推理时的上下文管理策略来平衡探索与利用。

Result: 在八个智能体环境和一个深度研究场景中的实验结果表明，该框架有效降低了对话惯性，并实现了性能提升。验证了上下文偏好学习能够校准模型偏好，改善智能体在多轮交互中的探索能力。

Conclusion: 对话惯性是few-shot LLM转化为智能体时的重要限制因素。通过注意力分析揭示的惯性现象为理解这一限制提供了新视角。提出的上下文偏好学习框架能够有效校准模型偏好，降低惯性，平衡探索与利用，从而提升智能体在多轮交互中的性能表现。
...
### [214] [TodyComm: Task-Oriented Dynamic Communication for Multi-Round LLM-based Multi-Agent System](https://arxiv.org/abs/2602.03688)
*Wenzhe Fan,Tommaso Tognoli,Henry Peng Zou,Chunyu Miao,Yibo Wang,Xinhua Zhang*

Main category: cs.AI

TL;DR: TodyComm：一种面向任务的动态通信算法，用于多轮LLM多智能体系统，通过策略梯度优化生成行为驱动的协作拓扑，适应每轮动态变化


<details>
  <summary>Details</summary>
Motivation: 现有方法在推理时使用固定通信拓扑，无法适应现实应用中智能体角色可能跨轮次变化的情况（如动态对抗、任务进展或时变通信带宽约束）

Method: 提出TodyComm（任务导向动态通信算法），通过策略梯度优化生成行为驱动的协作拓扑，使通信结构能够适应每轮的动态变化

Result: 在五个基准测试上的实验表明，在动态对抗和通信预算约束下，TodyComm在任务有效性方面表现优越，同时保持了令牌效率和可扩展性

Conclusion: TodyComm解决了多轮LLM多智能体系统中固定通信拓扑的局限性，通过动态适应每轮变化的通信结构实现了更优的任务性能
...
### [215] [AOrchestra: Automating Sub-Agent Creation for Agentic Orchestration](https://arxiv.org/abs/2602.03786)
...
TL;DR: AOrchestra：一个统一的、框架无关的智能体抽象系统，通过动态创建专用执行器来解决复杂长时程任务，在三个基准测试中取得显著性能提升


<details>
  <summary>Details</summary>
Motivation: 现有语言智能体系统缺乏对子智能体的动态抽象视图，限制了系统适应性。为解决复杂长时程任务自动化中的这一挑战，需要一种统一的智能体抽象方法

Method: 提出统一智能体抽象：将任何智能体建模为四元组(指令、上下文、工具、模型)。基于此构建AOrchestra系统，其中中央编排器在每个步骤具体化该四元组：策划任务相关上下文、选择工具和模型，并通过即时自动智能体创建委派执行

Result: 在三个挑战性基准测试(GAIA、SWE-Bench、Terminal-Bench)中，AOrchestra与Gemini-3-Flash配对时，相对于最强基线实现了16.28%的相对改进。系统支持可控的性能-成本权衡，接近帕累托效率

Conclusion: AOrchestra通过统一的智能体抽象和动态执行器创建，显著提升了语言智能体在复杂任务中的适应性和性能，同时减少了人工工程工作量，并保持框架无关性
...
### [216] [Understanding Agent Scaling in LLM-Based Multi-Agent Systems via Diversity](https://arxiv.org/abs/2602.03794)
*Yingxuan Yang,Chengrui Qu,Muning Wen,Laixi Shi,Ying Wen,Weinan Zhang,Adam Wierman,Shangding Gu*

Main category: cs.AI

TL;DR: 多智能体系统性能受限于任务内在不确定性而非智能体数量，异质性智能体通过提供互补证据显著提升性能，2个异质智能体可媲美16个同质智能体


<details>
  <summary>Details</summary>
Motivation: 研究LLM多智能体系统性能扩展的局限性：发现增加同质智能体数量存在严重收益递减，而异质性配置能持续带来显著提升，需要理解这种差异的根本原因

Method: 提出信息论框架分析MAS性能边界，证明性能受限于任务内在不确定性而非智能体数量；引入K*指标量化无真实标签时的有效通道数；通过实验验证异质配置优于同质扩展

Result: 异质配置始终优于同质扩展：2个异质智能体可匹配或超越16个同质智能体的性能；同质智能体因输出强相关而早期饱和，异质智能体提供互补证据

Conclusion: 多智能体系统性能受任务内在不确定性限制，异质性通过增加有效通道提升性能；提出基于多样性的高效鲁棒MAS设计原则，K*指标为无标签评估提供量化工具
...
### [217] [Conformal Thinking: Risk Control for Reasoning on a Compute Budget](https://arxiv.org/abs/2602.03814)
...
TL;DR: 提出基于风险控制的LLM推理预算框架，通过上下阈值机制自适应停止推理，在保证目标错误率的同时最小化计算开销


<details>
  <summary>Details</summary>
Motivation: 大型语言模型支持测试时扩展，准确率随token预算增加而提高，但设置token预算和自适应推理阈值存在实际挑战，需要在风险和准确率之间权衡

Method: 提出风险控制框架：1) 上阈值在模型置信度高时停止推理（可能输出错误）；2) 新颖的参数化下阈值在问题不可解时提前停止（可能过早停止）；3) 使用无分布风险控制方法基于验证集优化停止机制；4) 对于多预算控制标准，引入效率损失选择计算效率最高的退出机制

Result: 在多种推理任务和模型上的实证结果表明，风险控制方法有效，下阈值和集成停止机制带来计算效率提升，同时满足用户指定的风险目标

Conclusion: 提出的风险控制框架能够有效管理LLM推理的计算预算，在保证目标错误率的前提下最小化计算开销，为自适应推理提供了实用的解决方案
...
### [218] [AutoFigure: Generating and Refining Publication-Ready Scientific Illustrations](https://arxiv.org/abs/2602.03828)
*Minjun Zhu,Zhen Lin,Yixuan Weng,Panzhong Lu,Qiujie Xie,Yifan Wei,Sifan Liu,Qiyao Sun,Yue Zhang*

Main category: cs.AI

TL;DR: FigureBench是首个大规模科学文本到插图生成基准，包含3300个高质量文本-插图对；AutoFigure是首个基于长文本自动生成高质量科学插图的智能体框架，通过思维、重组和验证实现结构完整且美观的插图。


<details>
  <summary>Details</summary>
Motivation: 高质量科学插图对于有效传达复杂科技概念至关重要，但其手动制作在学术界和工业界都是公认的瓶颈，需要自动化解决方案来提升科学传播效率。

Method: 提出FigureBench基准数据集（3300个高质量文本-插图对）和AutoFigure智能体框架，该框架在最终渲染前通过广泛的思维、重组和验证过程，生成结构合理且美观的布局。

Result: 实验结果表明，AutoFigure在所有基线方法中表现最佳，能够生成可直接用于发表的科学插图，代码、数据集和HuggingFace空间已开源。

Conclusion: FigureBench为科学插图生成提供了首个大规模基准，AutoFigure框架通过智能体方法有效解决了科学插图自动生成的挑战，为科学传播提供了实用工具。
...
### [219] [Relaxed Triangle Inequality for Kullback-Leibler Divergence Between Multivariate Gaussian Distributions](https://arxiv.org/abs/2602.02577)
*Shiji Xiao,Yufeng Zhang,Chubo Liu,Yan Ding,Keqin Li,Kenli Li*

Main category: stat.ML

TL;DR: 本文研究了多元高斯分布之间KL散度的松弛三角不等式，给出了KL散度上确界的精确表达式及其可达条件，改进了现有上界结果。


<details>
  <summary>Details</summary>
Motivation: KL散度不是真正的距离度量，不满足三角不等式，这给理论分析和实际应用带来挑战。现有研究已证明多元高斯分布间的KL散度满足松弛三角不等式，但上确界仍未知，需要更精确的上界分析。

Method: 通过理论分析研究多元高斯分布间KL散度的松弛三角不等式，推导出KL散度上确界的精确表达式，并确定该上确界可达的条件。

Result: 当ε₁和ε₂较小时，KL(𝒩₁, 𝒩₃)的上确界为ε₁+ε₂+√(ε₁ε₂)+o(ε₁)+o(ε₂)，比现有上界3ε₁+3ε₂+2√(ε₁ε₂)+o(ε₁)+o(ε₂)更紧致，并给出了上确界可达的条件。

Conclusion: 本文给出了多元高斯分布间KL散度松弛三角不等式的精确上界，改进了现有结果，并在基于流的生成模型的分布外检测和安全强化学习中展示了应用价值。
...
### [220] [Rethinking Test-Time Training: Tilting The Latent Distribution For Few-Shot Source-Free Adaptation](https://arxiv.org/abs/2602.02633)
...
<details>
  <summary>Details</summary>
Motivation: 在部署环境中，即使轻量级参数更新也可能导致模型偏移或调优不稳定，需要研究在完全冻结模型且无法访问上游数据情况下的测试时适应方法

Method: 使用指数倾斜方法，基于少量标记支持集计算的任务相似性分数，以KL最优方式重加权编码器诱导的潜在嵌入分布，实现分布校正而不修改模型参数

Result: 该方法在多个基准测试和不同样本数量设置下，与基于参数更新的方法表现相当，同时在更严格和普遍的约束条件下运行

Conclusion: 即使采用完全冻结的模型管道，推理级别的分布校正对于测试时适应仍然是可行的，展示了无需参数更新的有效适应策略
...
### [221] [Near-Universal Multiplicative Updates for Nonnegative Einsum Factorization](https://arxiv.org/abs/2602.02759)
*John Hood,Aaron Schein*

Main category: stat.ML

TL;DR: NNEinFact：基于einsum的乘法更新算法，用于拟合任何可表示为张量收缩的非负张量分解，支持多种损失函数和缺失数据


<details>
  <summary>Details</summary>
Motivation: 多路数据在科学领域普遍存在，但缺乏用户友好的非负张量分解工具。现有方法包括基于梯度的自动微分（在非负设置中效果不佳）、选择有限的方法实现，或从头实现自定义模型

Method: 引入NNEinFact，一种基于einsum的乘法更新算法，通过最小化用户指定的损失函数（包括(α,β)-散度）来拟合任何可表示为张量收缩的非负张量分解。用户只需用字符串指定模型

Result: NNEinFact收敛到损失的局部最小值，支持缺失数据，可在数秒内拟合具有数亿条目的张量。在真实世界张量数据的保留预测任务中，自定义模型比标准模型性能提升超过37%，测试损失比基于梯度的方法减少一半以上，收敛速度快达90倍

Conclusion: NNEinFact为非负张量分解提供了一个灵活、高效且用户友好的解决方案，使研究人员能够轻松实现和测试自定义模型，显著优于现有方法
...
### [222] [Training-Free Self-Correction for Multimodal Masked Diffusion Models](https://arxiv.org/abs/2602.02927)
*Yidong Ouyang,Panwen Hu,Zhengyan Wan,Zhe Wang,Liyan Xie,Dmitriy Bespalov,Ying Nian Wu,Guang Cheng,Hongyuan Zha,Qiang Sun*

Main category: stat.ML

TL;DR: 提出了一种无需训练的自校正框架，利用预训练掩码扩散模型的归纳偏置，在采样过程中动态修正生成错误，显著提升文本到图像生成和多模态理解任务的质量


<details>
  <summary>Details</summary>
Motivation: 掩码扩散模型在采样时同时更新多个token并将已生成token视为不可变，这可能导致早期错误累积且无法修正。现有自校正方法存在额外训练需求或依赖未对齐的似然估计等局限性

Method: 提出无需训练的自校正框架，利用预训练掩码扩散模型的归纳偏置，在不修改模型参数或引入辅助评估器的情况下，在采样过程中动态重新评估和修正已生成token

Result: 该方法显著提升了文本到图像生成和多模态理解任务的生成质量，同时减少了采样步数。框架在不同掩码扩散架构上具有良好泛化性，证明了其鲁棒性和实际适用性

Conclusion: 提出的训练免费自校正框架有效解决了掩码扩散模型中错误累积问题，通过利用模型内在归纳偏置实现动态修正，为提升生成质量提供了实用且通用的解决方案
...
### [223] [Unified Inference Framework for Single and Multi-Player Performative Prediction: Method and Asymptotic Optimality](https://arxiv.org/abs/2602.03049)
*Zhixian Zhang,Xiaotian Hou,Linjun Zhang*

Main category: stat.ML

TL;DR: 提出统一框架处理单智能体与多智能体表演性预测，开发两种估计器：RRM用于表演稳定性估计，RePPI结合重要性采样的两阶段插件估计器用于表演最优性估计，均达到渐近正态性和半参数效率界。


<details>
  <summary>Details</summary>
Motivation: 现有研究将单智能体与多智能体表演性预测视为独立现象，缺乏统一统计推断框架。表演性预测环境中模型会改变其预测的数据分布，形成复杂反馈循环，需要可靠估计工具支持动态环境中的决策制定。

Method: 1) 提出重复风险最小化(RRM)程序估计表演稳定性，建立渐近正态性和渐近效率的严格推断理论；2) 针对表演最优性，提出两阶段插件估计器，结合重新校准预测增强推断(RePPI)与重要性采样，推导分布参数和插件结果的中心极限定理。

Result: 理论分析表明：RRM估计器具有渐近正态性和渐近效率；表演最优性估计器达到半参数效率界，在温和分布误设下保持鲁棒性。统一框架将单智能体表演性视为多智能体的特例。

Conclusion: 该工作为动态表演性环境中的可靠估计和决策制定提供了原则性工具包，建立了连接单智能体与多智能体表演性预测的统一统计推断框架，开发了具有理论保证的高效估计方法。
...
### [224] [Online Conformal Prediction via Universal Portfolio Algorithms](https://arxiv.org/abs/2602.03168)
*Tuo Liu,Edgar Dobriban,Francesco Orabona*

Main category: stat.ML

TL;DR: 提出基于(1-α)-分位数损失的在线保形预测通用理论，引入线性化遗憾概念，并开发无参数UP-OCP方法，通过两资产投资组合选择实现强有限时间覆盖保证


<details>
  <summary>Details</summary>
Motivation: 现有在线保形预测方法需要手动调整学习率且依赖算法特定分析，缺乏通用理论框架和参数自由方法

Method: 建立基于(1-α)-分位数损失的遗憾到覆盖理论，识别线性化遗憾关键概念，提出UP-OCP方法通过两资产投资组合选择问题实现参数自由在线保形预测

Result: 理论证明控制线性化遗憾可保证覆盖边界，UP-OCP在多项式增长预测下仍保持强有限时间误覆盖边界，实验显示比现有基线具有更好的尺寸/覆盖权衡

Conclusion: 建立了在线保形预测的通用理论框架，提出了无参数UP-OCP方法，为在线保形预测提供了理论保证和实用算法
...
### [225] [NeuralFLoC: Neural Flow-Based Joint Registration and Clustering of Functional Data](https://arxiv.org/abs/2602.03169)
...
Result: 在功能基准测试中展现出最先进的配准和聚类性能，对缺失数据、不规则采样和噪声具有鲁棒性，同时保持可扩展性。建立了通用逼近保证和渐近一致性。

Conclusion: NeuralFLoC为功能数据聚类提供了一种有效的无监督端到端解决方案，成功解决了相位变化带来的挑战，在多个方面优于现有方法。
...
### [226] [Latent Neural-ODE for Model-Informed Precision Dosing: Overcoming Structural Assumptions in Pharmacokinetics](https://arxiv.org/abs/2602.03215)
*Benjamin Maurel,Agathe Guilloux,Sarah Zohar,Moreno Ursino,Jean-Baptiste Woillard*

Main category: stat.ML

TL;DR: 提出基于潜在常微分方程（Latent ODEs）的深度学习模型，用于肾移植后他克莫司暴露量（AUC）预测，相比传统群体药代动力学模型具有更好的灵活性和准确性。


<details>
  <summary>Details</summary>
Motivation: 当前基于非线性混合效应（NLME）的群体药代动力学模型依赖刚性预设假设，难以捕捉复杂的患者特异性动力学，可能导致模型设定错误，需要更灵活的数据驱动方法。

Method: 开发基于潜在常微分方程的深度学习模型，直接从稀疏临床数据中学习个体化药代动力学动态。通过模拟实验评估模型鲁棒性，并与两种标准方法（NLME和迭代两阶段贝叶斯法）进行基准比较。使用开发数据集（n=178）和独立外部数据集（n=75）进行临床验证。

Result: 模拟实验中，潜在ODE模型表现出优越的鲁棒性，即使基础生物学机制偏离标准假设仍保持高准确性。内部验证中，平均RMSPE为7.99%，显著优于it2B的9.24%（p<0.001）。外部验证中，RMSPE为10.82%，与两种标准估计器（11.48%和11.54%）相当。

Conclusion: 潜在ODE模型是他克莫司AUC预测的强大可靠工具，其灵活架构为个性化医疗中的下一代多模态模型提供了有前景的基础。
...
### [227] [Principled Federated Random Forests for Heterogeneous Data](https://arxiv.org/abs/2602.03258)
*Rémi Khellaf,Erwan Scornet,Aurélien Bellet,Julie Josse*

Main category: stat.ML

TL;DR: FedForest：一种面向水平分区数据的联邦随机森林算法，能够处理客户端数据异质性，通过聚合客户端统计量近似集中式分割，支持客户端指示器分割实现个性化


<details>
  <summary>Details</summary>
Motivation: 随机森林在集中式表格数据中表现出色，但缺乏有效的联邦学习适配方法。现有联邦RF方法依赖启发式策略，无法优化全局不纯度准则，特别是在数据分布偏移情况下

Method: 提出FedForest算法：1）基于聚合客户端统计量的分割过程，近似集中式算法选择的分割；2）支持客户端指示器分割，实现非参数形式的个性化；3）适用于水平分区数据，处理从协变量偏移到复杂结果偏移的异质性

Result: 理论证明分割过程能紧密近似集中式算法选择的分割。实证表明FedForest在异构基准测试中接近集中式性能，同时保持通信效率

Conclusion: FedForest为联邦随机森林提供了一种原则性方法，解决了现有启发式方法的局限性，能够有效处理客户端数据异质性并支持个性化，在保持通信效率的同时实现接近集中式的性能
...
### [228] [Score-based diffusion models for diffuse optical tomography with uncertainty quantification](https://arxiv.org/abs/2602.03449)
*Fabian Schneider,Meghdoot Mozumder,Konstantin Tamarov,Leila Taghizadeh,Tanja Tarvainen,Tapio Helin,Duc-Lam Duong*

Main category: stat.ML

TL;DR: 该论文评估了无条件表示条件评分函数（UCoS）框架在扩散光学层析成像（DOT）中的应用，提出了一种结合学习评分和模型评分的新型正则化方法，以解决建模误差问题。


<details>
  <summary>Details</summary>
Motivation: 尽管基于评分的扩散模型在贝叶斯逆问题中表现出色，但在存在建模误差和实际测量数据的情况下，对现实逆问题的深入研究仍然不足。扩散光学层析成像（DOT）作为高度病态问题，对噪声和建模误差敏感，需要更稳健的解决方案。

Method: 采用无条件表示条件评分函数（UCoS）框架，提出了一种新型正则化方法：构建混合评分函数，由学习评分（数据驱动）和模型评分（基于物理模型）组成，防止评分函数过拟合。在模拟和实验测量数据上验证该方法。

Result: 实验表明，与经典基于模型的估计相比，数据驱动的先验分布能产生低方差的后验样本，且样本中心围绕真实值，即使在高度病态问题和存在建模误差的情况下也能保持良好性能。

Conclusion: 提出的混合评分正则化方法能有效处理扩散光学层析成像中的建模误差问题，数据驱动先验在高度病态逆问题中表现出优越性能，为实际测量数据应用提供了可靠框架。
...
### [229] [Improved Analysis of the Accelerated Noisy Power Method with Applications to Decentralized PCA](https://arxiv.org/abs/2602.03682)
*Pierre Aguié,Mathieu Even,Laurent Massoulié*

Main category: stat.ML

TL;DR: 改进了加速噪声幂方法的分析，放宽了对扰动幅度的限制条件，保持了加速收敛率，并证明了分析的最优性，应用于去中心化PCA获得首个可证明加速收敛的算法。


<details>
  <summary>Details</summary>
Motivation: 现有加速噪声幂方法分析需要过于严格的扰动幅度上界，限制了实际应用。需要改进分析以在更宽松条件下保持加速收敛率。

Method: 对加速噪声幂方法进行改进分析，推导出更宽松的噪声条件，证明分析的最优性，并将结果应用于去中心化PCA算法设计。

Result: 在更温和的扰动条件下保持了加速收敛率，证明了分析的最优性（收敛率无法改进，噪声条件无法放松），设计了首个具有可证明加速收敛的去中心化PCA算法。

Conclusion: 改进的加速噪声幂方法分析放宽了实际应用限制，提供了最优收敛保证，成功应用于去中心化PCA，实现了通信成本与非加速方法相似但具有加速收敛的首个算法。
...
### [230] [Efficient Variance-reduced Estimation from Generative EHR Models: The SCOPE and REACH Estimators](https://arxiv.org/abs/2602.03730)
*Luke Solo,Matthew B. A. McDermott,William F. Parker,Bashar Ramadan,Michael C. Burkhart,Brett K. Beaulieu-Jones*

Main category: stat.ML

TL;DR: 提出两种新估计器SCOPE和REACH，利用生成模型丢弃的下一令牌概率分布，相比蒙特卡洛模拟实现10倍效率提升，降低临床部署成本。


<details>
  <summary>Details</summary>
Motivation: 现有基于自监督训练的生成模型在预测临床结局时通常使用蒙特卡洛模拟，但存在三个关键限制：1) 稀疏的估计分布难以区分患者风险水平；2) 极高的计算成本；3) 高采样方差。这些限制阻碍了生成式EHR模型在资源受限临床环境中的实际部署。

Method: 提出两种新估计器：条件结果概率求和估计器(SCOPE)和基于预期条件风险的估计器(REACH)。这两种方法都利用了标准蒙特卡洛模拟中丢弃的下一令牌概率分布。作者从理论上证明了两种估计器都是无偏的，并且REACH保证在任何模型和结果下都能实现相对于蒙特卡洛采样的方差减少。

Result: 在MIMIC-IV数据集上使用ETHOS-ARES框架进行医院死亡率预测，SCOPE和REACH仅需10-11个样本就能匹配100个样本蒙特卡洛模拟的性能（95%置信区间：[9,11]），实现了约10倍的推理成本降低且不损害校准度。对于ICU入院预测，效率提升较为有限（约1.2倍），作者将此归因于结果的"自发性"较低。

Conclusion: SCOPE和REACH方法通过利用生成模型中通常被丢弃的概率信息，显著提高了生成式EHR模型在资源受限临床环境中部署的可行性。这些方法在保持预测准确性的同时大幅降低了计算成本，为临床决策支持系统的实际应用提供了更高效的解决方案。
...
### [231] [Fast Sampling for Flows and Diffusions with Lazy and Point Mass Stochastic Interpolants](https://arxiv.org/abs/2602.03789)
*Gabriel Damsholt,Jes Frellsen,Susanne Ditlevsen*

Main category: stat.ML

TL;DR: 该论文提出了一个统一框架，用于转换不同插值计划和扩散系数的随机微分方程样本路径，并扩展了随机插值框架以包含点质量计划，在特定条件下识别出使漂移项为零的"懒惰计划"家族。


<details>
  <summary>Details</summary>
Motivation: 随机插值方法统一了流和扩散这两种流行的生成建模框架，但其主要超参数——插值计划的选择对性能有重要影响。现有方法缺乏在不同计划和扩散系数之间转换样本路径的理论基础，限制了模型的灵活性和效率。

Method: 1) 证明如何将任意扩散系数和任意计划下的SDE样本路径转换为另一任意计划和扩散系数下的唯一样本路径；2) 扩展随机插值框架以包含点质量计划（高斯基测度坍缩为点质量测度）；3) 在高斯数据假设下识别使漂移项为零的"懒惰计划"家族；4) 将理论结果应用于预训练流模型，实现无需重新训练即可减少生成步骤。

Result: 1) 建立了不同计划和扩散系数之间样本路径转换的理论基础；2) 识别出两种特殊情况：确定性采样得到扩散模型中常用的方差保持计划，统计最优SDE采样得到点质量计划；3) 在高度非高斯现实数据（图像生成）上验证了理论结果的有效性，通过应用懒惰计划转换到预训练流模型，实现了更少步骤的图像生成而无需重新训练。

Conclusion: 该研究为随机插值方法提供了强大的理论工具，使得在不同插值计划和扩散系数之间转换成为可能。特别是识别出的"懒惰计划"家族为优化生成过程提供了新途径，在保持模型性能的同时显著减少了推理计算量，具有重要的实际应用价值。
...
### [232] [Preference-based Conditional Treatment Effects and Policy Learning](https://arxiv.org/abs/2602.03823)
*Dovid Parnas,Mathieu Even,Julie Josse,Uri Shalit*

Main category: stat.ML

TL;DR: 提出基于偏好的条件处理效应估计新框架CPTE，仅需结果排序而非具体数值，统一多种应用场景，解决非可识别性问题并提供高效估计方法


<details>
  <summary>Details</summary>
Motivation: 传统处理效应估计需要具体结果数值，限制了在多元、序数或偏好驱动结果场景的应用。需要更灵活的框架来处理异质性效应，并统一条件必要性概率、条件胜率比、广义成对比较等应用

Method: 提出条件偏好处理效应(CPTE)框架，仅要求结果在偏好规则下排序。通过匹配、分位数和分布回归进行估计，设计高效影响函数估计器校正插件偏差并最大化策略价值

Result: CPTE为先前不可识别的估计量提供了新的可识别性条件，合成和半合成实验显示明显的性能提升和实际影响

Conclusion: CPTE框架通过仅需排序的偏好方法，灵活建模异质性处理效应，统一多种应用场景，解决非可识别性问题，并提供高效估计策略，具有实际应用价值
